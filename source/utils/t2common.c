...
void freeDataModelParam(void *data) 
{
    if (data) 
    {
        ...
        if (param->reference) free(param->reference);
...
void freeDataModelTable(void *data) 
{
    if (data) 
    {
        ...
        if (table->reference) free(table->reference);
...
{
    if (!pattern || !paramName) return false;
    ...
    while (*pattern && *paramName) 
    {
        if (*pattern == '*') 
        {
            ...
            while (*paramName && *paramName != *pattern) paramName++;
        } else {
            if (*pattern != *paramName) return false;
            pattern++;
            paramName++;
