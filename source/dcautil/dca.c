...
static bool check_rotated_logs =
    false; // using this variable to indicate whether it needs to check the
           // rotated logs or not . Initialising it with false.
static bool firstreport_after_bootup =
    false; // the rotated logs check should run only for the first time.
// Using same error code used by safeclib for buffer overflow for easy metrics
// collection safeclib is not compleately introduced in T2 but to be in sync
// with legacy
...
/** @brief This API processes the top command log file patterns to retrieve load
 * average and process usage.
 *
...
int processTopPattern(rdkList_t *pchead, Vector *grepResultList) {
  ...
  while (NULL != tlist) {
    ...
    if (NULL != tmp) {
      if ((NULL != tmp->header) &&
          (NULL != strstr(tmp->header, "Load_Average"))) {
...
static T2ERROR processTr181Objects(rdkList_t *pchead) {
  ...
  for (tlist = pchead; tlist != NULL; tlist = rdk_list_find_next_node(tlist)) {
    ...
    tmp = tlist->m_pUserData;
    if (NULL != tmp) {
      if (NULL != tmp->header && NULL != tmp->pattern &&
          strlen(tmp->pattern) < TR181BUF_LENGTH && NULL == tmp->data) {

        // Check whether given object has multi-instance token, if no token
        // found it will be treated as a single instance object Or if more than
        // one token found, skip the object as it is not valid/supported Check
        // for first multi-instance token
        ...
        if (NULL == tck) { // Single instance check
          ...
          if (T2ERROR_SUCCESS == ret_val) {
            ...
            T2Debug("Telemetry data source not found. Type = <message_bus>. "
                    "Content string = %s\n",
                    tmp->pattern);
          }
          ...
          if (NULL == tck) {
            ...
            snprintf(tr181objBuff, sizeof(tr181objBuff), "%sNumberOfEntries",
                     tmp->pattern);
            ...
            if (T2ERROR_SUCCESS == ret_val) {
              ...
              if (obj_count > 0) {
                for (i = 1; i <= obj_count; i++) {
                  ...
                  snprintf(tr181objBuff, sizeof(tr181objBuff), "%s%d%s",
                           tmp->pattern, i,
                           (tmp->pattern + length + DELIMITER_SIZE));
                  ret_val = getParameterValue(tr181objBuff, &tr181dataBuff);
                  if (T2ERROR_SUCCESS == ret_val) {
                    appendData(tmp, tr181dataBuff);
                  } else {
                    T2Debug("Telemetry data source not found. Type = "
                            "<message_bus>. Content string = %s\n",
                            tr181objBuff);
                  }
                  free(tr181dataBuff);
                  tr181dataBuff = NULL;
              ...
              T2Debug("Failed to get NumberOfEntries. Type = <message_bus>. "
                      "Content string = %s\n",
                      tr181objBuff);
            }
          } else {
            T2Debug("Skipping Telemetry object due to invalid format. Type = "
                    "<message_bus>. Content string = %s\n",
                    tmp->pattern);
          }
      ...
      if (tmp->pattern && grepResultList != NULL) {
        if (tmp->d_type == OCCURENCE) {
          if (tmp->count != 0) {
            char tmp_str[5] = {0};
            if (tmp->count > 9999) {
              T2Debug("Count value is %d higher than limit of 9999 changing "
                      "the value to %d to track buffer overflow",
...
static int getSplitParameterValue(char *line, pcdata_t *pcnode) {
  ...
  if (strFound != NULL) {
    ...
    if (tlen > plen) {
      ...
      // Ideally component should not print logs with empty values but we have
      // to consider logs from OSS components
...
int getErrorCode(char *str, char *ec) {
  ...
  char tmpEC[LEN] = {0};
  while (str[i] != '\0') {
    if (len >= 4 && str[i] == 'R' && str[i + 1] == 'D' && str[i + 2] == 'K' &&
        str[i + 3] == '-') {
      i += 4;
      j = 0;
      if (str[i] == '0' || str[i] == '1') {
...
static int handleRDKErrCodes(rdkList_t **rdkec_head, char *line) {
  ...
  if (strcmp(err_code, "") != 0) {
    ...
    if (NULL != tnode) {
      ...
      /* Args:  rdkList_t **pch, char *pattern, char *header, DType_t dtype, int
       * count, char *data, bool trim, char *regex */
 ...
 * @retval Return 0 upon success, -1 on failure.
 */
static int processCountPattern(hash_map_t *logSeekMap, char *logfile,
                               rdkList_t *pchead, rdkList_t **rdkec_head,
                               int *firstSeekFromEOF, bool check_rotated_logs) {
  T2Debug("%s ++in\n", __FUNCTION__);
  char temp[MAXLINE];
  T2Debug("Read from log file %s \n", logfile);
  while (getLogLine(logSeekMap, temp, MAXLINE, logfile, firstSeekFromEOF,
                    check_rotated_logs) != NULL) {
    ...
    if (NULL != pc_node) {
      ...
      // This is a RDK-V specific calls for reporting RDK error codes .
      // Retaining for video porting
...
/**
 * @brief Generic pattern function based on pattern to call top/count or using
 * ccsp message bus.
 *
 ...
 */
static int processPattern(char **prev_file, char *logfile,
                          rdkList_t **rdkec_head, rdkList_t *pchead,
                          Vector *grepResultList, hash_map_t *logSeekMap,
                          int *firstSeekFromEOF, bool check_rotated_logs) {
  ...
  if (NULL != logfile) {
    ...
    if ((NULL == *prev_file) || (strcmp(*prev_file, logfile) != 0)) {
      if (NULL == *prev_file) {
        *prev_file = strdup(logfile);
        if (*prev_file == NULL) {
          T2Error(
              "Insufficient memory available to allocate duplicate string %s\n",
              logfile);
        }
      } else {
    ...
    if (NULL != pchead) {
      if (0 == strcmp(logfile, "top_log.txt")) {
        ...
        processCountPattern(logSeekMap, logfile, pchead, rdkec_head,
                            firstSeekFromEOF, check_rotated_logs);
        if (grepResultList != NULL) {
          addToVector(pchead, grepResultList);
        } else {
          addToJson(pchead);
...
/** @description: Main logic function to parse sorted vector list and to process
 * the pattern list
 *  @param filename
 *  @return -1 on failure, 0 on success
 */
static int parseMarkerList(char *profileName, Vector *vMarkerList,
  ...
  if ((gsProfile->execCounter == 1) &&
      (firstreport_after_bootup ==
       false)) { // checking the execution count and first report after bootup
                 // because after config reload again execution count will get
                 // initialised and again reaches 1. check_rotated logs flag is
                 // to check the rotated log files even when seekvalue is less
                 // than filesize for the first time.
  ...
  for (var = 0; var < vCount; ++var) {
    ...
    if (is_skip_param == 0) {
      if (0 == insertPCNode(&pchead, temp_pattern, temp_header, dtype, 0, NULL,
                            trim, regex)) {
        processPattern(&prevfile, filename, &rdkec_head, pchead, grepResultList,
                       gsProfile->logFileSeekMap,
                       &(markerList->firstSeekFromEOF), check_rotated_logs);
        pchead = NULL;
      }
    } else {
      T2Debug(
          "Current iteration for this parameter needs to be excluded, but the "
          "seek values needs to be updated in case of logfile based marker\n");
...
int getDCAResultsInJson(char *profileName, void *markerList,
...
int getDCAResultsInVector(char *profileName, Vector *vecMarkerList,
                          Vector **grepResultList, bool check_rotated) {
  ...
  if (NULL != vecMarkerList) {
    ...
    if ((rc = parseMarkerList(profileName, vecMarkerList, *grepResultList,
                              check_rotated)) == -1) {
