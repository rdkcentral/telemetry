...
static T2ERROR addParameter(ProfileXConf *profile, const char *name,
  ...
  } else if (fileName == NULL) // Event Marker
  {
    ...
    } else if (accumulateSuffix != NULL &&
               strcmp(accumulateSuffix, ACCUMULATE_MARKER_SUFFIX) == 0) {
  ...
  } else // Grep Marker
  {
    ...
    gMarker->firstSeekFromEOF =
        0; // memset will already set to 0 just a safeguard
...
T2ERROR processConfigurationXConf(char *configData,
  ...
  cJSON *telemetry_data =
      cJSON_GetObjectItem(json_root, "urn:settings:TelemetryProfile");

  //    cJSON *jprofileID = cJSON_GetObjectItem(telemetry_data, "id");
  cJSON *jprofileName =
      cJSON_GetObjectItem(telemetry_data, "telemetryProfile:name");
  cJSON *juploadUrl =
      cJSON_GetObjectItem(telemetry_data, "uploadRepository:URL");
  ...
  //    if(jprofileID == NULL || jprofileName == NULL || juploadUrl == NULL ||
  //    jschedule == NULL || profileData == NULL || marker_count == 0)
  if (jprofileName == NULL || juploadUrl == NULL || jschedule == NULL ||
      profileData == NULL || marker_count == 0) {
  ...
  T2Info("Received profile name : %s with interval of : %d secs and upload url "
         ": %s \n",
         jprofileName->valuestring, reportIntervalInSec,
         juploadUrl->valuestring);
  ...
  for (markerIndex = 0; markerIndex < marker_count; markerIndex++) {
    ...
    if (pSubitem != NULL) {
      ...
      if (header != NULL && content != NULL) {
        ...
        } else if (!strncmp(logfile, MT_TR181PARAM_PATTERN,
                            MT_TR181PATAM_PATTERN_LENGTH)) {
          ret = addParameter(profile, header, content, NULL, -1);
        } else if (!strncmp(logfile, MT_EVENT_PATTERN,
                            MT_EVENT_PATTERN_LENGTH)) {
