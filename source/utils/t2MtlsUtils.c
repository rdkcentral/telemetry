...
// Prepare certs required for mTls commmunication
  ...
  bool ret = false;
  ret = getDevicePropertyData("UseSEBasedCert", UseHWBasedCert,
                              sizeof(UseHWBasedCert));
  if (ret == true) {
 ...
 * So there are two variants of the same function depending on the logic used to
 * retrieve values.
 */
#if defined(MTLS_FROM_ENV)
  ...
  if (access(dynamicMtlsCert, F_OK) != -1) { // Dynamic cert
    *certName = strdup(dynamicMtlsCert);
    T2Info("Using xpki Dynamic Certs connection certname: %s\n",
           dynamicMtlsCert);
#ifdef LIBRDKCONFIG_BUILD
    if (rdkconfig_get(&MtlsBuf, &MtlsSize, dynamicMtlsDestFile) ==
        RDKCONFIG_FAIL) {
    ...
    *phrase = (char *)MtlsBuf;
    ...
    T2Info("Using xpki Static Certs connection certname: %s\n", staticMtlsCert);
    T2Info("xPKIStaticCert: /etc/ssl/certs/staticDeviceCert.pk12 uptime %.2lf "
           "seconds,telemetry",
           get_system_uptime());
    *certName = strdup(staticMtlsCert);
#ifdef LIBRDKCONFIG_BUILD
    ...
    if (rdkconfig_get(&MtlsBuf, &MtlsSize, staticMtlsDestFile) ==
        RDKCONFIG_FAIL) {
