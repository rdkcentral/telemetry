 ...
 * @brief       An rbus property is a name/value pair.  It allows assigning a
 * name (e.g. path) to an rbus value. An rbusProperty_t is a reference counted
 * handle to an rbus property.
 *
 * This API is not thread-safe.
 * All instances of types rbusValue_t, rbusProperty_t, and rbusObject_t are
 * referenced counted. Instances of these types may have private references to
 * other instances of these types. There are read methods in this api that
 * return references to these private instances. There are write methods in this
 * api that release current references and retain new ones. When this happens
 * any reference held by the user becomes unlinked from the parent instance. The
 * instance will also be destroyed unless the user retains it for themselves.
 * @{
 */
...
/** @fn void rbusProperty_Init(rbusProperty_t* pproperty, char const* name,
 * rbusValue_t value)
 ...
 *  @param  pproperty Optional reference to an address where the new property
 * will be assigned. The caller is responsible for releasing the property with
 * rbusProperty_Release
 *  @param  name Optional name to assign the property.  The name is
 * dublicated/copied. If NULL is passed the property's name will be NULL.
 *  @param  value Optional value to assign the property.  The property will
 * retain ownership of the value. If the value is NULL, the property's value
 * will be NULL.
 *  @return The new property
 */
rbusProperty_t rbusProperty_Init(rbusProperty_t *pproperty, char const *name,
                                 rbusValue_t value);
...
rbusProperty_t rbusProperty_InitTime(char const *name,
                                     rbusDateTime_t const *tv);
rbusProperty_t rbusProperty_InitString(char const *name, char const *s);
rbusProperty_t rbusProperty_InitBytes(char const *name, uint8_t const *bytes,
                                      int len);
rbusProperty_t rbusProperty_InitProperty(char const *name,
                                         struct _rbusProperty *property);
rbusProperty_t rbusProperty_InitObject(char const *name,
                                       struct _rbusObject *object);
///@}

/** @fn void rbusProperty_Retain(rbusProperty_t property)
 *  @brief Take shared ownership of the property.  This allows a property to
 * have multiple owners.  The first owner obtains ownership with
 * rbusProperty_Init. Additional owners can be assigned afterwards with
 * rbusProperty_Retain. Each owner must call rbusProperty_Release once done
 * using the property.
 ...
 *  @brief Release ownership of the property.  This must be called when done
 * using a property that was retained with either rbusProperty_Init or
 * rbusProperty_Retain.
...
/** @fn void rbusProperty_Compare(rbusProperty_t property1, rbusProperty_t
 * property2)
 *  @brief Compare two properties for equality.  They are equal if both their
 ...
 *  @return The name of the property.  To avoid copies, this is a reference to
 * the underlying name.
 */
 ...
 *  @param property A property.
 *  @return The value of the property.  If the caller intends to store this
 * value, it should take ownership by calling rbusValue_Retain and then when its
 * done with it, call rbusValue_Release.
 */
 ...
 *  @brief  These functions get the value of a property to a specific type and
 * data.
 ...
 * @brief These functions check that the type being requested matches the
 * value's actual type, and returns the data if it does.  If the type does not
...
rbusValueError_t rbusProperty_GetByteEx(rbusProperty_t property,
                                        unsigned char *u);
...
rbusValueError_t rbusProperty_GetUInt16Ex(rbusProperty_t property,
                                          uint16_t *u16);
rbusValueError_t rbusProperty_GetInt32Ex(rbusProperty_t property, int32_t *i32);
rbusValueError_t rbusProperty_GetUInt32Ex(rbusProperty_t property,
                                          uint32_t *u32);
rbusValueError_t rbusProperty_GetInt64Ex(rbusProperty_t property, int64_t *i64);
rbusValueError_t rbusProperty_GetUInt64Ex(rbusProperty_t property,
                                          uint64_t *u64);
...
rbusValueError_t rbusProperty_GetTimeEx(rbusProperty_t property,
                                        rbusDateTime_t const **tv);
rbusValueError_t rbusProperty_GetStringEx(rbusProperty_t property,
                                          char const **s, int *len);
rbusValueError_t rbusProperty_GetBytesEx(rbusProperty_t property,
                                         uint8_t const **bytes, int *len);
rbusValueError_t rbusProperty_GetPropertyEx(rbusProperty_t property,
                                            struct _rbusProperty **prop);
rbusValueError_t rbusProperty_GetObjectEx(rbusProperty_t property,
                                          struct _rbusObject **obj);
///@}

/** @name rbusProperty_Set[Type]
 *  @brief  These functions set the value of a property to a specific type and
 * data.
 *  @param property A property.
 *  @param value The type specific data to set
 */
...
void rbusProperty_SetBytes(rbusProperty_t property, uint8_t const *bytes,
                           int len);
 ...
 *  @param name Optional name to assign the new property.  The name is
 * dublicated/copied. If NULL is passed the property's name will be NULL.
 *  @param value The type specific data to set
 * @return The new property.  If the property param was NULL, the called must
 * release this.
 */
///@{
rbusProperty_t rbusProperty_AppendBoolean(rbusProperty_t property,
                                          char const *name, bool b);
rbusProperty_t rbusProperty_AppendChar(rbusProperty_t property,
                                       char const *name, char c);
rbusProperty_t rbusProperty_AppendByte(rbusProperty_t property,
                                       char const *name, unsigned char u);
rbusProperty_t rbusProperty_AppendInt8(rbusProperty_t property,
                                       char const *name, int8_t i8);
rbusProperty_t rbusProperty_AppendUInt8(rbusProperty_t property,
                                        char const *name, uint8_t u8);
rbusProperty_t rbusProperty_AppendInt16(rbusProperty_t property,
                                        char const *name, int16_t i16);
rbusProperty_t rbusProperty_AppendUInt16(rbusProperty_t property,
                                         char const *name, uint16_t u16);
rbusProperty_t rbusProperty_AppendInt32(rbusProperty_t property,
                                        char const *name, int32_t i32);
rbusProperty_t rbusProperty_AppendUInt32(rbusProperty_t property,
                                         char const *name, uint32_t u32);
rbusProperty_t rbusProperty_AppendInt64(rbusProperty_t property,
                                        char const *name, int64_t i64);
rbusProperty_t rbusProperty_AppendUInt64(rbusProperty_t property,
                                         char const *name, uint64_t u64);
rbusProperty_t rbusProperty_AppendSingle(rbusProperty_t property,
                                         char const *name, float f32);
rbusProperty_t rbusProperty_AppendDouble(rbusProperty_t property,
                                         char const *name, double f64);
rbusProperty_t rbusProperty_AppendTime(rbusProperty_t property,
                                       char const *name,
                                       rbusDateTime_t const *tv);
rbusProperty_t rbusProperty_AppendString(rbusProperty_t property,
                                         char const *name, char const *s);
rbusProperty_t rbusProperty_AppendBytes(rbusProperty_t property,
                                        char const *name, uint8_t const *bytes,
                                        int len);
rbusProperty_t rbusProperty_AppendProperty(rbusProperty_t property,
                                           char const *name,
                                           struct _rbusProperty *p);
rbusProperty_t rbusProperty_AppendObject(rbusProperty_t property,
                                         char const *name,
                                         struct _rbusObject *o);
///@}

/** @fn char* rbusProperty_GetNext(rbusProperty_t property)
 *  @brief Get the next property from the list.  Properties can be linked
 * together into a list. Given a property in that list, the next property in the
 * list can be obtained with this method.
 *  @param property A property.
 *  @return The next property in the list.  If the caller intends to store this
 * property, it should take ownership by calling rbusProperty_Retain and then
 ...
 *  @brief Set the next property in the list.  Properties can be linked together
 into a list.
 *         Given a property in that list, the next property in the list can be
 set with this method.
 *         If next had been previously set, then ownership of the former next
