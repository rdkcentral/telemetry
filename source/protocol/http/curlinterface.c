...
typedef enum _ADDRESS_TYPE { ADDR_UNKNOWN, ADDR_IPV4, ADDR_IPV6 } ADDRESS_TYPE;

static void sendOverHTTPInit() { pthread_mutex_init(&curlFileMutex, NULL); }
...
static T2ERROR setHeader(CURL *curl, const char *destURL,
                         struct curl_slist **headerList,
                         childResponse *childCurlResponse) {
...
T2ERROR sendReportOverHTTP(char *httpUrl, char *payload, pid_t *outForkedPid) {
...
#ifndef LIBRDKCERTSEL_BUILD
  if (mtls_enable == true &&
      T2ERROR_SUCCESS != getMtlsCerts(&pCertFile, &pCertPC)) {
  ...
  if (childPid == 0) {
    ...
    if (curl) {
      ...
      if (setHeader(curl, httpUrl, &headerList, &childCurlResponse) !=
          T2ERROR_SUCCESS) {
      ...
      do {
        ...
        pCertURI = NULL;
        curlGetCertStatus =
            rdkcertselector_getCert(curlCertSelector, &pCertURI, &pCertPC);
...
#endif
          if ((mtls_enable == true) &&
              (setMtlsHeaders(curl, pCertFile, pCertPC, &childCurlResponse) !=
               T2ERROR_SUCCESS)) {
            curl_easy_cleanup(curl); // CID 189985: Resource leak
            goto child_cleanReturn;
          }
      ...
      } while (rdkcertselector_setCurlStatus(curlCertSelector, curl_code,
                                             (const char *)httpUrl) ==
               TRY_ANOTHER);
#endif
  ...
  child_cleanReturn:
    ...
    // if(childCurlResponse.curlStatus == CURLE_OK) commenting this as we are
    // observing childCurlResponse.curlStatus as 1, from line with CID 143029
    // Unchecked return value from library
    T2Info("The return status from the child with pid %d SetopCode: %s; "
           "ResponseCode : %s; HTTP_CODE : %ld; Line Number : %d \n",
           childPid, curl_easy_strerror(childCurlResponse.curlSetopCode),
           curl_easy_strerror(childCurlResponse.curlResponse),
           childCurlResponse.http_code, childCurlResponse.lineNumber);
    if (childCurlResponse.http_code == 200 ||
        childCurlResponse.curlResponse == CURLE_OK) {
      ret = T2ERROR_SUCCESS;
