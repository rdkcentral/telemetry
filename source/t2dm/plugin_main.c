...
int ANSC_EXPORT_API COSA_Init(
    ULONG uMaxVersionSupported,
    void *hCosaPlugInfo /* PCOSA_PLUGIN_INFO passed in by the caller */
) {
  PCOSA_PLUGIN_INFO pPlugInfo = (PCOSA_PLUGIN_INFO)hCosaPlugInfo;
  COSAGetInterfaceByNameProc pGetInterfaceByNameProc =
      (COSAGetInterfaceByNameProc)NULL;
  ...
  pGetInterfaceByNameProc =
      (COSAGetInterfaceByNameProc)pPlugInfo->AcquireFunction(
  ...
  g_GetMessageBusHandle =
      (COSAGetHandleProc)pPlugInfo->AcquireFunction("COSAGetMessageBusHandle");
  ...
  g_GetSubsystemPrefix = (COSAGetSubsystemPrefixProc)pPlugInfo->AcquireFunction(
  ...
  if (g_GetSubsystemPrefix != NULL) {
    ...
    if ((tmpSubsystemPrefix = g_GetSubsystemPrefix(g_pDslhDmlAgent))) {
      strncpy(g_SubSysPrefix_Irep, tmpSubsystemPrefix,
              sizeof(g_SubSysPrefix_Irep) - 1);
