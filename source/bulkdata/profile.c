  ...
  if (data != NULL) {
    ...
    if (profile->t2HTTPDest) {
      ...
      if (profile->t2HTTPDest->RequestURIparamList) {
        Vector_Destroy(profile->t2HTTPDest->RequestURIparamList,
                       freeRequestURIparam);
      }
    ...
    if (profile->t2RBUSDest) {
      if (profile->t2RBUSDest->rbusMethodName) {
        memset(profile->t2RBUSDest->rbusMethodName, 0,
               strlen(profile->t2RBUSDest->rbusMethodName));
        free(profile->t2RBUSDest->rbusMethodName);
        profile->t2RBUSDest->rbusMethodName = NULL;
      }
      if (profile->t2RBUSDest->rbusMethodParamList) {
        // TBD determine whether data is simple string before passing free as
        // cleanup function
...
void getMarkerCompRbusSub(bool subscription) {
  ...
  getComponentMarkerList(T2REPORTCOMPONENT,
                         (void **)&eventMarkerListForComponent);
    ...
    for (i = 0; i < length; ++i) {
      ...
      if (markerName) {
        ...
        T2Debug("%d T2RbusEventReg with name = %s: subscription = %s ret %d \n",
                i, markerName, (subscription ? "Subscribe" : "Un-Subscribe"),
                ret);
      } else {
...
static void *CollectAndReport(void *data) {
  T2Debug("%s ++in\n", __FUNCTION__);
  if (data == NULL) {
    T2Error("data passed is NULL can't identify the profile, existing from "
            "CollectAndReport\n");
    return NULL;
  }
  Profile *profile = (Profile *)data;
  ...
  do {
    ...
    if (profile->name == NULL || profile->encodingType == NULL ||
        profile->protocol == NULL) {
      T2Error("Incomplete profile parameters\n");
      if (profile->triggerReportOnCondition) {
        T2Info(" Unlock trigger condition mutex and set report on condition to "
               "false \n");
    ...
    if (!strcmp(profile->encodingType, "JSON") ||
        !strcmp(profile->encodingType, "MessagePack")) {
      ...
      if (jsonEncoding->reportFormat != JSONRF_KEYVALUEPAIR) {
        ...
        if (profile->triggerReportOnCondition) {
          T2Info(" Unlock trigger condition mutex and set report on condition "
                 "to false \n");
          profile->triggerReportOnCondition = false;
          pthread_mutex_unlock(&profile->triggerCondMutex);
        } else {
      ...
      if (T2ERROR_SUCCESS != initJSONReportProfile(&profile->jsonReportObj,
                                                   &valArray,
                                                   profile->RootName)) {
        ...
        if (profile->staticParamList != NULL &&
            Vector_Size(profile->staticParamList) > 0) {
        ...
        if (profile->paramList != NULL && Vector_Size(profile->paramList) > 0) {
          ...
          if (profileParamVals != NULL) {
            encodeParamResultInJSON(valArray, profile->paramList,
                                    profileParamVals);
          }
          Vector_Destroy(profileParamVals, freeProfileValues);
        }
        if (profile->gMarkerList != NULL &&
            Vector_Size(profile->gMarkerList) > 0) {
          getGrepResults(profile->name, profile->gMarkerList, &grepResultList,
                         profile->bClearSeekMap,
                         false); // Passing 5th argument as false so that it
                                 // doesn't check rotated logs for the first
                                 // reporting after bootup for multiprofiles.
          encodeGrepResultInJSON(valArray, grepResultList);
          Vector_Destroy(grepResultList, freeGResult);
        }
        if (profile->eMarkerList != NULL &&
            Vector_Size(profile->eMarkerList) > 0) {
        ...
        if (root != NULL) {
          ...
          if (cJSON_GetArraySize(array) == 0) {
            T2Warning("Array size of Report is %d. Report is empty. Cannot "
                      "send empty report\n",
                      cJSON_GetArraySize(array));
            profile->reportInProgress = false;
            if (profile->triggerReportOnCondition) {
              T2Info(" Unlock trigger condition mutex and set report on "
                     "condition to false \n");
              profile->triggerReportOnCondition = false;
              pthread_mutex_unlock(&profile->triggerCondMutex);
              if (profile->callBackOnReportGenerationComplete)
        ...
        if (strcmp(profile->protocol, "HTTP") == 0 ||
            strcmp(profile->protocol, "RBUS_METHOD") == 0) {
          char *httpUrl = NULL;
          if (strcmp(profile->protocol, "HTTP") == 0) {
            httpUrl = prepareHttpUrl(
                profile->t2HTTPDest); /* Append URL with http properties */
            if (profile->maxUploadLatency > 0) {
              ...
              n = pthread_cond_timedwait(&profile->reportcond,
                                         &profile->reportMutex,
                                         &profile->maxlatencyTime);
              if (n == ETIMEDOUT) {
                ...
                if (profile->triggerReportOnCondition) {
                  ...
                  if (profile->callBackOnReportGenerationComplete != NULL) {
                    T2Debug("Calling callback function "
                            "profile->callBackOnReportGenerationComplete \n");
                    profile->callBackOnReportGenerationComplete(profile->name);
                  }
                } else {
                  T2Debug(" profile->triggerReportOnCondition is not set \n");
            ...
            if (profile->maxUploadLatency > 0) {
              ...
              if (n == ETIMEDOUT) {
                T2Info("TIMEOUT for maxUploadLatency of profile %s\n",
                       profile->name);
                ret = sendReportsOverRBUSMethod(
                    profile->t2RBUSDest->rbusMethodName,
                    profile->t2RBUSDest->rbusMethodParamList, jsonReport);
              } else {
                T2Error("Profile : %s pthread_cond_timedwait ERROR!!!\n",
                        profile->name);
                pthread_mutex_unlock(&profile->reportMutex);
          ...
          if ((ret == T2ERROR_FAILURE &&
               strcmp(profile->protocol, "HTTP") == 0) ||
            ...
            if (profile->cachedReportList != NULL &&
                Vector_Size(profile->cachedReportList) >= MAX_CACHED_REPORTS) {
              while (Vector_Size(profile->cachedReportList) >
                     MAX_CACHED_REPORTS) {
                int pos = Vector_Size(profile->cachedReportList);
                T2Info("Max Cached Reports Limit Exceeded, Removing the extra "
                       "reports\n");
                char *extraCachedreport =
                    (char *)Vector_At(profile->cachedReportList, (pos - 1));
                Vector_RemoveItem(profile->cachedReportList,
                                  (void *)extraCachedreport, NULL);
                free(extraCachedreport);
              }
              T2Info("Max Cached Reports Limit Reached, Overwriting third "
                     "recent report\n");
              char *thirdCachedReport = (char *)Vector_At(
                  profile->cachedReportList, MAX_CACHED_REPORTS - 3);
              Vector_RemoveItem(profile->cachedReportList,
                                (void *)thirdCachedReport, NULL);
            ...
            T2Info("Report Cached, No. of reportes cached = %lu\n",
                   (unsigned long)Vector_Size(profile->cachedReportList));
            // Save messages from profile->cachedReportList to a file in
            // persistent location .
            saveCachedReportToPersistenceFolder(profile->name,
                                                profile->cachedReportList);

            if (strcmp(profile->protocol, "RBUS_METHOD") == 0) {
              profile->SendErr++;
              if (profile->SendErr > 3 &&
                  !(rbusCheckMethodExists(
                      profile->t2RBUSDest
                          ->rbusMethodName))) { // to delete the profile in the
                                                // next CollectAndReport or
                                                // triggercondition
                ...
                T2Error("ERROR: no method provider; profile will be deleted: "
                        "%s %s\n",
                        profile->name, profile->t2RBUSDest->rbusMethodName);
                if (T2ERROR_SUCCESS != deleteProfile(profile->name)) {
                  T2Error("Failed to delete profile after RBUS_METHOD "
                          "failures: %s\n",
                          profile->name);
                  T2Info("%s --out\n", __FUNCTION__);
                  // return NULL;
          ...
          } else if (profile->cachedReportList != NULL &&
                     Vector_Size(profile->cachedReportList) > 0) {
            T2Info("Trying to send  %lu cached reports\n",
                   (unsigned long)Vector_Size(profile->cachedReportList));
            if (strcmp(profile->protocol, "HTTP") == 0) {
              ret =
                  sendCachedReportsOverHTTP(httpUrl, profile->cachedReportList);
            } else {
              ret = sendCachedReportsOverRBUSMethod(
                  profile->t2RBUSDest->rbusMethodName,
                  profile->t2RBUSDest->rbusMethodParamList,
                  profile->cachedReportList);
            }
    ...
    T2Info("Elapsed Time for : %s = %lu.%lu (Sec.NanoSec)\n", profile->name,
           (unsigned long)elapsedTime.tv_sec, elapsedTime.tv_nsec);
...
void NotifyTimeout(const char *profileName, bool isClearSeekMap) {
  ...
  if (profile->enable && !profile->reportInProgress) {
    ...
    /* To avoid previous report thread to go into zombie state, mark it
     * detached. */
    if (profile->threadExists) {
      ...
      pthread_create(&profile->reportThread, NULL, CollectAndReport,
                     (void *)profile);
  } else {
    T2Warning("Either profile is disabled or report generation still in "
              "progress - ignoring the request\n");
  }
...
T2ERROR Profile_storeMarkerEvent(const char *profileName, T2Event *eventInfo) {
  ...
  for (; eventIndex < Vector_Size(profile->eMarkerList); eventIndex++) {
    EventMarker *tempEventMarker =
        (EventMarker *)Vector_At(profile->eMarkerList, eventIndex);
  ...
  if (lookupEvent != NULL) {
    ...
    case MTYPE_COUNTER:
      ...
      if (lookupEvent->reportTimestampParam == REPORTTIMESTAMP_UNIXEPOCH) {
        ...
        T2Debug("Timestamp for %s is %s\n", lookupEvent->markerName_CT,
                lookupEvent->timestamp);
      }
      break;
    ...
    case MTYPE_ACCUMULATE:
      ...
      if (arraySize < MAX_ACCUMULATE) {
        Vector_PushBack(lookupEvent->u.accumulatedValues,
                        strdup(eventInfo->value));
        ...
        T2Warning("Max size of the array has been reached appending warning "
                  "message : %s\n",
                  MAX_ACCUMULATE_MSG);
        Vector_PushBack(lookupEvent->u.accumulatedValues,
                        strdup(MAX_ACCUMULATE_MSG));
    ...
    T2Error(
        "Event doens't match any marker information, shouldn't come here\n");
    return T2ERROR_FAILURE;
  }
...
T2ERROR enableProfile(const char *profileName) {
  ...
  if (profile->enable) {
    ...
    for (; emIndex < Vector_Size(profile->eMarkerList); emIndex++) {
      ...
      addT2EventMarker(eMarker->markerName, eMarker->compName, profile->name,
                       eMarker->skipFreq);
    }
    if (registerProfileWithScheduler(
            profile->name, profile->reportingInterval,
            profile->activationTimeoutPeriod, profile->deleteonTimeout, true,
            profile->reportOnUpdate, profile->firstReportingInterval,
  ...
  for (; profileIndex < Vector_Size(profileList); profileIndex++) {
    ...
    if (tempProfile->enable) {
      ...
      for (; emIndex < Vector_Size(tempProfile->eMarkerList); emIndex++) {
        ...
        addT2EventMarker(eMarker->markerName, eMarker->compName,
                         tempProfile->name, eMarker->skipFreq);
      }
    }
  }
  pthread_mutex_unlock(&plMutex);
...
T2ERROR deleteAllProfiles(bool delFromDisk) {
  ...
  pthread_mutex_lock(&plMutex);
  if (profileList == NULL) {
    T2Error("profile list is not initialized yet or profileList is empty, "
            "ignoring\n");
    pthread_mutex_unlock(&plMutex);
    return T2ERROR_FAILURE;
  }
  ...
  if (delFromDisk == true) {
    removeProfileFromDisk(REPORTPROFILES_PERSISTENCE_PATH,
...
static void loadReportProfilesFromDisk() {
  fetchLocalConfigs(SHORTLIVED_PROFILES_PATH,
                    NULL); // API used for creating /tmp/t2reportprofiles dir
#if defined(FEATURE_SUPPORT_WEBCONFIG)
  T2Info("loadReportProfilesFromDisk \n");
  char filePath[REPORTPROFILES_FILE_PATH_SIZE] = {'\0'};
  snprintf(filePath, sizeof(filePath), "%s%s", REPORTPROFILES_PERSISTENCE_PATH,
           MSGPACK_REPORTPROFILES_PERSISTENT_FILE);
  ...
  if (fp != NULL) {
    ...
    if (fread(msgpack.msgpack_blob, sizeof(char), msgpack.msgpack_blob_size,
              fp) < (size_t)msgpack.msgpack_blob_size) {
  ...
  if (strcmp(paramValue, "DO_NOT_SHARE") == 0) {
    T2Warning(
        "PrivacyModes is DO_NOT_SHARE. Reportprofiles is not supported\n");
    free(paramValue);
    paramValue = NULL;
    return;
  }
  ...
  for (; configIndex < Vector_Size(configList); configIndex++) {
    ...
    if (T2ERROR_SUCCESS == processConfiguration(&config->configData,
                                                config->name, NULL, &profile)) {
  ...
  T2Info("Completed processing %lu profiles on the disk,trying to fetch "
...
T2ERROR appendTriggerCondition(Profile *tempProfile, const char *referenceName,
  ...
  // It gets released from CollectAndReport thread once trigger condition based
  // report is sent out. Reports from multiple trigger condition has to be sent
  // mutually exclusively as separate reports.
  if (!pthread_mutex_trylock(&tempProfile->triggerCondMutex)) {
    T2Debug(
        "%s : Lock acquisition succeeded for tempProfile->triggerCondMutex\n ",
        __FUNCTION__);
    ...
    cJSON_AddItemToObject(temparrayItem1, "TriggerConditionResult",
                          temparrayItem);
    tempProfile->jsonReportObj = temparrayItem1;
    // pthread_mutex_unlock(&tempProfile->triggerCondMutex);

  } else {
    T2Warning("%s : Failed to get a lock on tempProfile->triggerCondMutex for "
              "condition %s \n ",
              __FUNCTION__, referenceName);
    status = T2ERROR_FAILURE;
    // Push this trigger condition to a que.
    // Implement a callback function to trigger report generation for
    // referenceName & value
    ...
    if (!pthread_mutex_trylock(&triggerConditionQueMutex)) {
      ...
      if (NULL == triggerConditionQueue) {
        ...
        if (!triggerConditionQueue) {
          T2Error("Failed to create triggerConditionQueue, not proceeding "
                  "further\n");
      ...
      triggerConditionObj *triggerCond =
          (triggerConditionObj *)malloc(sizeof(triggerConditionObj));
      if (triggerCond) {
        if (NULL != referenceName && NULL != referenceValue) {
          T2Info(
              "%s : Push referenceName = %s  & referenceValue = %s to que \n ",
          ...
          T2Warning("%s : referenceName or referenceValue is published as "
                    "null, ignoring trigger condition \n ",
                    __FUNCTION__);
        }
      ...
      /* ref RDKB-55438 Commenting following code as this is causing crash, we
      can't free triggerCond here as this has been added to the queue, this is
      freed in the reportGenerationCompleteReceiver function after queue pop
      ...
      */
      ...
      T2Warning("%s : Failed to get a lock on triggerConditionQueMutex for "
                "condition %s \n ",
                __FUNCTION__, referenceName);
    }
  }
  ...
  if (!pthread_mutex_lock(&triggerConditionQueMutex)) {
    if (NULL != triggerConditionQueue) {
      ...
      if (t2_queue_count(triggerConditionQueue) > 0) {
        triggerConditionObj *triggerCond =
            (triggerConditionObj *)t2_queue_pop(triggerConditionQueue);
        if (triggerCond) {
          // Que has to be unlocked before generating a trigger condition
          pthread_mutex_unlock(&triggerConditionQueMutex);
          triggerReportOnCondtion(triggerCond->referenceName,
                                  triggerCond->referenceValue);
          free(triggerCond);
          triggerCond = NULL;
        } else {
...
T2ERROR triggerReportOnCondtion(const char *referenceName,
  ...
  for (; profileIndex < Vector_Size(profileList); profileIndex++) {
    tempProfile = (Profile *)Vector_At(profileList, profileIndex);
    if (tempProfile->triggerConditionList &&
        (tempProfile->triggerConditionList->count > 0)) {
      for (j = 0; j < tempProfile->triggerConditionList->count; j++) {
        TriggerCondition *triggerCondition = ((TriggerCondition *)Vector_At(
            tempProfile->triggerConditionList, j));
        if (strcmp(triggerCondition->reference, referenceName) == 0) {
          if (triggerCondition->report) {
            if (T2ERROR_SUCCESS == appendTriggerCondition(tempProfile,
                                                          referenceName,
                                                          referenceValue)) {
              tempProfile->triggerReportOnCondition = true;
              tempProfile->minThresholdDuration =
                  triggerCondition->minThresholdDuration;
              T2Debug("%s : Assign callback function as "
                      "reportGenerationCompleteReceiver \n",
                      __FUNCTION__);
              tempProfile->callBackOnReportGenerationComplete =
                  reportGenerationCompleteReceiver;

              char *tempProfilename = strdup(tempProfile->name); // RDKB-42640

              // plmutex should be unlocked before sending interrupt for report
              // generation
              T2Debug("%s : Release lock on &plMutex\n ", __FUNCTION__);
              pthread_mutex_unlock(&plMutex);
              T2Info("Triggering report on condition for %s with %s operator, "
                     "%d threshold\n",
                     triggerCondition->reference, triggerCondition->oprator,
                     triggerCondition->threshold);
              if (tempProfile->isSchedulerstarted) {
                SendInterruptToTimeoutThread(tempProfilename);
              } else {
                T2Info("For Profile %s scheduler is not enabled yet so "
                       "triggering the condition is ignored now\n",
                       tempProfilename);
              ...
              T2Info("Report generation will take place by popping up from the "
                     "que by callback function \n");
              T2Debug("%s : Release lock on &plMutex\n ", __FUNCTION__);
              pthread_mutex_unlock(&plMutex);
              return T2ERROR_SUCCESS;
            }
