...
#define WEBCONFIG_BLOB_VERSION                                                 \
  "/usr/opensync/data/telemetry_webconfig_blob_version.txt"
#else
#define WEBCONFIG_BLOB_VERSION "/opt/telemetry_webconfig_blob_version.txt"
#endif

// Used in check_component_crash to inform Webconfig about telemetry component
// crash
#define TELEMETRY_INIT_FILE_BOOTUP "/tmp/telemetry_initialized_bootup"

#define MAX_PROFILENAMES_LENGTH 2048
#define T2_VERSION_DATAMODEL_PARAM                                             \
...
static void drop_root() {
  ...
  T2Info("NonRoot feature is enabled, dropping root privileges for Telemetry "
         "2.0 Process\n");
  init_capability();
  drop_root_caps(&appcaps);
  if (update_process_caps(&appcaps) !=
      -1) // CID 281096: Unchecked return value (CHECKED_RETURN)
...
uint32_t getTelemetryBlobVersion(char *subdoc) {
  ...
  if (file == NULL) {
    T2Debug(
        "Failed to read from /nvram/telemetry_webconfig_blob_version.txt \n");
  } else {
    /* CID 157387: Unchecked return value from library */
    if ((ret = fscanf(file, "%u", &version)) != 1) {
      T2Debug("Failed to read version from "
              "/nvram/telemetry_webconfig_blob_version.txt \n");
    }
...
int tele_web_config_init() {
  ...
  for (i = 0; i < SUBDOC_COUNT; i++) {
    strncpy(blobDataPointer->subdoc_name, sub_docs[i],
            sizeof(blobDataPointer->subdoc_name) - 1);
    blobDataPointer++;
  }
  blobDataPointer = blobData;
...
void ReportProfiles_Interrupt() {
  ...
  // Interrupt the multi profile first as the DCADONE Flag is added from the
  // xconf
...
T2ERROR ReportProfiles_addReportProfile(Profile *profile) {
  ...
  T2ER_StartDispatchThread(); // Error case can be ignored as Dispatch thread
...
static void *reportOnDemand(void *input) {
  ...
  } else if (!strncmp(action, ON_DEMAND_ACTION_ABORT,
                      MAX_PROFILENAMES_LENGTH)) {
...
T2ERROR initReportProfiles() {
  ...
  initScheduler((TimeoutNotificationCB)ReportProfiles_TimeoutCb,
                (ActivationTimeoutCB)ReportProfiles_ActivationTimeoutCb,
                (NotifySchedulerstartCB)NotifySchedulerstart);
  ...
  t2Version = strdup("2.0.1"); // Setting the version to 2.0.1
  {
    ...
    if (T2ERROR_SUCCESS == datamodel_init()) {
...
#endif
      {
        ...
        interfaceListForBus =
            (callBackHandlers *)malloc(sizeof(callBackHandlers));
        if (interfaceListForBus) {
          interfaceListForBus->dmCallBack = datamodel_processProfile;
          interfaceListForBus->dmMsgPckCallBackHandler =
              datamodel_MsgpackProcessProfile;
          interfaceListForBus->dmSavedJsonCallBack =
              datamodel_getSavedJsonProfilesasString;
          interfaceListForBus->dmSavedMsgPackCallBack =
          ...
          interfaceListForBus->privacyModesDoNotShare =
              privacymode_do_not_share;
          interfaceListForBus->mprofilesdeleteDoNotShare =
...
#endif
// Message pack format is supported only for reportprofile which is activated
// only if version is set to 2.0.1 Call webconfig init only when it is required
// and datamodel's have been initialized .
#if defined(FEATURE_SUPPORT_WEBCONFIG)
...
void generateDcaReport(bool isDelayed, bool isOnDemand) {
  ...
  if (ProfileXConf_isSet()) {
    /**
     * Field requirement - Generate the first report at early stage after around
     * 2 mins of stabilization during boot This is to make it at par with legacy
     * dca reporting pattern
     */
    if (isDelayed) {
...
T2ERROR RemovePreRPfromDisk(const char *path, hash_map_t *map) {
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    if (NULL == hash_map_get(map, entry->d_name)) {
      T2Debug("%s : Removed %s report profile from the disk due to coming new "
              "report profile \n",
...
void ReportProfiles_ProcessReportProfilesBlob(cJSON *profiles_root,
  ...
  if (profiles_root == NULL) {
    T2Error("Profile profiles_root is null . Unable to "
  ...
  if (strcmp(paramValue, "DO_NOT_SHARE") == 0) {
    T2Warning(
        "Privacy Mode is DO_NOT_SHARE. Reportprofiles is not supported\n");
    free(paramValue);
    paramValue = NULL;
    return;
  }
  ...
  if (profiles_count == 0) {
    if (rprofiletypes == T2_TEMP_RP) {
      T2Info("Empty report profiles are not valid configuration for temporary "
             "report profiles. \n");
    } else {
      T2Debug("Empty report profiles in configuration. Delete all active "
              "profiles. \n");
  ...
  for (profileIndex = 0; profileIndex < profiles_count; profileIndex++) {
    ...
    if (singleProfile == NULL) {
      T2Error("Incomplete profile information, unable to create profile for "
              "index %u \n",
              profileIndex);
      continue;
    }
    ...
    if (nameObj == NULL || hashObj == NULL || profileObj == NULL ||
        strcmp(nameObj->valuestring, "") == 0 ||
        strcmp(hashObj->valuestring, "") == 0) {
      T2Error(
          "Incomplete profile object information, unable to create profile\n");
      continue;
    }

    ReportProfile *profileEntry =
        (ReportProfile *)malloc(sizeof(ReportProfile));
  ...
  if (rprofiletypes == T2_RP) {
    ...
    while (count >= 0) {
      ...
      if (NULL == hash_map_get(receivedProfileHashMap, profileNameKey)) {
        T2Debug("%s Profile %s not present in current config . Remove profile "
                "from disk \n",
                __FUNCTION__, profileNameKey);
    ...
    if (T2ERROR_SUCCESS !=
        RemovePreRPfromDisk(DirPath, receivedProfileHashMap)) {
  ...
  for (profileIndex = 0; profileIndex < hash_map_count(receivedProfileHashMap);
       profileIndex++) {
    ReportProfile *profileEntry =
        (ReportProfile *)hash_map_lookup(receivedProfileHashMap, profileIndex);
    ...
    if (existingProfileHash != NULL) {
      ...
      if (!strcmp(existingProfileHash, profileEntry->hash)) {
        T2Debug("%s Profile hash for %s is same as previous profile, ignore "
                "processing config\n",
        ...
        if (T2ERROR_SUCCESS ==
            processConfiguration(&(profileEntry->config), profileName,
                                 profileEntry->hash,
                                 &profile)) { // CHECK if process configuration
                                              // should have locking mechanism
          if (profile->reportOnUpdate) {
            T2Info("%s Profile %s present in current config and hash value is "
                   "different. Generating  cjson report for the profile. \n",
                   __FUNCTION__, profileName);
            NotifyTimeout(profileName, true);
          }
          if (T2ERROR_SUCCESS !=
              saveConfigToFile(DirPath, profile->name, profileEntry->config)) {
      ...
      T2Debug(
          "%s Previous entry for profile %s not found . Adding new profile.\n",
          __FUNCTION__, profileName);
      Profile *profile = 0;
...
pErr Process_Telemetry_WebConfigRequest(void *Data) {
  ...
  // no one should be able to cancle a thread executing telemetry code as it
  // leaves it in inconsistent state
  // int oldtype;
  // pthread_setcanceltype(PTHREAD_CANCEL_DISABLE, &oldtype);
  // not using this as of now as the thread calling this can call free resources
  // in such a case it will lead to crash
  int retval = __ReportProfiles_ProcessReportProfilesMsgPackBlob(Data);
  // pthread_setcanceltype(oldtype,NULL);// do not restore now as we may get
  // memleak from data
...
size_t calculateTimeout(size_t numOfEntries) {
  ...
  size_t timeOut =
      (noOfProfilesInDevice + 1) * MAXTIMEOUT_PERPROFILE *
      (numOfEntries + 1); // adding +1 to protect from any value being 0
  T2Info("Timeout value for Webconfig is calculated with profile count : %d "
         "and maxtimeout per profile : %d is %zu \n",
...
void ReportProfiles_ProcessReportProfilesMsgPackBlob(char *msgpack_blob,
  ...
  if (NULL == subdoc_name || NULL == transaction_id || NULL == version) {
    ...
    msgpack_unpacked_destroy(&result);
    T2Debug("%s --out\n", __FUNCTION__);
    /* Return - further processing are only for webconfig framework which yet to
     * be ported to generic layer from broadband */
    return;
  }
  ...
  strncpy(execDataPf->subdoc_name, "telemetry",
          sizeof(execDataPf->subdoc_name) - 1);
  ...
  T2Debug("subdocversion is %d transac_id in integer is %d entry_count is %lu "
          "subdoc_name is %s"
          " calcTimeout is %p\n",
          execDataPf->version, execDataPf->txid,
          (ulong)execDataPf->numOfEntries, execDataPf->subdoc_name,
          execDataPf->calcTimeout);
...
int __ReportProfiles_ProcessReportProfilesMsgPackBlob(void *msgpack) {
  ...
  for (profileIndex = 0; profileIndex < profiles_count; profileIndex++) {
    ...
    if (nameObj == NULL || hashObj == NULL || profileObj == NULL ||
        msgpack_strcmp(nameObj, "") == 0 || msgpack_strcmp(hashObj, "") == 0) {
      T2Error(
          "Incomplete profile object information, unable to create profile\n");
      continue;
    }
    ...
    if (NULL == existingProfileHash) {
      if (T2ERROR_SUCCESS ==
          processMsgPackConfiguration(singleProfile, &profile)) {
        ReportProfiles_addReportProfile(profile);
  ...
  if (save_flag) {
    ...
    MsgPackSaveConfig(REPORTPROFILES_PERSISTENCE_PATH,
                      MSGPACK_REPORTPROFILES_PERSISTENT_FILE, msgpack_blob,
                      msgpack_blob_size);
...
bool isMtlsEnabled(void) {
  ...
  if (isT2MtlsEnable != true) {
    if (T2ERROR_SUCCESS ==
        getParameterValue(TR181_DEVICE_PARTNER_ID, &paramValue)) {
