...
public:
  ...
  virtual rbusError_t rbusMethod_Invoke(rbusHandle_t, char const *,
                                        rbusObject_t, rbusObject_t *) = 0;
  ...
  /* virtual bool isRbusEnabled() = 0;
   ...
   virtual int regDEforCompEventList(const char*, T2EventMarkerListCallback) =
   0; virtual int publishEventsProfileUpdates() = 0; virtual void
   unregisterDEforCompEventList() = 0; virtual int
   regDEforProfileDataModel(callBackHandlers*) = 0; virtual int
   registerForTelemetryEvents(TelemetryEventCallback) = 0; virtual int
   T2RbusReportEventConsumer(char*, bool) = 0; virtual int
   rbusT2ConsumerUnReg(Vector *) = 0; virtual int rbusT2ConsumerReg(Vector *) =
   0; virtual void
   registerConditionalReportCallBack(triggerReportOnCondtionCallBack) = 0;
...
public:
  ...
  MOCK_METHOD3(rbusObject_SetValue,
               void(rbusObject_t, char const *, rbusValue_t));
  ...
  MOCK_METHOD4(rbusMethod_Invoke, rbusError_t(rbusHandle_t, char const *,
                                              rbusObject_t, rbusObject_t *));
  ...
  /* MOCK_METHOD0(isRbusEnabled, bool());
   ...
   MOCK_METHOD2(regDEforCompEventList, int(const char*,
   T2EventMarkerListCallback)); MOCK_METHOD0(publishEventsProfileUpdates,
   int()); MOCK_METHOD0(unregisterDEforCompEventList, void());
   ...
   MOCK_METHOD1(registerConditionalReportCallBack,
   void(triggerReportOnCondtionCallBack));
   MOCK_METHOD3(sendReportsOverRBUSMethod, int(char *, Vector*, char*));
   MOCK_METHOD3(sendCachedReportsOverRBUSMethod, int(char *, Vector*,
   Vector*));*/
};
