...
typedef rtError (*rtRouteMessageHandler)(rtConnectedClient *sender,
                                         rtMessageHeader *hdr,
                                         uint8_t const *buff, int n,
...
typedef rtError (*rtDriectClientHandler)(uint8_t isClientRequest,
                                         rtMessageHeader *hdr,
                                         uint8_t const *pInbuff, int inLength,
                                         uint8_t **pOutBuff,
                                         uint32_t *pOutLength);

rtError rtRouteBase_BindListener(char const *socket_name, int no_delay,
                                 int indefinite_retry, rtListener **pListener);
rtError rtRouteBase_CloseListener(rtListener *pListener);

rtError rtRouteDirect_StartInstance(const char *socket_name,
                                    rtDriectClientHandler messageHandler);
rtError rtRouteDirect_SendMessage(const rtPrivateClientInfo *pClient,
                                  uint8_t const *pInBuff, int inLength);
