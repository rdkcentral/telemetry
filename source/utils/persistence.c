...
T2ERROR fetchLocalConfigs(const char *path, Vector *configList) {
  if (path == NULL ||
      ((strcmp(path, SHORTLIVED_PROFILES_PATH) != 0) && configList == NULL)) {
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    if (entry->d_name[0] == '.' ||
        (strcmp(entry->d_name, MSGPACK_REPORTPROFILES_PERSISTENT_FILE) == 0))
      continue;
    ...
    if (status == 0) {
      ...
      config->configData =
          (char *)malloc((filestat.st_size + 1) * sizeof(char));
...
T2ERROR saveCachedReportToPersistenceFolder(const char *profileName,
  ...
  if (dir == NULL) {
    T2Info("Persistence folder %s not present, creating folder\n",
           CACHED_MESSAGE_PATH);
    if (mkdir(CACHED_MESSAGE_PATH, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH) !=
        0) {
      T2Error("%s,%d: Failed to make directory : %s  \n", __FUNCTION__,
              __LINE__, CACHED_MESSAGE_PATH);
  ...
  snprintf(absFilePath, (MAX_FILENAME_LENGTH - 1), "%s/%s", CACHED_MESSAGE_PATH,
           profileName);
...
T2ERROR savePrivacyModeToPersistentFolder(char *data) {
  ...
  snprintf(filePath, sizeof(filePath), "%s/%s", PRIVACYMODE_PATH,
           "privacymodes.txt");
  fp = fopen(filePath, "w+");
  if (fp == NULL) {
    T2Error("Unable to write to file : %s\n", filePath);
