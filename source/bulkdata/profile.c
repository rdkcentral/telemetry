...
static pthread_mutex_t profileListLock; // L0: protects profileList (rdlock for reads, wrlock for writes)
...
T2ERROR profileWithNameExists(const char *profileName, bool *bProfileExists)
{
    ...
    pthread_mutex_lock(&profileListLock);
    for(; profileIndex < Vector_Size(profileList); profileIndex++)
    {
        tempProfile = (Profile *)Vector_At(profileList, profileIndex);
        if(strcmp(tempProfile->name, profileName) == 0)
        {
            *bProfileExists = true;
            pthread_mutex_unlock(&profileListLock);
            return T2ERROR_SUCCESS;
        }
    }
    *bProfileExists = false;
...
T2ERROR deleteProfile(const char *profileName)
{
    ...
    /* Release profileListLock before pthread_join to avoid deadlock.
     * pthread_join can block indefinitely if the CollectAndReport thread
     * is stuck (e.g., waiting on rbusMethodMutex). Holding profileListLock during
    ...
    /* Re-acquire profileListLock (wrlock) for profile cleanup operations */
    pthread_mutex_lock(&profileListLock);
...
T2ERROR initProfileList(bool checkPreviousSeek)
{
    ...
    initialized = true;
    if(pthread_mutex_init(&profileListLock, NULL) != 0)
    {
    ...
    {
        T2Error("%s mutex init has failed\n", __FUNCTION__);
        pthread_mutex_destroy(&profileListLock);
        initialized = false;
        return T2ERROR_FAILURE;
    }
...
void NotifySchedulerstart(char* profileName, bool isschedulerstarted)
{
    ...
    Profile *tempProfile = NULL;
    pthread_mutex_lock(&profileListLock);
    T2Debug("profileListLock is rdlocked  %s\n", __FUNCTION__);
    for(; profileIndex < Vector_Size(profileList); profileIndex++)
    {
        tempProfile = (Profile *)Vector_At(profileList, profileIndex);
        if(tempProfile != NULL)
    ...
    pthread_mutex_unlock(&profileListLock);
    T2Debug("profileListLock is unlocked  %s\n", __FUNCTION__);
    return;
}
