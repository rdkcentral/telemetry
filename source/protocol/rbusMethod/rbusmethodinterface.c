...
T2ERROR sendReportsOverRBUSMethod(char *methodName, Vector *inputParams,
  ...
  for (i = 0; i < Vector_Size(inputParams); i++) {
    ...
    RBUSMethodParam *rbusMethodParam =
        (RBUSMethodParam *)Vector_At(inputParams, i);
    if (rbusMethodParam) {
  ...
  isRbusMethod = false;
  if (T2ERROR_SUCCESS ==
      rbusMethodCaller(methodName, &inParams, payload, &asyncMethodHandler)) {
    int retry_count = 0;
    while (retry_count < MAX_RETRY_ATTEMPTS) {
      if (!(pthread_mutex_trylock(
              &rbusMethodMutex))) { // locking the rbusMethodMutex, in
                                    // asyncMethodHandler mutex unlock is
                                    // present
        if (isRbusMethod) {
        ...
        retry_count++;
        sleep(2); // giving 2 seconds sleep for 5 times which will be equal to
                  // waiting for RBUS_METHOD_TIMEOUT value // Removing the
                  // pthread_mutex_unlock before sleep as rbusmethodmutex will
                  // be unlocked by asyncMethodHandler.
        if (retry_count == 5) {
  ...
  /*rbusMethodMutex is locked before rbusMethodCaller if rbusMethodCaller
    function returns failure rbusMethodMutex will be unlocked here and in second
    case if rbusMethodCaller returns success then in asyncMethodHandler already
    mutex unlock is done which unlocks the rbusMethodMutex lock done above so
    adding another lock inside rbusMethodCaller if loop for rbusMethodMutex
    which later gets unlocked by the below mutex unlock.*/
  pthread_mutex_unlock(&rbusMethodMutex);
