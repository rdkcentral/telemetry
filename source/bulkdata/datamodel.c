    ...
    if (t2_queue_count(rpMsgPkgQueue) > 0) {
      ...
      if (msgpack) {
        ReportProfiles_ProcessReportProfilesMsgPackBlob(
            msgpack->msgpack_blob, msgpack->msgpack_blob_size);
        free(msgpack);
      }
    }
...
/* Description:
 *      The API validate JSON format and check if 'profiles' field is present in
 * JSON data. It saves the received json data in temporary file and notify
 * process_rp_thread() for incoming data. Arguments: char *JsonBlob         List
 * of active profiles.
 */
...
int datamodel_getSavedMsgpackProfilesasString(char **SavedProfiles) {
  T2Debug("%s ++in\n", __FUNCTION__);
  //#if defined(FEATURE_SUPPORT_WEBCONFIG) get should work without the
  // restriction of webconfig
