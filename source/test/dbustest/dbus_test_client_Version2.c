 ...
 * Fixed version with separate D-Bus connections for method calls and signal listening
 * 
 * Compilation command:
 * gcc -o dbus_test_client dbus_test_client.c -ldbus-1 -I/usr/include/dbus-1.0 -I/usr/lib/x86_64-linux-gnu/dbus-1.0/include -lpthread
 * 
 * Alternative:
 * gcc -o dbus_test_client dbus_test_client.c $(pkg-config --cflags --libs dbus-1) -lpthread
 * 
 * Usage:
 * ./dbus_test_client
 * 
...
static T2ERROR dbus_getGetOperationalStatus(const char* paramName, uint32_t* value)
{
    ...
    /* Use method call connection (bus_handle) with reduced timeout */
    reply = dbus_connection_send_with_reply_and_block((DBusConnection*)bus_handle, msg, 200, &error);
    dbus_message_unref(msg);
...
static T2ERROR initMessageBus(void)
{
    ...
    if(0 == dbus_checkStatus())
    {
        ...
        else
        {
            ...
            snprintf(signalDbusName, 124, "telemetry.t2.lib_default_signals");
        }
        ...
        if (dbus_error_is_set(&error))
        {
            EVENT_ERROR("D-Bus method call connection init failed: %s\n", error.message);
            dbus_error_free(&error);
            return T2ERROR_FAILURE;
        }
        ...
        if (dbus_error_is_set(&error))
        {
            EVENT_ERROR("D-Bus method call name request failed: %s\n", error.message);
            dbus_error_free(&error);
            dbus_connection_unref((DBusConnection*)bus_handle);
            bus_handle = NULL;
            return T2ERROR_FAILURE;
        ...
        {
            EVENT_ERROR("D-Bus method call name request returned: %d\n", ret);
            dbus_connection_unref((DBusConnection*)bus_handle);
            bus_handle = NULL;
            return T2ERROR_FAILURE;
        }
        ...
        {
            EVENT_DEBUG("D-Bus method call connection initialized as: %s\n", dbusName);
        }
        ...
        if (dbus_error_is_set(&error))
        {
            EVENT_ERROR("D-Bus signal connection init failed: %s\n", error.message);
            dbus_error_free(&error);
        ...
        if (dbus_error_is_set(&error))
        {
            EVENT_ERROR("D-Bus signal name request failed: %s\n", error.message);
            dbus_error_free(&error);
            dbus_connection_unref((DBusConnection*)signal_bus_handle);
            signal_bus_handle = NULL;
        }
        ...
        if (ret != DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER && ret != DBUS_REQUEST_NAME_REPLY_ALREADY_OWNER)
        {
            EVENT_ERROR("D-Bus signal name request returned: %d\n", ret);
            dbus_connection_unref((DBusConnection*)signal_bus_handle);
            signal_bus_handle = NULL;
        }
        else
        {
            EVENT_DEBUG("D-Bus signal connection initialized as: %s\n", signalDbusName);
            isDbusEnabled = true;
        }
        ...
        if (signal_bus_handle && bus_handle)
        {
            ...
            EVENT_DEBUG("D-Bus initialized successfully with separate connections\n");
        }
    }
    else
    {
...
static T2ERROR doPopulateEventMarkerList(void)
{
    ...
    /* Get marker list via D-Bus method call using method call connection */
    DBusMessage *msg = NULL;
    DBusMessage *reply = NULL;
    DBusError error;
    dbus_error_init(&error);
    ...
    if (!reply)
    {
        EVENT_ERROR("D-Bus mode: GetMarkerList return with No reply received\n");
        return T2ERROR_SUCCESS;
    }
...
{
    /* Optimization: If T2 is already ready and subscribed, skip D-Bus status check */
    // if(isT2Ready && isEventSubscribed)
    // {
    //     EVENT_DEBUG("T2 already ready and subscribed - skipping status check\n");
    //     return false;  /* No caching required */
    // }
    ...
    /* Setup ProfileUpdate subscription if not already done */
    if(!isEventSubscribed && signal_bus_handle)
    {
...
static int filtered_event_send(const char* data, const char *markerName)
{
    ...
    if(isDbusEnabled)
    {
        ...
        /* D-Bus method call to send event using method call connection */
        DBusMessage *msg = NULL;
        DBusError error;
        dbus_error_init(&error);
        ...
        else
        {
            ...
            {
                /* Send without blocking - fire and forget */
                ...
                {
                    /* Don't flush - let connection handle it asynchronously */
...
void t2_init(char *component)
{
    ...
    /* Check if T2 is ready - this will also setup signal subscription */
    isCachingRequired();
...
int main(int argc, char *argv[])
{
    ...
    printf("║   Component: %-27s ║\n", COMP_NAME);
    printf("║   Using Separate D-Bus Connections       ║\n");
    printf("╚═══════════════════════════════════════════╝\n\n");
    ...
    printf("\nWaiting for ProfileUpdate signals...\n");
