...
static char *getProfileParameter(Profile *profile, const char *ref) {
  ...
  if (!strcmp(pName, "Name")) {
    ...
    if (profile->reportingInterval)
      snprintf(pValue, MAX_STATIC_PROP_VAL_LEN, "%d",
...
static T2ERROR addhttpURIreqParameter(Profile *profile, const char *Hname,
                                      const char *Href) {
  T2Debug("%s ++in\n", __FUNCTION__);
...
static T2ERROR addRbusMethodParameter(Profile *profile, const char *name,
                                      const char *value) {
  T2Debug("%s ++in\n", __FUNCTION__);
  if (NULL == name || NULL == value)
    return T2ERROR_SUCCESS;

  RBUSMethodParam *rbusMethodParam =
      (RBUSMethodParam *)malloc(sizeof(RBUSMethodParam));
  char *val = NULL;
...
static T2ERROR addParameter(Profile *profile, const char *name, const char *ref,
                            const char *fileName, int skipFreq,
                            int firstSeekFromEOF, const char *ptype,
                            const char *use, bool ReportEmpty,
                            reportTimestampFormat reportTimestamp, bool trim,
                            const char *regex) {
  ...
  if (!(strcmp(ptype, "dataModel"))) {
    ...
    if (strstr(ref, "Profile.") ==
        ref) { // Static parameters from profile configuration
    ...
    // T2Debug("Adding Event Marker :: Param/Marker Name : %s ref/pattern/Comp :
    // %s skipFreq : %d\n", name, ref, skipFreq);
    ...
    // T2Debug("Adding Grep Marker :: Param/Marker Name : %s ref/pattern/Comp :
    // %s fileName : %s skipFreq : %d\n", name, ref, fileName, skipFreq);
...
T2ERROR addMsgPckTriggerCondition(Profile *profile, msgpack_object *value_map) {
  ...
  for (i = 0; i < TcArraySize; i++) {
    ...
    TriggerCondition *triggerCondition =
        (TriggerCondition *)malloc(sizeof(TriggerCondition));
    ...
    tcMinThresholdDurationInt =
        msgpack_get_map_value(tcArrayMap, "minThresholdDuration");
    if (tcMinThresholdDurationInt) {
      msgpack_print(tcMinThresholdDurationInt,
                    msgpack_get_obj_name(tcMinThresholdDurationInt));
...
T2ERROR addTriggerCondition(Profile *profile, cJSON *jprofileTriggerCondition) {
  ...
  for (triggerConditionIndex = 0;
       triggerConditionIndex < triggerCondition_count;
       triggerConditionIndex++) {
    char *tcType = NULL;
    char *tcOperator = NULL;
    int tcThreshold = 0;
    int tcMinThresholdDuration = 0;
    ...
    cJSON *tcSubitem =
        cJSON_GetArrayItem(jprofileTriggerCondition, triggerConditionIndex);
    ...
    if (tcSubitem != NULL) {
      ...
      cJSON *jTcSubItemMinThresholdDuration =
          cJSON_GetObjectItem(tcSubitem, "minThresholdDuration");
...
T2ERROR verifyTriggerCondition(cJSON *jprofileTriggerCondition) {
  ...
  for (triggerConditionIndex = 0;
    ...
    if (tcSubitem != NULL) {
      ...
      if ((strncmp(operator, "any", strlen("any") + 1) != 0)) {
        cJSON *jTcSubItemThreshold =
            cJSON_GetObjectItem(tcSubitem, "threshold");
...
void time_param_Reporting_Adjustments_valid_set(
    Profile *profile, cJSON *jprofileReportingInterval,
    cJSON *jprofileActivationTimeout, cJSON *jprofileTimeReference,
    cJSON *jprofileReportOnUpdate, cJSON *jprofilefirstReportingInterval,
    cJSON *jprofilemaxUploadLatency, cJSON *jprofileReportingAdjustments) {
  if (profile
          ->generateNow) { // If GenerateNow is true and ReportingAdjustments is
                           // present, ReportingAdjustments shall be ignored
    ...
    if (jprofileReportingAdjustments) {
      ...
      if ((profile->timeRef == NULL) ||
          strcmp(profile->timeRef, DEFAULT_TIME_REFERENCE) ==
              0) { // Property 'FirstReportingInterval' if present along with
                   // Timereference,  FirstReportingInterval shall be ignored
        if (jprofilefirstReportingInterval) {
          int firstReportIntervalInSec =
              jprofilefirstReportingInterval->valueint;
      ...
      if (profile->timeRef != NULL &&
          strcmp(profile->timeRef, DEFAULT_TIME_REFERENCE) !=
              0) { // This value is only valid when TimeReference is different
                   // from the default.
        if (jprofilemaxUploadLatency) { // randomize the upload time of a
                                        // generated report.
...
T2ERROR addParameter_marker_config(Profile *profile, cJSON *jprofileParameter,
                                   int ThisProfileParameter_count) {
  ...
  for (ProfileParameterIndex = 0;
       ProfileParameterIndex < ThisProfileParameter_count;
       ProfileParameterIndex++) {
    header = NULL;
    content = NULL;
    logfile = NULL;
    skipFrequency = 0;
    ...
    cJSON *pSubitem =
        cJSON_GetArrayItem(jprofileParameter, ProfileParameterIndex);
    if (pSubitem != NULL) {
      cJSON *jpSubitemtype = cJSON_GetObjectItem(pSubitem, "type");
      if (jpSubitemtype) {
        paramtype = jpSubitemtype->valuestring;
      } else { /*TODO:Confirm if type is not configured. For now avoid
      ...
      cJSON *jpSubitemreportEmpty =
          cJSON_GetObjectItem(pSubitem, "reportEmpty");
      if (jpSubitemreportEmpty) {
        reportEmpty = cJSON_IsTrue(jpSubitemreportEmpty);
      }
      cJSON *jpSubitemtrim = cJSON_GetObjectItem(
          pSubitem,
          "trim"); // RDKB-53161 adding trim parameter in multiprofiles
      if (jpSubitemtrim) {
      ...
      if (!(strcmp(paramtype, "dataModel"))) { /*Marker is TR181 param*/
        ...
        if (jpSubitemreference == NULL) {
          T2Error("Reference property is mandatory for datamodel parameter. "
                  "Ignore the adding of this parameter to paramlist\n");
          continue;
        }
        content = jpSubitemreference->valuestring;
        if (!jpSubitemname) {
          header =
              jpSubitemreference->valuestring; /*Default Name can be reference*/
        }
        cJSON *jpMethod = cJSON_GetObjectItem(pSubitem, "method");
        cJSON *jpSubitemreportTimestamp =
            cJSON_GetObjectItem(pSubitem, "reportTimestamp");
        if (jpMethod) {
          method = jpMethod->valuestring;
          T2Debug("Method property is present and the value is %s\n", method);
          if (!(strcmp(method, "subscribe"))) {
            T2Info(
                "Method is subscribe converting the parmeter to event type\n");
            paramtype = "event";
            ...
            if (jpSubitemreportTimestamp) {
              ...
              if (!(strcmp(jpSubitemreportTimestamp->valuestring,
                           "Unix-Epoch"))) {
        ...
        cJSON *jpSubitemname = cJSON_GetObjectItem(
            pSubitem, "name"); // Optional repalcement name in report
        ...
        if (jpSubitemeventName) {
          header =
              jpSubitemeventName->valuestring; /*Default Name can be eventName*/
        }
        ...
        cJSON *jpSubitemFirstSeekFromEOF =
            cJSON_GetObjectItem(pSubitem, "firstSeekFromEOF");
        if (jpSubitemname) {
          header = jpSubitemname->valuestring;
        }
        ...
        if (jpSubitemFirstSeekFromEOF) {
          if (cJSON_IsNumber(jpSubitemFirstSeekFromEOF)) {
            ...
            T2Error(
                "%s First SeekFromEOF is not a number ignoring the value \n",
                __FUNCTION__);
          }
        }
      } else {
      ...
      // CID 337454: Explicit null dereferenced (FORWARD_NULL) ;CID 337448:
      // Explicit null dereferenced (FORWARD_NULL)
      if (content != NULL && header != NULL) {
        ret = addParameter(profile, header, content, logfile, skipFrequency,
                           firstSeekFromEOF, paramtype, use, reportEmpty,
                           rtformat, trim,
                           regex); // add Multiple Report Profile Parameter
      } else {
        T2Error("%s Error in adding parameter to profile %s \n", __FUNCTION__,
                profile->name); // header and content is mandatory for adding
                                // the parameter
        continue;
      }
...
T2ERROR encodingSet(Profile *profile, cJSON *jprofileEncodingType,
                    cJSON *jprofileJSONReportFormat,
  ...
  if ((profile->jsonEncoding) &&
    ...
    if (jprofileJSONReportTimestamp) {
      ...
      } else if (!(strcmp(jprofileJSONReportTimestamp->valuestring,
                          "Unix-Epoch"))) {
...
T2ERROR protocolSet(Profile *profile, cJSON *jprofileProtocol,
                    ...
                    cJSON *jprofileHTTPRequestURIParameter,
                    int ThisprofileHTTPRequestURIParameter_count,
                    cJSON *jprofileRBUSMethodName,
                    cJSON *jprofileRBUSMethodParamArr,
                    int rbusMethodParamArrCount) {
  if (profile == NULL) {
  ...
  if ((profile->t2HTTPDest) &&
    ...
    if (jprofileHTTPRequestURIParameter) {
      ...
      for (profileHTTPRequestURIParameterIndex = 0;
           profileHTTPRequestURIParameterIndex <
           ThisprofileHTTPRequestURIParameter_count;
           profileHTTPRequestURIParameterIndex++) {
        href = NULL;
        hname = NULL;
        cJSON *pHTTPRPsubitem =
            cJSON_GetArrayItem(jprofileHTTPRequestURIParameter,
                               profileHTTPRequestURIParameterIndex);
        if (pHTTPRPsubitem != NULL) {
          cJSON *pHTTPRPsubitemReference =
              cJSON_GetObjectItem(pHTTPRPsubitem, "Reference");
          if (pHTTPRPsubitemReference) {
            href = pHTTPRPsubitemReference->valuestring;
            hname =
                pHTTPRPsubitemReference->valuestring; // default value for Name
            if (!(strcmp(
                    href,
                    ""))) { /*if reference is empty avoid adding this object*/
              continue;
            }
          } else {
            continue;
          ...
          cJSON *pHTTPRPsubitemName =
              cJSON_GetObjectItem(pHTTPRPsubitem, "Name");
          ...
          if (httpret != T2ERROR_SUCCESS) {
            T2Error("%s Error in adding request URIparameterReference: %s for "
                    "the profile: %s \n",
  ...
  if ((profile->t2RBUSDest) &&
      (strcmp(jprofileProtocol->valuestring, "RBUS_METHOD") == 0)) {
    profile->t2RBUSDest->rbusMethodName =
        strdup(jprofileRBUSMethodName->valuestring);
    ...
    if (jprofileRBUSMethodParamArr) {
      ...
      for (rbusMethodParamIndex = 0;
           rbusMethodParamIndex < rbusMethodParamArrCount;
           rbusMethodParamIndex++) {
        name = NULL;
        value = NULL;
        cJSON *jRbusParamElement = cJSON_GetArrayItem(
            jprofileRBUSMethodParamArr, rbusMethodParamIndex);
...
T2ERROR processConfiguration(char **configData, char *profileName,
  ...
  cJSON *jprofileReportingInterval =
      cJSON_GetObjectItem(json_root, "ReportingInterval");
  cJSON *jprofileRootName = cJSON_GetObjectItem(json_root, "RootName");
  cJSON *jprofileActivationTimeout =
      cJSON_GetObjectItem(json_root, "ActivationTimeOut");
  cJSON *jprofileTimeReference =
      cJSON_GetObjectItem(json_root, "TimeReference");
  ...
  cJSON *jprofileDeleteOnTimeout =
      cJSON_GetObjectItem(json_root, "DeleteOnTimeout");
  cJSON *jprofileTriggerCondition =
      cJSON_GetObjectItem(json_root, "TriggerCondition");
  cJSON *jprofileReportingAdjustments =
      cJSON_GetObjectItem(json_root, "ReportingAdjustments");
  cJSON *jprofileReportOnUpdate = NULL;
  cJSON *jprofilefirstReportingInterval = NULL;
  cJSON *jprofilemaxUploadLatency = NULL;

  if (jprofileReportingAdjustments) {
    jprofileReportOnUpdate =
        cJSON_GetObjectItem(jprofileReportingAdjustments, "ReportOnUpdate");
    jprofilefirstReportingInterval = cJSON_GetObjectItem(
        jprofileReportingAdjustments, "FirstReportingInterval");
    jprofilemaxUploadLatency =
        cJSON_GetObjectItem(jprofileReportingAdjustments, "MaxUploadLatency");
  }

  if (jprofileParameter) {
    ThisProfileParameter_count = cJSON_GetArraySize(jprofileParameter);
  }
  if (profileName == NULL || jprofileProtocol == NULL ||
      jprofileEncodingType == NULL || jprofileParameter == NULL ||
      (profileHash == NULL && jprofileHash == NULL)) {
    T2Error("Incomplete profile information, unable to create profile\n");
    cJSON_Delete(json_root);
    return T2ERROR_FAILURE;
  }
  if (jprofileReportingInterval && jprofilemaxUploadLatency &&
      ((jprofileReportingInterval->valueint * 1000) <
       jprofilemaxUploadLatency->valueint)) {
    T2Error("MaxUploadLatency is greater than reporting interval. Invalid "
            "Profile\n");
    cJSON_Delete(json_root);
    return T2ERROR_FAILURE;
  }
  if (!jprofileTriggerCondition && !jprofileReportingInterval) {
    T2Error("If TriggerCondition is not given ReportingInterval parameter is "
            "mandatory. Reporting interval is not configured for this "
  ...
  if (!jprofileHash) {
    cJSON_AddStringToObject(
        json_root, "Hash",
        profileHash); // updating the versionHash to persist the hash along with
  ...
  if (jprofileProtocol) {
    if (strcmp(jprofileProtocol->valuestring, "HTTP") == 0) {
      ...
      jprofileHTTPCompression =
          cJSON_GetObjectItem(jprofileHTTP, "Compression");
      jprofileHTTPMethod = cJSON_GetObjectItem(jprofileHTTP, "Method");
      if (jprofileHTTPURL == NULL || jprofileHTTPCompression == NULL ||
          jprofileHTTPMethod == NULL) {
        T2Error("Incomplete profile information, unable to create profile\n");
        cJSON_Delete(json_root);
        return T2ERROR_FAILURE;
      }
      jprofileHTTPRequestURIParameter =
          cJSON_GetObjectItem(jprofileHTTP, "RequestURIParameter");
      if (jprofileHTTPRequestURIParameter) {
        ThisprofileHTTPRequestURIParameter_count =
            cJSON_GetArraySize(jprofileHTTPRequestURIParameter);
      }
    } else if (strcmp(jprofileProtocol->valuestring, "RBUS_METHOD") == 0) {
      jprofileRBUSMethod = cJSON_GetObjectItem(json_root, "RBUS_METHOD");
      jprofileRBUSMethodName =
          cJSON_GetObjectItem(jprofileRBUSMethod, "Method");
      if (jprofileRBUSMethodName == NULL ||
          jprofileRBUSMethodName->valuestring == NULL ||
          strcmp(jprofileRBUSMethodName->valuestring, "") == 0) {
        T2Error("RBUS Method not configured . Ignoring profile %s \n",
                profileName);
        cJSON_Delete(json_root);
        return T2ERROR_FAILURE;
      }
      if (jprofileRBUSMethodName->valuestring &&
          !rbusCheckMethodExists(jprofileRBUSMethodName->valuestring)) {
        T2Warning("WARN: no method provider: %s %s\n", profileName,
                  jprofileRBUSMethodName->valuestring);
      }
      jprofileRBUSMethodParamArr =
          cJSON_GetObjectItem(jprofileRBUSMethod, "Parameters");
      if (jprofileRBUSMethodParamArr) {
        rbusMethodParamArrCount =
            cJSON_GetArraySize(jprofileRBUSMethodParamArr);
      }
    } else {
  ...
  if ((jprofileEncodingType) &&
      (strcmp(jprofileEncodingType->valuestring, "JSON") == 0)) {
    jprofileJSONEncoding = cJSON_GetObjectItem(json_root, "JSONEncoding");
    jprofileJSONReportFormat =
        cJSON_GetObjectItem(jprofileJSONEncoding, "ReportFormat");
    if (jprofileJSONReportFormat == NULL) {
      T2Error("Incomplete profile information, unable to create profile\n");
      cJSON_Delete(json_root);
      return T2ERROR_FAILURE;
    }
    jprofileJSONReportTimestamp =
        cJSON_GetObjectItem(jprofileJSONEncoding, "ReportTimestamp");
  }

  if (jprofileReportingInterval && jprofileActivationTimeout &&
      jprofileActivationTimeout->valueint <
          jprofileReportingInterval->valueint) {

    T2Error("activationTimeoutPeriod is less than reporting interval. invalid "
            "profile: %s \n",
            profileName);
    cJSON_Delete(json_root);
    return T2ERROR_FAILURE;
  ...
  time_param_Reporting_Adjustments_valid_set(
      profile, jprofileReportingInterval, jprofileActivationTimeout,
      jprofileTimeReference, jprofileReportOnUpdate,
      jprofilefirstReportingInterval, jprofilemaxUploadLatency,
      jprofileReportingAdjustments);
  ...
  if (profile->reportingInterval >
      profile->activationTimeoutPeriod) { // When Reporting interval and
                                          // activation time is not given in the
                                          // profile it takes the max value
                                          // INFINITE TIMEOUT or UINT_MAX
    T2Error("activationTimeoutPeriod is less than reporting interval. Max "
            "values assigned is inappropriate .. Invalid profile: %s \n",
            profileName);
    cJSON_Delete(json_root);
    if (profile->timeRef != NULL) {
  ...
  retvalue = encodingSet(profile, jprofileEncodingType,
                         jprofileJSONReportFormat, jprofileJSONReportTimestamp);
  ...
  retvalue = protocolSet(profile, jprofileProtocol, jprofileHTTPURL,
                         jprofileHTTPRequestURIParameter,
                         ThisprofileHTTPRequestURIParameter_count,
                         jprofileRBUSMethodName, jprofileRBUSMethodParamArr,
                         rbusMethodParamArrCount);
  if (retvalue != T2ERROR_SUCCESS) {
  ...
  retvalue = addParameter_marker_config(profile, jprofileParameter,
                                        ThisProfileParameter_count);
  if (retvalue != T2ERROR_SUCCESS) {
  ...
  // Not included for RDKB-25008 . DCA utils expects the list to be sorted based
  // on logfile names
  cJSON_Delete(json_root);
  json_root = NULL;
    ...
    if (obj->via.map.size != 0) {
      ...
      for (; current < end; current++)
        if (current->key.type == MSGPACK_OBJECT_STR) {
          if (0 ==
              strncmp(key, current->key.via.str.ptr, current->key.via.str.size))
            return &current->val;
        }
    }
  return NULL;
...
T2ERROR time_param_MsgPackReporting_Adjustments_valid_set(
    Profile *profile, msgpack_object *ReportingInterval_u64,
    msgpack_object *ActivationTimeout_u64, msgpack_object *TimeReference_str,
    msgpack_object *ReportOnUpdate_boolean,
    msgpack_object *FirstReportInterval_u64,
    msgpack_object *MaxUploadLatency_u64) {
  if (profile
          ->generateNow) { ////If GenerateNow is true and ReportingAdjustments
                           /// is present, ReportingAdjustments shall be ignored
    profile->reportingInterval = 0;
    profile->reportOnUpdate = false;
    profile->firstReportingInterval = 0;
    profile->maxUploadLatency = 0;
    ...
    if (ReportingInterval_u64 && ActivationTimeout_u64 &&
      ...
      T2Error("activationTimeoutPeriod is less than reporting interval. "
              "invalid profile: %s \n",
              profile->name);
      free(profile->name);
      free(profile->hash);
    ...
    if (ReportOnUpdate_boolean != NULL) {
      msgpack_print(ReportOnUpdate_boolean,
                    msgpack_get_obj_name(ReportOnUpdate_boolean));
    ...
    if (profile->timeRef == NULL ||
      ...
      if (FirstReportInterval_u64 != NULL) {
        msgpack_print(FirstReportInterval_u64,
                      msgpack_get_obj_name(FirstReportInterval_u64));
        MSGPACK_GET_NUMBER(FirstReportInterval_u64,
                           profile->firstReportingInterval);
    ...
    if (profile->timeRef != NULL &&
      ...
      if (MaxUploadLatency_u64 != NULL) {
        msgpack_print(MaxUploadLatency_u64,
                      msgpack_get_obj_name(MaxUploadLatency_u64));
...
T2ERROR addParameterMsgpack_marker_config(Profile *profile,
  ...
  for (i = 0; i < Parameter_array_size; i++) {
    ...
    Parameter_reportEmpty_boolean =
        msgpack_get_map_value(Parameter_array_map, "reportEmpty");
    msgpack_print(Parameter_reportEmpty_boolean,
                  msgpack_get_obj_name(Parameter_reportEmpty_boolean));
    ...
    msgpack_print(Parameter_trim_boolean,
                  msgpack_get_obj_name(Parameter_trim_boolean));
    MSGPACK_GET_NUMBER(Parameter_trim_boolean, trim);
    T2Debug("trimParam: %u\n", trim);

    Parameter_regex_str = msgpack_get_map_value(Parameter_array_map, "regex");
    msgpack_print(Parameter_regex_str,
                  msgpack_get_obj_name(Parameter_regex_str));
    ...
    if (0 == msgpack_strcmp(Parameter_type_str, "dataModel")) {
      ...
      msgpack_print(Parameter_name_str,
                    msgpack_get_obj_name(Parameter_name_str));
      header = msgpack_strdup(Parameter_name_str);

      Parameter_reference_str =
          msgpack_get_map_value(Parameter_array_map, "reference");
      msgpack_print(Parameter_reference_str,
                    msgpack_get_obj_name(Parameter_reference_str));
      ...
      Parameter_method_str =
          msgpack_get_map_value(Parameter_array_map, "method");
      msgpack_print(Parameter_method_str,
                    msgpack_get_obj_name(Parameter_method_str));
      method = msgpack_strdup(Parameter_method_str);
      Parameter_reportTimestamp_str =
          msgpack_get_map_value(Parameter_array_map, "reportTimestamp");
      if (method) {
        T2Debug("Method property is present and the value is %s\n", method);
        if (!(strcmp(method, "subscribe"))) {
          T2Info(
              "Method is subscribe converting the parameter to event type\n");
          // free now as we will change the values in the below code which will
          // be freed later
          free(header);
          free(paramtype);
          free(content);
          // reportTimestamp parameter is applicable only when method is
          // subscribe
          if (Parameter_reportTimestamp_str != NULL) {
            msgpack_print(Parameter_reportTimestamp_str,
                          msgpack_get_obj_name(Parameter_reportTimestamp_str));
            if (0 ==
                msgpack_strcmp(Parameter_reportTimestamp_str, "Unix-Epoch")) {
              rtformat = REPORTTIMESTAMP_UNIXEPOCH;
            }
          }
          header = msgpack_strdup(Parameter_reference_str);
      ...
      Parameter_eventName_str =
          msgpack_get_map_value(Parameter_array_map, "eventName");
      msgpack_print(Parameter_eventName_str,
                    msgpack_get_obj_name(Parameter_eventName_str));
      header = msgpack_strdup(Parameter_eventName_str);

      Parameter_component_str =
          msgpack_get_map_value(Parameter_array_map, "component");
      msgpack_print(Parameter_component_str,
                    msgpack_get_obj_name(Parameter_component_str));
      ...
      Parameter_marker_str =
          msgpack_get_map_value(Parameter_array_map, "marker");
      msgpack_print(Parameter_marker_str,
                    msgpack_get_obj_name(Parameter_marker_str));
      header = msgpack_strdup(Parameter_marker_str);

      Parameter_search_str =
          msgpack_get_map_value(Parameter_array_map, "search");
      msgpack_print(Parameter_search_str,
                    msgpack_get_obj_name(Parameter_search_str));
      content = msgpack_strdup(Parameter_search_str);

      Parameter_logFile_str =
          msgpack_get_map_value(Parameter_array_map, "logFile");
      msgpack_print(Parameter_logFile_str,
                    msgpack_get_obj_name(Parameter_logFile_str));
      logfile = msgpack_strdup(Parameter_logFile_str);

      if ((Parameter_firstSeekFromEOF_int = msgpack_get_map_value(
               Parameter_array_map, "firstSeekFromEOF")) != NULL) {
        msgpack_print(Parameter_firstSeekFromEOF_int,
                      msgpack_get_obj_name(firstSeekFromEOF));
    ...
    if (header != NULL && content != NULL) {
      ...
      T2Error("%s Error in adding parameter to profile %s \n", __FUNCTION__,
              profile->name); // header and content is mandatory for adding the
                              // parameter
      if (header != NULL) {
        free(header);
      }
...
T2ERROR encodingSetMsgpack(Profile *profile, msgpack_object *value_map) {
  ...
  ReportTimestamp_str =
      msgpack_get_map_value(JSONEncoding_map, "ReportTimestamp");
...
T2ERROR protocolSetMsgpack(Profile *profile, msgpack_object *value_map,
  ...
  if (0 == msgpack_strcmp(Protocol_str, "HTTP")) {
    ...
    Vector_Create(&(profile->t2HTTPDest->RequestURIparamList));
    RequestURIParameter_array =
        msgpack_get_map_value(HTTP_map, "RequestURIParameter");
    int httpParamCount = 0;
    MSGPACK_GET_ARRAY_SIZE(RequestURIParameter_array,
                           RequestURIParameter_array_size);
    for (i = 0; i < RequestURIParameter_array_size; i++) {
      ...
      href = NULL;
      hname = NULL;
      RequestURIParameter_array_map =
          msgpack_get_array_element(RequestURIParameter_array, i);

      RequestURIParameter_Reference_str =
          msgpack_get_map_value(RequestURIParameter_array_map, "Reference");
      ...
      if (NULL != href) { /*if reference is empty avoid adding this object*/
        RequestURIParameter_Name_str =
            msgpack_get_map_value(RequestURIParameter_array_map, "Name");
        msgpack_print(RequestURIParameter_Name_str,
                      msgpack_get_obj_name(RequestURIParameter_Name_str));
  ...
  if (0 == msgpack_strcmp(Protocol_str, "RBUS_METHOD")) {
    ...
    Vector_Create(&(profile->t2RBUSDest->rbusMethodParamList));
    RbusMethodParameter_array =
        msgpack_get_map_value(RbusMethod_map, "Parameters");
    int rbusMethodParamCount = 0;
    MSGPACK_GET_ARRAY_SIZE(RbusMethodParameter_array,
                           RbusMethodParameter_array_size);
    ...
    for (i = 0; i < RbusMethodParameter_array_size; i++) {
      ...
      RbusMethodIParameter_array_map =
          msgpack_get_array_element(RbusMethodParameter_array, i);

      RequestRbusParameter_Name_str =
          msgpack_get_map_value(RbusMethodIParameter_array_map, "name");
      msgpack_print(RequestRbusParameter_Name_str,
                    msgpack_get_obj_name(RequestRbusParameter_Name_str));
      name = msgpack_strdup(RequestRbusParameter_Name_str);

      RequestRbusParameter_Value_str =
          msgpack_get_map_value(RbusMethodIParameter_array_map, "value");
      msgpack_print(RequestRbusParameter_Value_str,
                    msgpack_get_obj_name(RequestRbusParameter_Value_str));
...
T2ERROR processMsgPackConfiguration(msgpack_object *profiles_array_map,
  ...
  if (profile == NULL) {
    T2Error(
        "Malloc error exiting: can not allocate memory to create profile \n");
  ...
  ReportingAdjustments_map =
      msgpack_get_map_value(value_map, "ReportingAdjustments");
  if (ReportingAdjustments_map) {
    ReportOnUpdate_boolean =
        msgpack_get_map_value(ReportingAdjustments_map, "ReportOnUpdate");
    FirstReportInterval_u64 = msgpack_get_map_value(ReportingAdjustments_map,
                                                    "FirstReportingInterval");
    MaxUploadLatency_u64 =
        msgpack_get_map_value(ReportingAdjustments_map, "MaxUploadLatency");
  }

  ReportingInterval_u64 = msgpack_get_map_value(value_map, "ReportingInterval");
  msgpack_print(ReportingInterval_u64,
                msgpack_get_obj_name(ReportingInterval_u64));
  ...
  if (ReportingInterval_u64 && MaxUploadLatency_u64 &&
      ((profile->reportingInterval * 1000) < profile->maxUploadLatency)) {
    T2Error("MaxUploadLatency is greater than reporting interval. Invalid "
            "Profile\n");
    if (profile->name) {
      free(profile->name);
  ...
  msgpack_print(DeleteOnTimout_boolean,
                msgpack_get_obj_name(DeleteOnTimeout_boolean));
  ...
  msgpack_print(ActivationTimeout_u64,
                msgpack_get_obj_name(ActivationTimeout_u64));
  ...
  T2ERROR retval = time_param_MsgPackReporting_Adjustments_valid_set(
      profile, ReportingInterval_u64, ActivationTimeout_u64, TimeReference_str,
      ReportOnUpdate_boolean, FirstReportInterval_u64, MaxUploadLatency_u64);
  if (retval != T2ERROR_SUCCESS) {
    T2Error("Time parameters are not valid\n");
    return retval;
  }
  if (TriggerConditionArray && (!profile->generateNow) &&
      (!profile->reportingInterval)) {
    profile->reportingInterval = UINT_MAX;
  }
  if (profile->reportingInterval) {
  ...
  msgpack_print(EncodingType_str,
                msgpack_get_obj_name(EncodingType_str)); // ENCODING PARAMETERS
  profile->encodingType = msgpack_strdup(EncodingType_str);
  if (0 == msgpack_strcmp(EncodingType_str, "JSON")) {
    msgpack_object *JSONEncoding_map =
        msgpack_get_map_value(value_map, "JSONEncoding");
    msgpack_object *ReportFormat_str =
        msgpack_get_map_value(JSONEncoding_map, "ReportFormat");
    ...
    if (NULL == profile->jsonEncoding) {
      ...
      free(profile);
      T2Error("Malloc error exiting: can not allocate memory to create "
              "profile->jsonEncoding \n");
      return T2ERROR_MEMALLOC_FAILED;
    }
  }

  msgpack_print(Protocol_str,
                msgpack_get_obj_name(Protocol_str)); // PROTOCOL PARAMETERS
  ...
  if (0 == msgpack_strcmp(Protocol_str, "HTTP")) {
    ...
    msgpack_object *Compression_str =
        msgpack_get_map_value(HTTP_map, "Compression");
    ...
    if (profile->t2RBUSDest->rbusMethodName == NULL ||
        strcmp(profile->t2RBUSDest->rbusMethodName, "") == 0) {
      T2Error("RBUS Method not configured. Ignoring profile %s \n",
              profile->name);
    ...
    if (profile->t2RBUSDest->rbusMethodName &&
        !rbusCheckMethodExists(profile->t2RBUSDest->rbusMethodName)) {
      T2Warning("WARN: no method provider: %s %s\n", profile->name,
                profile->t2RBUSDest->rbusMethodName);
    }
  } else {
  ...
  T2Debug("profile->RootName: %s\n", profile->RootName);
  // Protocol Configuration
  retval = protocolSetMsgpack(profile, value_map,
                              Protocol_str); // Protocol Parameter
  if (retval != T2ERROR_SUCCESS) {
