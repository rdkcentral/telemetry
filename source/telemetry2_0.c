  ...
  if (DAEMONPID == getpid()) {
    ...
    if (sig == SIGINT) {
      ...
      if (T2ERROR_SUCCESS == startXConfClient()) {
        ...
        T2Info("XCONF config reload - IN PROGRESS ... Ignore current reload "
               "request \n");
      }
#endif
    ...
    // This logic is added to terminate child imediately if we get terminate
    // signals eg:SIGTERM / SIGKILL ...
    if (!(sig == SIGUSR1 || sig == LOG_UPLOAD || sig == LOG_UPLOAD_ONDEMAND ||
          sig == SIGIO || sig == SIGCHLD || sig == SIGPIPE || sig == SIGUSR2 ||
          sig == EXEC_RELOAD || sig == SIGALRM)) {
      exit(0);
    }
  }
}
...
static void t2DaemonMainModeInit() {
  ...
  /**
   * Signal handling is being used as way to handle log uploads . Double check
   * whether we get minidump events for crashes
   */
#ifdef INCLUDE_BREAKPAD
#ifndef ENABLE_RDKC_SUPPORT
...
static int checkAnotherTelemetryInstance(void) {
  ...
  /* OK to proceed (lock will be released and file descriptor will be closed on
   * exit) */
