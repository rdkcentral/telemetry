...
TEST_F(reportgenTestFixture, encodeGrepResultInJSON_trim_and_regexParam_both_set_success)
{
    ...
    gparam->mType = MTYPE_ABSOLUTE;
    gparam->trimParam = true;
    gparam->regexParam = strdup("[A-Z]+");
    gparam->u.markerValue = strdup("  FOOfoo  ");
    ...
    // Ensure all these are mocked (otherwise real functions are called, risking invalid pointer use)
    EXPECT_CALL(*m_reportgenMock, regcomp(_, _, _)).WillRepeatedly(Return(0));
    EXPECT_CALL(*m_reportgenMock, regexec(_, _, _, _, _)).WillRepeatedly([](const regex_t*, const char*, size_t, regmatch_t* pmatch, int) {
        pmatch[0].rm_so = 0; pmatch[0].rm_eo = 3; // Simulate "FOO"
        return 0;
    });
    EXPECT_CALL(*m_reportgenMock, regfree(_)).WillRepeatedly(Return());

    // If your applyRegexToValue mock is present, either let code call it, or also mock
    EXPECT_CALL(*m_reportgenMock, applyRegexToValue(::testing::_, StrEq("[A-Z]+")))
        .WillOnce([](char **val, const char*) {
            // Don't free here: let the production code manage memory.
            if (*val) free(*val);
            *val = strdup("FOO");
...
TEST_F(reportgenTestFixture, encodeGrepResultInJSON_null_marker_skipped) {
    Vector *list = nullptr;
    ASSERT_EQ(Vector_Create(&list), T2ERROR_SUCCESS);
    Vector_PushBack(list, nullptr); // Intentionally NULL marker
    cJSON *valArray = (cJSON*)malloc(sizeof(cJSON));
    EXPECT_EQ(T2ERROR_SUCCESS, encodeGrepResultInJSON(valArray, list));
    Vector_Destroy(list, nullptr);
    free(valArray);
}
// 2. Counter marker with regex: regcomp fails, then regexec fails, then succeeds (full inner flow)
TEST_F(reportgenTestFixture, encodeGrepResultInJSON_counter_regex_all_branches) {
    Vector* list = NULL;
    ASSERT_EQ(Vector_Create(&list), T2ERROR_SUCCESS);
    GrepMarker* marker = (GrepMarker*)malloc(sizeof(GrepMarker));
    memset(marker, 0, sizeof(GrepMarker));
    ...
    marker->u.count = 2;
    marker->markerName = strdup("mark1");
    marker->regexParam = strdup("[0-9]");
    marker->trimParam = true;
    Vector_PushBack(list, marker);
    cJSON* valArray = (cJSON*)malloc(sizeof(cJSON));
    cJSON* obj = (cJSON*)0xa10;
    // Object creation
    EXPECT_CALL(*m_reportgenMock, cJSON_CreateObject()).WillOnce(Return(obj));
    // String add fails, to test error path
    EXPECT_CALL(*m_reportgenMock, cJSON_AddStringToObject(obj, _, _)).WillOnce(Return(nullptr));
    EXPECT_CALL(*m_reportgenMock, cJSON_Delete(obj)).Times(1);
    EXPECT_EQ(T2ERROR_FAILURE, encodeGrepResultInJSON(valArray, list));
    free(marker->markerName);
    free(marker->regexParam);
    free(marker);
    Vector_Destroy(list, nullptr);
...
TEST_F(reportgenTestFixture, encodeEventMarkersInJSON_TimevectorToarray_null_cleanup)
{
    ...
    // First cJSON_CreateArray() → vector for values: succeed. Second → for timestamp vector: fail!
    EXPECT_CALL(*m_reportgenMock, cJSON_CreateArray())
        .WillOnce(Return(vectorToArray))  // for accumulatedValues
        .WillOnce(Return(nullptr));       // for accumulatedTimestamp

    // Regex extraction simulation on accumulatedValues
    ...
    // Value vector conversion (optional, since function under test will call it)
    // Clean up: arrayItem should be deleted
    EXPECT_CALL(*m_reportgenMock, cJSON_Delete(arrayItem)).Times(1);

    // The function should return failure due to timestamp array OOM
    EXPECT_EQ(T2ERROR_FAILURE, encodeEventMarkersInJSON(valArray, eventMarkerList));
...
TEST_F(reportgenTestFixture, tagReportAsCached_NoArrayErrorBranch) {
    char* fakeJson = strdup("{\"foo\":42}");
    ...
    EXPECT_CALL(*m_reportgenMock, cJSON_AddStringToObject(
        ...
        .WillOnce(::testing::Return(mockReportTypeObj));
    // Try "searchResult" (not found)
    EXPECT_CALL(*m_reportgenMock, cJSON_GetObjectItemCaseSensitive(mockRoot, ::testing::StrEq("searchResult")))
        .WillOnce(::testing::Return(nullptr));
    // Try "Report" (not found)
    EXPECT_CALL(*m_reportgenMock, cJSON_GetObjectItemCaseSensitive(mockRoot, ::testing::StrEq("Report")))
        .WillOnce(::testing::Return(nullptr));
    // Clean up both objects on error
