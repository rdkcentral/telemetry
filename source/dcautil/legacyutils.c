...
void removeProfileFromSeekMap(char *profileName) {
  ...
  pthread_mutex_lock(&pSeekLock);
  if (profileSeekMap) {
    GrepSeekProfile *gsProfile =
        (GrepSeekProfile *)hash_map_remove(profileSeekMap, profileName);
    if (NULL != gsProfile) {
...
void removeProfileFromExecMap(char *profileName) {
  ...
  pthread_mutex_unlock(&pExecCountLock);
  // Adding this logic if profile name is same then we will not remove the
  // seekMap in this case we need to check if the seek map exists, and assign
  // the execCounter to 0
  pthread_mutex_lock(&pSeekLock);
  if (profileSeekMap) {
    GrepSeekProfile *gsProfile =
        (GrepSeekProfile *)hash_map_get(profileSeekMap, profileName);
    if (NULL != gsProfile) {
      gsProfile->execCounter = 0;
    }
  }
 ...
 */
static int getLogSeekValue(hash_map_t *logSeekMap, char *name,
                           long *seek_value) {
  ...
  if (logSeekMap) {
    ...
    if (data) {
      *seek_value = *data;
      ...
      T2Debug(
          "data is null .. Setting seek value to 0 from getLogSeekValue \n");
      *seek_value = 0;
    }
  } else {
...
/**
 * @brief This API is to find the load average of system and add it to the
 * SearchResult JSON.
 *
...
static int fsize(FILE *fp) {
  ...
  if (prev >= 0) {
    if (fseek(fp, prev, SEEK_SET) != 0) {
      T2Error("Cannot set the file position indicator for the stream pointed "
              "to by stream\n");
    }
  }
  return sz;
...
void updateLastSeekval(hash_map_t *logSeekMap, char **prev_file,
  ...
  if ((NULL == PERSISTENT_PATH) || (NULL == LOG_PATH) || (NULL == filename) ||
      (NULL == logSeekMap)) {
...
char *getLogLine(hash_map_t *logSeekMap, char *buf, int buflen, char *name,
  ...
  /* If name is already an absolute path then prepend empty string instead of
   * LOG_PATH */
  if (name[0] == '/')
    logpath = "";
  ...
  if (NULL == pcurrentLogFile) {
    ...
    if (NULL != currentLogFile) {
      ...
      if (0 != getLogSeekValue(logSeekMap, name, &seek_value)) {
        ...
        // if the seek value is given in the profile use that value to calculate
        // the seek value from the END OF FILE the seek value becomes filesize -
        // firstSeekFromEOF since it cant be negetive set it zero if negative
        if (*firstSeekFromEOF != 0) {
          seek_value = (fileSize > (*firstSeekFromEOF))
                           ? (fileSize - (*firstSeekFromEOF))
                           : 0;
          T2Debug("First Seek from the EOF is given  value : %d; the size of "
                  "file is %ld; current seek after calculations %ld\n",
                  *firstSeekFromEOF, fileSize, seek_value);
          *firstSeekFromEOF = 0; // update this to zero as this initial seek
                                 // update should only run for the first time
        }
        if (seek_value <= fileSize) {
          if (check_rotated_logs) { // considering the rotated log file  within
                                    // few minutes after bootup, it should check
                                    // only for the first time
            char *rotatedLog = malloc(fileExtn_len);
            if (NULL != rotatedLog) {
              snprintf(rotatedLog, fileExtn_len, "%s%s%s", logpath, name,
                       fileExtn);
              ...
              if (fseek(pcurrentLogFile, seek_value, 0) != 0) {
                T2Error("Cannot set the file position indicator for the stream "
          ...
          if ((NULL != DEVICE_TYPE) &&
              (0 == strcmp("broadband", DEVICE_TYPE))) {
...
/**
 *  @brief Function to update the global paths like PERSISTENT_PATH,LOG_PATH
 * from include.properties file.
 *
...
/**
 *  @brief Function to update the configuration values from device.properties
 * file.
 *
 *  @param[in] logpath   Log file path
 *  @param[in] perspath  Persistent path
...
bool isPropsInitialized() { return isPropsIntialized; }
