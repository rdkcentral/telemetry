 ...
 * @brief       rbusFilter
 * @defgroup    rbusFilter
 * @brief       An rbus filter represents a value test placed on an event that
 * must be met before the event is published.
 *
 * The filter contains an expression which evaluates to a boolean result when a
 * test value is applied to it. There are 2 types of filter : relation and
 * logic.  These are tantamount to the relation and logic operators in C/C++.
 * Relation operator are: >, >=, <, <=, ==, and !=.  Logic operators are: &&,
 * ||, and !. Relation filters are created with rbusFilter_InitRelation and are
 * what an event value is tested with. They are defined with an operator (e.g.
 * >) and a value (e.g. 10) to form an expression such as: (X > 10). Logic
 * filters are created with rbusFilter_InitLogic and are applied on relation
 * filters to AND, OR, or NOT them, such as ((X > 10) || (X < -10)).
 * Additionally, Logic filters can be applied to other logic filters to created
 * nested logic such as: ((X > 10) || (X < -10)) || ((X > -5) && (X < 5)). After
 * a filter is created, its expression can be evaluate using a given value (a
 * given X) by calling rbusFilter_Apply. For example, given X=3, the last
 ...
 * This creates an equation in the form (X op value), where the op and value are
 * supplied with this function, and the X to actually compare against is
 * supplied with rbusFilter_Apply. It's the caller's responsibility to release
 * ownership by calling rbusFilter_Release once it's done with it.
 *  @param  filter      A reference to an address where the new filter will be
 * assigned. The caller is responsible for releasing the filter with
 * rbusFilter_Release.
 *  @param  op          The relation operator to use
 *  @param  value       The value to compare against
 */
void rbusFilter_InitRelation(rbusFilter_t *filter,
                             rbusFilter_RelationOperator_t op,
                             rbusValue_t value);
 ...
 *
 * Create a logic filter from a logic operator and 1 or 2 filters to apply the
 * logic operator to. This creates an equation in the form either (left op
 * right) if op is not RBUS_FILTER_OPERATOR_NOT, or !(left) otherwise.  This
 * function allows you to combine both relation and logic filters to create
 * nested expression.  The filter is actually tested against a value using
 * rbusFilter_Apply. It's the caller's responsibility to release ownership by
 * calling rbusFilter_Release once it's done with it.
 *  @param  filter      A reference to an address where the new filter will be
 * assigned. The caller is responsible for releasing the filter with
 * rbusFilter_Release.
 *  @param  op          The logic operator to use
 *  @param  left        The left side of the equation
 *                      If op is RBUS_FILTER_OPERATOR_NOT then only the left
 * filter will be notted
 *  @param  right       The right side of the equation
 *                      If op is RBUS_FILTER_OPERATOR_NOT the right should be
 * NULL.
 */
void rbusFilter_InitLogic(rbusFilter_t *filter, rbusFilter_LogicOperator_t op,
                          rbusFilter_t left, rbusFilter_t right);
 ...
 * rbusFilter_Init.... Additional owners can be assigned afterwards with
 * rbusFilter_Retain. Each owner must call rbusFilter_Release once done using
 * the filter.
 ...
 *         a filter that was retained with either rbusFilter_Init... or
 * rbusFilter_Retain.
...
rbusFilter_RelationOperator_t
rbusFilter_GetRelationOperator(rbusFilter_t filter);
 ...
 *  @brief A debug utility function to write the filter as a string to a file
 * stream.
 */
