 ...
 * --------------------
 * calculates the elapsed time between time2 and time1(in seconds and nano
 * seconds)
 *
 * time2: start time
 * time1: finish time
 ...
 */
int getLapsedTime(struct timespec *output, struct timespec *time1,
...
// This function is used to calculate the difference between the currentUTC time
// and UTC time configured in the TimeReference.It takes hr:min:sec for the time
// calculation. When the timeRef value is missed the report will be generated at
// the same time ref after 24 hrs.
static unsigned int getSchdInSec(char *timeRef) {
  ...
  timenow = (currtimeptr.tm_hour * 3600) + (currtimeptr.tm_min * 60) +
            currtimeptr.tm_sec;
...
void *TimeoutThread(void *arg) {
  ...
  while (tProfile->repeat && !tProfile->terminated && tProfile->name) {
    ...
    if (tProfile->timeRef &&
        strcmp(tProfile->timeRef, DEFAULT_TIME_REFERENCE) != 0) {
    ...
    if (tProfile->firstexecution == true) {
      ...
      if (tProfile->timeRefinSec !=
          0) { // this loop is used to choose the minimum waiting value based on
               // the comparison b/w TimeRef and Reporting Interval
        if (tProfile->timeOutDuration <= tProfile->timeRefinSec) {
          _ts.tv_sec += tProfile->timeOutDuration;
          T2Info("Waiting for %d sec for next TIMEOUT for profile as reporting "
                 "interval is taken - %s\n",
          ...
          T2Info("Waiting for %d sec for next TIMEOUT for profile as Time "
                 "Reference is taken - %s\n",
    ...
    if (tProfile->firstreportint > 0 && tProfile->firstexecution == true) {
      ...
      T2Info("Waiting for %d sec for next TIMEOUT for profile as "
             "firstreporting interval is given - %s\n",
             tProfile->firstreportint, tProfile->name);
      ...
      if (tProfile->timeOutDuration == UINT_MAX &&
        ...
        notifySchedulerstartcb(tProfile->name, true);
        T2Info("Waiting for condition as reporting interval is not configured "
               "for profile - %s\n",
               tProfile->name);
        n = pthread_cond_wait(&tProfile->tCond, &tProfile->tMutex);
      } else {
        notifySchedulerstartcb(tProfile->name, true);
        T2Info("Waiting for timeref or reporting interval for the profile - %s "
               "is started\n",
               tProfile->name);
        n = pthread_cond_timedwait(&tProfile->tCond, &tProfile->tMutex, &_ts);
      }
    ...
    if (n == ETIMEDOUT) {
      ...
      if (minThresholdTime) {
        ...
        T2Debug("minThresholdTime left %ld -\n",
                (long int)(_MinThresholdTimeTs.tv_sec -
                           _MinThresholdTimeStart.tv_sec));
        if (minThresholdTime < (unsigned int)(_MinThresholdTimeTs.tv_sec -
                                              _MinThresholdTimeStart.tv_sec)) {
          minThresholdTime = 0;
    ...
    if (profileName == NULL || (strcmp(profileName, tProfile->name) == 0)) {
      ...
      if (mutex_return != 0) {
        ...
        if (mutex_return == EBUSY)
          T2Error("tProfile Mutex is Busy, already the report generation might "
                  "be in progress \n");
...
void uninitScheduler() {
  ...
  for (; index < profileList->count; ++index) {
    ...
    T2Info(" tProfile->tId = %d tProfile->name = %s\n", (int)tProfile->tId,
           tProfile->name);
    // pthread_join(tProfile->tId, NULL); // pthread_detach in
    // freeSchedulerProfile will detach the thread
...
T2ERROR registerProfileWithScheduler(
    ...
    unsigned int activationTimeout, bool deleteonTimeout, bool repeat,
  ...
  if (timeoutNotificationCb == NULL) {
    T2Error("Timerout Callback not set - Scheduler isn't initialized yet, "
    ...
    if (sc_initialized) { // Check for existing scheduler to avoid duplicate
                          // scheduler entries
      T2Debug("%s:%d scMutex is locked\n", __FUNCTION__, __LINE__);
      if (pthread_mutex_lock(&scMutex) != 0) {
        T2Error("scMutex lock failed\n");
    ...
    SchedulerProfile *tProfile =
        (SchedulerProfile *)malloc(sizeof(SchedulerProfile));
...
T2ERROR unregisterProfileFromScheduler(const char *profileName) {
  ...
  for (; index < profileList->count; ++index) {
    ...
    if (strcmp(tProfile->name, profileName) == 0) {
      ...
      // pthread_join(tProfile->tId, NULL); // pthread_detach in
      // freeSchedulerProfile will detach the thread
      sched_yield(); // This will give chance for the signal receiving thread to
                     // start
      int count = 0;
