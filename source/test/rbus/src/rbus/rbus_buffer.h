...
void rbusValue_SetTLV(rbusValue_t v, rbusValueType_t type, uint32_t length,
                      void const *value);
...
void rbusBuffer_WriteTypeLengthValue(rbusBuffer_t buff, rbusValueType_t type,
                                     uint16_t length, void const *value);
...
/*The Read functions return 0 on success or -1 on failure (reading that would
 * result in memory overrun)*/
...
int rbusBuffer_ReadString(rbusBuffer_t const buff, char **s,
                          int *len); /* caller must free *s */
int rbusBuffer_ReadDateTime(rbusBuffer_t const buff, rbusDateTime_t *tv);
int rbusBuffer_ReadBytes(rbusBuffer_t const buff, uint8_t **bytes,
                         int *len); /* caller must free *bytes */
