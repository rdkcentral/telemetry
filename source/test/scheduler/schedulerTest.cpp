...
{
    EXPECT_EQ(T2ERROR_SUCCESS, SendInterruptToTimeoutThread("RDKB_Profile", false));
}
...
TEST(SENDINTERRUPTTOTIMEOUTTHREAD, NOT_INITIALIZED)
{
    ...
    EXPECT_EQ(T2ERROR_FAILURE, SendInterruptToTimeoutThread("SHOULD_FAIL", false));
}
...
{
    EXPECT_EQ(T2ERROR_INVALID_ARGS, SendInterruptToTimeoutThread(NULL, false));
}
#if 0
