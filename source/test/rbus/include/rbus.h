 ...
 *              This is the only file a provider or client needs to include in
 * their project.
 */
 ...
 *  - register object and implement their get, set, create, and delete
 * operations
 ...
 *
 * A process can be both a provider and a client.  A process can implement many
 * properties, objects, events, and remote methods, and also be a client of the
 * same coming from other providers.
 *
 * All objects, properties, events, and methods are assigned names by the
 * provider. Each name should be unique across a system, otherwise there will be
 * conflicts in the bus routing.
 *
 * RBus supports the naming convention defined by TR-069, where a name has a
 * hierarchical structure like a file directory structure, with each level of
 * the hierarchy separated by a dot ('.'), and where object instances and
 * denoted using brace({}).
 *
 * The RBus API is intended, but not limited, to allow the implementation of a
 * TR-181 data model.
 */
...
///  @brief     The maximum hierarchical depth (e.g. the max token count) a name
///  can be for any element.
...
typedef enum _rbusError {
  ...
  RBUS_ERROR_ELEMENT_NAME_DUPLICATE, /**< One or more element name(s) were
                                        previously registered */
  RBUS_ERROR_ELEMENT_NAME_MISSING, /**< No names were provided in the name field
                                    */
  RBUS_ERROR_COMPONENT_DOES_NOT_EXIST, /**< A bus connection for this component
                                          name was not previously opened. */
  RBUS_ERROR_ELEMENT_DOES_NOT_EXIST,   /**< One or more data element name(s) do
                                          not currently have a valid registration
                                        */
  RBUS_ERROR_ACCESS_NOT_ALLOWED, /**< Access to the requested data element was
                                    not permitted by the provider component. */
  RBUS_ERROR_INVALID_CONTEXT, /**< The Context is not same as what was sent in
                                 the get callback handler.*/
  RBUS_ERROR_TIMEOUT,         /**< The operation timedout   */
  RBUS_ERROR_ASYNC_RESPONSE,  /**< The method request will be handle
                                 asynchronously by provider */
...
typedef struct _rbusSetOptions {
  ...
  uint32_t sessionId; /**< Session id. One or more parameter can be set together
                         over a session. The session ID binds all set
                         operations. A value of 0 indicates this is not a
                         session based operation. A non-zero value indicates
                         that the value should be "remembered" temporarily. Only
                         when the "commit" parameter is "true", should all
                         remembered parameters in this session be set together.
                           Call rbus_createSession to generate a session id.*/
} rbusSetOptions_t;

/** @struct     rbusGetHandlerOptions_t
 *  @brief      Additional options that are passed to the provider when GET
 * function called.
...
typedef struct _rbusGetHandlerOptions {
  ...
  char const
      *requestingComponent; /**< Component that invoking the GET method. */
} rbusGetHandlerOptions_t;

/** @struct     rbusSetHandlerOptions_t
 *  @brief      Additional options that are passed to the provider when SET
 * function called.
 *  @ingroup    Common Providers
 */
typedef struct _rbusSetHandlerOptions {
  ...
  uint32_t sessionId; /**< Session id. One or more parameter can be set together
                         ...
                         remembered parameters in this session be set together.
                           Call rbus_createSession to generate a session id.*/
  char const
      *requestingComponent; /**< Component that invoking the SET method. */
 ...
 */
typedef enum {
  RBUS_EVENT_OBJECT_CREATED, /**< Notification that an object instance was
                                created in table. */
  RBUS_EVENT_OBJECT_DELETED, /**< Notification that an object instance was
                                deleted in table. */
  RBUS_EVENT_VALUE_CHANGED, /**< Notification that a property value was changed.
                             */
  RBUS_EVENT_GENERAL,       /**< Provider defined event.*/
  RBUS_EVENT_INITIAL_VALUE, /**< Notification of initial value immediately after
                               subscription*/
 ...
 *          rbusError_t error)
 *  @brief A component will receive this API callback when a subscription
 * response is received. This callback is registered with
 * rbusEvent_SubscribeAsync or rbusEvent_SubscribeExAsync. \n Used by: Any
 * component that subscribes async for events.
 *  @param rbusHandle     Bus Handle
 *  @param subscription   Subscription data created when the client subscribed
 *                        for the event. This will contain the userData, for
 * example.
 *  @param error          Any error that occured
 *                        success, or any provider error code, or timeout (if
 * retry limit reached)
 *  @return void
 */
 ...
 *  @return void
 */
typedef void (*rbusEventHandler_t)(rbusHandle_t handle,
                                   rbusEvent_t const *eventData,
                                   rbusEventSubscription_t *subscription);

typedef void (*rbusEventHandlerRawData_t)(
    rbusHandle_t handle, rbusEventRawData_t const *eventData,
    rbusEventSubscription_t *subscription);
...
typedef struct _rbusEventSubscription {
  ...
  uint32_t interval;     /**< Total interval period after which
                          ...
                          */
  uint32_t
      duration;        /** Optional maximum duration in seconds until which
  ...
  rbusHandle_t handle; /** Private use only: The rbus handle associated with
                          this subscription */
  rbusSubscribeAsyncRespHandler_t
      asyncHandler; /** Private use only: The async handler being used for any
                       background subscription retries */
 ...
 *  @ingroup Methods
 */
typedef void (*rbusMethodAsyncRespHandler_t)(rbusHandle_t handle,
...
/// @brief rbusElementType_t indicates the type of data elements which can be
/// registered with RBus
typedef enum {
 ...
 * already set.  The provider can use this name to identify the property if
 * needed. The provider's responsibility is to set the value of the property
 * parameter. A provider may install this get handler on a table if the provider
 * doesn't use rbusTable_addRow to add rows and instead will handle partial path
 * queries through this get handler.
 *  @param      handle          the rbus handle the property is registered to.
 *  @param      property        the property whose value must be set by the
 * handler.
 *  @param      options         the additional information that to be used for
 * GET.
 *  @return                     RBus error code as defined by rbusError_t.
 */
typedef rbusError_t (*rbusGetHandler_t)(rbusHandle_t handle,
                                        rbusProperty_t property,
 ...
 * property parameter passed to this function will have the name of the property
 * already set.  The provider can use this name to identify the property if
 * needed. The property parameter will also have the value. The
 * rbusSetHandlerOptions_t contains addition information which the provider must
 * use to handle how the write should occur.  It is the provider's
 * responsibility to set its internal representation of that property's value
 * with the value contained within the property parameter.
 *  @param      handle          the rbus handle the property is registered to.
 *  @param      property        the property whose value must be set by the
 * handler.
 *  @param      options         the additional information that to be used for
 ...
 *  @return                     RBus error code as defined by rbusError_t.
 */
typedef rbusError_t (*rbusSetHandler_t)(rbusHandle_t handle,
                                        rbusProperty_t property,
                                        rbusSetHandlerOptions_t *options);
 ...
 * The aliasName parameter can optionally be used to specify a unique name for
 * the row. A new row should be assigned a unique instance number and this
 * number should be returned in the instNum output parameter.
 *  @param  handle          Bus Handle
 *  @param  tableName       The name of a table (e.g. "Device.IP.Interface.")
 *  @param  aliasName       An optional name for the new row.  Must be unique in
 * the table.  Can be NULL.
 *  @param  instNum         Output parameter where the instance number for the
 * new row is returned
 *  @return RBus error code as defined by rbusError_t.
 *  Possible values are: RBUS_ERROR_INVALID_INPUT
 */
typedef rbusError_t (*rbusTableAddRowHandler_t)(rbusHandle_t handle,
                                                char const *tableName,
                                                char const *aliasName,
                                                uint32_t *instNum);
 ...
 *
 * A provider must implement this handler to allow rows to be removed from a
 * table. The rowName parameter will be a fully qualified name, specifying
 * either the row's instance number (e.g. "Device.IP.Interface.1") or the row's
 * alias (e.g. "Device.IP.Interface.[lan1]").
 *  @param  handle          Bus Handle
 *  @param  rowName         The name of a table row (e.g.
 * "Device.IP.Interface.1")
 *  @return RBus error code as defined by rbusError_t.
 *  Possible values are: RBUS_ERROR_INVALID_INPUT
 */
typedef rbusError_t (*rbusTableRemoveRowHandler_t)(rbusHandle_t handle,
                                                   char const *rowName);
 ...
 * calling rbusMethod_SendAsyncResponse, passing the asyncHandle and output
 * params.
 *  @param  handle          Bus Handle
 *  @param  methodName      The name of the method being invoked ( e.g.
 * "Device.Foo.SomeFunc()");
 ...
 *  Possible values are: RBUS_ERROR_INVALID_INPUT
 */
typedef rbusError_t (*rbusMethodHandler_t)(rbusHandle_t handle,
                                           char const *methodName,
                                           rbusObject_t inParams,
                                           rbusObject_t outParams,
                                           rbusMethodAsyncHandle_t asyncHandle);
 ...
 *  @param      action          whether the event is being subscribed or
 * unsubscribe to.
 *  @param      eventName       the fully qualified event name.
 *  @param      filter          an the filter the subscriber would like the
 * provider to use to decide when the event can be sent.  This can be NULL if no
 * filter was specified by the client.
 *  @param      interval        if non-zero, indicates that the event should be
 * publish repeatedly every 'interval' seconds.  if a filter is also provided,
 * it would publish only when the filter is triggered
 *  @param      autoPublish     output parameter used to disable the default
 * behaviour where rbus automatically publishing events for provider data
 * elements. When providers set autoPublish to true the value will be checked
 * once per second and the maximum event rate is one event per two seconds. If
 * faster eventing or real-time eventing is required providers can set
 * autoPublish to false and implement a custom approach. For fastest response
 * time and to avoid missing changes that occur faster than once per second, the
 *                              preferred way is to use a callback triggered
 * from the lowest level of code to detect a value change. This callback may be
 * invoked by vendor code via a HAL API or other method.  This callback can be
 * received by the component that provides this event and used to send the
 *                              publish message in real time.
 *  @return                     RBus error code as defined by rbusError_t.
 */
typedef rbusError_t (*rbusEventSubHandler_t)(
    rbusHandle_t handle, rbusEventSubAction_t action, char const *eventName,
 ...
 * eventSubHandler|allow a provider to know when value-change was subscribed
 * to|allow a provider to know when a table update event is subscribed to
 *
 ...
 *
 * A component that wants to handle the logs in its own way must register a
 * callback handler to get the log messages.
 ...
 *  @return                     None.
 */
typedef void (*rbusLogHandler)(rbusLogLevel_t level, const char *file, int line,
 ...
 *          void)
 *
 *  @brief Components use this API to check whether the rbus is enabled in this
 * device/platform Used by: Components that uses rbus to register events, tables
 * and parameters.
 *
 ...
 *  All calls to this API will receive a dedicated bus handle for that
 * component. If a component calls this API more than once, any previous
 * busHandle and all previous data element registrations will be canceled. \n
 ...
 *  @param      handle          Bus Handle
 *  @param      componentName   the name of the component initializing onto the
 * bus
 *  @return                     RBus error code as defined by rbusError_t.
 *  Possible values are:
 ...
 *  socket connection remains up until the last component in that software
 * process closes its bus connection. \n Used by:  All RBus components (multiple
 * components may share a software process)
 *  @param      handle          Bus Handle
 *  @return                     RBus error code as defined by rbusError_t.
 *  Possible values are:
 *  RBUS_ERROR_BUS_ERROR: Indicates there is some bus error. Try later.
 ...
 * @param traceParent The traceparent part of the TraceContext in HTTP header
 * format
 * @param traceState The tracestate part of the TraceContext in HTTP header
 * format
 * @note Neither the tracePraent or traceState need to include the name of the
 * HTTP header only the value. For example. An example HTTP traceparent may look
 * like traceparent: 00-0af7651916cd43dd8448eb211c80319c-b7ad6b7169203331-01 The
 * caller should only supply the actual value. There is no need to include the
 *  "traceparent:" prefix. If the "traceparent:" prefix is also included, it
 * will be removed.
 * @return
 * @see https://www.w3.org/TR/trace-context/
 */
rbusError_t rbusHandle_SetTraceContextFromString(rbusHandle_t rbus,
                                                 char const *traceParent,
                                                 char const *traceState);

rbusError_t rbusHandle_ClearTraceContext(rbusHandle_t rbus);
 ...
 * @see https://www.w3.org/TR/trace-context/
 */
rbusError_t rbusHandle_GetTraceContextAsString(rbusHandle_t rbus,
                                               char *traceParent,
 ...
 *  number of data elements and a list of data  elements.  The bus
 * infrastructure provides the number of components and the component names. \n
 *  Used by: Client
 *  @param      handle          Bus Handle
 *  @param      numElements     The number (count) of data elements
 *  @param      elementNames    The list of element for which components are to
 * be found
 *  @param      numComponents   Total number of components
 *  @param      componentName   Name of the components discovered
 *  @return RBus error code as defined by rbusError_t.
 *  Possible values are:
 *  RBUS_ERROR_ELEMENT_DOES_NOT_EXIST: Data Element was not previously
 * registered.
 */
rbusError_t rbus_discoverComponentName(rbusHandle_t handle, int numElements,
                                       char const **elementNames,
                                       int *numComponents,
                                       char ***componentName);
 ...
 *  Used by: Client
 *  @param      handle          Bus Handle
 *  @param      name            Name of the component or partial path of an
 element name.
 *  @param      nextLevel       Indicates whether to retrieve only the immediate
 elements (true) or retrieve all elements under the partial path (false).
 *  @param      numElements     Number (count) of data elements that were
 discovered
 *  @param      elementNames    List of elements discovered
 *  @return RBus error code as defined by rbusError_t.
 *  Possible values are:
 *  RBUS_ERROR_COMPONENT_DOES_NOT_EXIST: Component name was not previously
 registered.
 */
 ...
 */
rbusError_t rbus_regDataElements(rbusHandle_t handle, int numDataElements,
                                 rbusDataElement_t *elements);
 ...
 *  may be accessed/subscribed by other component(s)
 *  @param      handle      Bus Handle
 *  @param      numDataElements The number (count) of data elements to
 * unregister
 ...
 */
rbusError_t rbus_unregDataElements(rbusHandle_t handle, int numDataElements,
                                   rbusDataElement_t *elements);
 ...
 *
 * The consumer should call rbusValue_Release when finished using the returned
 * value.
 *  @param      handle          Bus Handle
 ...
 *  @return RBus error code as defined by rbusError_t.
 *  Possible values are:
 *  RBUS_ERROR_ACCESS_NOT_ALLOWED: Access to requested parameter is not
 * permitted. RBUS_ERROR_ELEMENT_DOES_NOT_EXIST: Data Element was not previously
 * registered.
 */
 ...
 *  @param      handle          Bus Handle
 *  @param      paramCount      The number (count) of input elements
 (parameters)
 ...
 */
rbusError_t rbus_getExt(rbusHandle_t handle, int paramCount,
                        char const **paramNames, int *numProps,
                        rbusProperty_t *properties);
 ...
 *  @return RBus error code as defined by rbusError_t.
 *  Possible values are:
 *  RBUS_ERROR_ACCESS_NOT_ALLOWED: Access to the requested parameter is not
 * permitted
 */
rbusError_t rbus_getBoolean(rbusHandle_t handle, char const *paramName,
                            bool *paramVal);
 ...
 * permitted
 */
rbusError_t rbus_getInt(rbusHandle_t handle, char const *paramName,
                        int *paramVal);
 ...
 * permitted.
 */
rbusError_t rbus_getUint(rbusHandle_t handle, char const *paramName,
                         unsigned int *paramVal);
 ...
 *  @param      paramVal        The value of the string parameter which the
 * caller must free
 *  @return RBus error code as defined by rbusError_t.
 *  Possible values are:
 *  RBUS_ERROR_ACCESS_NOT_ALLOWED: Access to requested parameter is not
 * permitted.
 */
rbusError_t rbus_getStr(rbusHandle_t handle, char const *paramName,
                        char **paramVal);
 ...
 *                              be created which contains the parameter's name
 * and the respective value to set that parameter to.
 *  @param      opts            Extra options such as session info.
 *                              Set NULL if not needed, in which case a session
 *                              is not used and the set is commited immediately.
 *  @return RBus error code as defined by rbusError_t.
 *  Possible values are:
 *  RBUS_ERROR_ACCESS_NOT_ALLOWED: Access to requested parameter is not
 * permitted. RBUS_ERROR_DESTINATION_NOT_REACHABLE: Destination element was not
 * reachable.
 */
rbusError_t rbus_setMulti(rbusHandle_t handle, int numProps,
                          rbusProperty_t properties, rbusSetOptions_t *opts);
 ...
 * permitted.
 */
rbusError_t rbus_setBoolean(rbusHandle_t handle, char const *paramName,
                            bool paramVal);
 ...
 * permitted
 */
rbusError_t rbus_setInt(rbusHandle_t handle, char const *paramName,
                        int paramVal);
 ...
 *  @param      handle          Bus Handle
 *  @param      paramName       The name of the unsigned integer parameter
 *  @param      paramVal        The value to set the unsigned integer parameter
 * to
 *  @return RBus error code as defined by rbusError_t.
 *  Possible values are:
 *  RBUS_ERROR_ACCESS_NOT_ALLOWED: Access to requested parameter is not
 * permitted.
 */
rbusError_t rbus_setUInt(rbusHandle_t handle, char const *paramName,
                         unsigned int paramVal);
 ...
 * permitted.
 */
rbusError_t rbus_setStr(rbusHandle_t handle, char const *paramName,
                        char const *paramVal);
 ...
 * such as "Device.IP.Interface." The aliasName parameter can optionally be used
 * to specify a unique name for the row.  Any additional properties on the row
 * must be updated separately using set operations.  Each new row gets a unique
 * instance number assigned to it and this is returned in the instNum output
 * parameter.  Consumers can use either the optionally set aliasName or the
 * returned instNum to identify the new row. Used by:  Any component that needs
 * to add a table rows in another component.
 *  @param  handle          Bus Handle
 *  @param  tableName       The name of a table (e.g. "Device.IP.Interface.")
 *  @param  aliasName       An optional name for the new row.  Must be unique in
 * the table.  Can be NULL.
 ...
 *
 * This API removes a row from a table. The rowName parameter must be a fully
 * qualified name, specifying either the row's instance number (e.g.
 * "Device.IP.Interface.1") or the row's alias (e.g.
 * "Device.IP.Interface.[lan1]"). Used by:  Any component that needs to add a
 * table rows in another component.
 *  @param  handle          Bus Handle
 *  @param  rowName         The name of a table row (e.g.
 * "Device.IP.Interface.1")
 *  @return RBus error code as defined by rbusError_t.
 ...
 *  @param  handle          Bus Handle
 *  @param  tableName       The name of a table (e.g. "Device.IP.Interface.")
 *  @param  rowNames        Output parameter where a list of rbusRowName_t
 * structures is returned
 *  @return RBus error code as defined by rbusError_t.
 *  Possible values are: RBUS_ERROR_INVALID_INPUT
 *  @ingroup Tables
 */
rbusError_t rbusTable_getRowNames(rbusHandle_t handle, char const *tableName,
                                  rbusRowName_t **rowNames);
 ...
 *
 * This method is used to free the memory for the row name lists returned from
 * rbusTable_getRowNames. Used by:  Any component that uses the
 * busTable_getRowNames method
 *  @param  handle          Bus Handle
 *  @param  rows            The row name list returned from
 * rbusTable_getRowNames
 *  @return RBus error code as defined by rbusError_t.
 *  Possible values are: RBUS_ERROR_INVALID_INPUT
 *  @ingroup Tables
 */
rbusError_t rbusTable_freeRowNames(rbusHandle_t handle, rbusRowName_t *rows);
 ...
 *
 * This method allows a consumer to get the information on all elements inside
 * an object. Used by:  Any component that needs to know the info on the
 * elements inside an object.
 *  @param  handle          Bus Handle
 *  @param  elemName        The name of the element to start from
 *  @param  depth           Depth control flag
 *                              depth = 0: only the start element
 *                              depth = RBUS_MAX_NAME_DEPTH: the start element,
 * its children, grand-children, ... to max depth depth > 0: the start element,
 * its children, grand-children, ... to the given depth depth < 0: only the
 * elements at the exact depth level of abs(depth)  (e.g. -1 would return only
 * the next level elements)
 *  @param  elemInfo        Output element info list
 *  @return RBus error code as defined by rbusError_t.
 *  Possible values are: RBUS_ERROR_INVALID_INPUT
 */
rbusError_t rbusElementInfo_get(rbusHandle_t handle, char const *elemName,
                                int depth, rbusElementInfo_t **elemInfo);
 ...
 *
 * This method allows a consumer to free the element info list returned from the
 * rbusElementInfo_get method. Used by:  Any component that uses the
 * rbusElementInfo_get method
 *  @param  handle          Bus Handle
 *  @param  elemInfoList    The element info list return from
 * rbusElementInfo_get
 *  @return RBus error code as defined by rbusError_t.
 *  Possible values are: RBUS_ERROR_INVALID_INPUT
 */
rbusError_t rbusElementInfo_free(rbusHandle_t handle,
                                 rbusElementInfo_t *elemInfo);
 ...
 *
 * This method allows a provider to register a row that it adds to its own
 * table. A provider can add a row internally without the need to call
 * rbusTable_addRow which would call the provider's tableAddRow handler.
 * However, in order for consumers to know the row exists, it must be
 * registered. Used by:  Any provider that adds a row to its own table.
 *  @param  handle          Bus Handle
 *  @param  tableName       The name of a table (e.g. "Device.IP.Interface.")
 *  @param  instNum         The unique instance number the provider has assigned
 * this row.
 *  @param  aliasName       An optional name for the new row.  Must be unique in
 * the table.  Can be NULL.
 *  @return RBus error code as defined by rbusError_t.
 ...
 *  @ingroup Tables
 */
rbusError_t rbusTable_registerRow(rbusHandle_t handle, char const *tableName,
                                  uint32_t instNum, char const *aliasName);
 ...
 *
 * The method allows a provider to unregister a row that it removes from its own
 * table. A provider can remove a row internally without the need to call
 * rbusTable_removeRow which would call the provider's tableRemoveRow handler.
 * However, in order for consumer to know the row no longer exists, it must be
 * unregistered. Used by:  Any provider that removes a row from its own table.
 *  @param  handle          Bus Handle
 *  @param  rowName         The name of a table row (e.g.
 * "Device.IP.Interface.1")
 *  @return RBus error code as defined by rbusError_t.
...
rbusError_t rbusTable_unregisterRow(rbusHandle_t handle, char const *rowName);
 ...
 *  @ingroup Events
 */
bool rbusEvent_IsSubscriptionExist(rbusHandle_t handle, char const *eventName,
                                   rbusEventSubscription_t *subscription);
 ...
 * If timeout is positive, internal retries will be attempted if the
 * subscription cannot be routed to an existing provider, and the retries will
 * continue until either a provider is found, an unrecoverable error occurs, or
 * retry timeout reached. A component should call rbusEvent_Unsubscribe to stop
 * receiving the event.
 *  @param      handle          Bus Handle
 *  @param      eventName       The fully qualified name of the event
 *  @param      handler         The event callback handler
 *  @param      userData        User data to be passed back to the callback
 * handler
 *  @param      timeout         Max time in seconds to attempt retrying
 * subscribe
 *  @return RBus error code as defined by rbusError_t.
 ...
 *          void*               userData,
 *          int                 timeout)
 *  @brief Subscribe to a single event using rawdata method, with reduced memory
 * and cpu footprint. Used by: Components that need to subscribe to an event.
 * The handler will be called back when the event is generated.
 * If timeout is positive, internal retries will be attempted if the
 * subscription cannot be routed to an existing provider, and the retries will
 * continue until either a provider is found, an unrecoverable error occurs, or
 * retry timeout reached. A component should call rbusEvent_UnsubscribeRawData
 ...
 *  @ingroup Events
 */
rbusError_t rbusEvent_SubscribeRawData(rbusHandle_t handle,
                                       char const *eventName,
                                       rbusEventHandler_t handler,
 ...
 * subscription request, an unrecoverable error occurs, or retry timeout
 * reached. If timeout is positive, internal retries will be attempted if the
 * subscription cannot be routed to an existing provider, and the retries will
 * continue until either a provider is found, an unrecoverable error occurs, or
 * retry timeout reached. A component should call rbusEvent_Unsubscribe to stop
 * receiving the event.
 ...
 */
rbusError_t
rbusEvent_SubscribeAsync(rbusHandle_t handle, char const *eventName,
                         rbusEventHandler_t handler,
                         rbusSubscribeAsyncRespHandler_t subscribeHandler,
                         void *userData, int timeout);
...
rbusError_t rbusEvent_UnsubscribeRawData(rbusHandle_t handle,
                                         char const *eventName);
 ...
 *          int timeout)
 *  @brief  Subscribe to one or more events with the option to add extra
 * attributes to each subscription through the rbusEventSubscription_t
 * structure\n Used by: Components that need to subscribe to events. For each
 * rbusEventSubscription_t subscription, the eventName and handler are required
 * to be set.  Other options may be set to NULL or 0 if not needed. Subscribing
 * to all items in the subscription array is transactional. That is, all must
 * succeed to subscribe or none will be subscribed. If timeout is positive,
 * internal retries will be attempted if the subscription cannot be routed to an
 * existing provider, and the retries will continue until either a provider is
 * found, an unrecoverable error occurs, or retry timeout reached.
 *  @param      handle            Bus Handle
 ...
 *  @ingroup Events
 */
rbusError_t rbusEvent_SubscribeEx(rbusHandle_t handle,
                                  rbusEventSubscription_t *subscription,
                                  int numSubscriptions, int timeout);
 ...
 *  @brief  Subscribe to one or more events with the option to add extra
 * attributes to each subscription through the rbusEventSubscription_t
 * structure\n Used by: Components that need to subscribe to events using
 * rawdata method, with reduced memory and cpu footprint. For each
 * rbusEventSubscription_t subscription, the eventName and handler are required
 * to be set.  Other options may be set to NULL or 0 if not needed. Subscribing
 * to all items in the subscription array is transactional. That is, all must
 * succeed to subscribe or none will be subscribed. If timeout is positive,
 ...
 *  @ingroup Events
 */
rbusError_t rbusEvent_SubscribeExRawData(rbusHandle_t handle,
                                         rbusEventSubscription_t *subscription,
                                         int numSubscriptions, int timeout);
 ...
 *          rbusSubscribeAsyncRespHandler_t subscribeHandler,
 *          int timeout)
 *  @brief  Subscribe asynchronously to one or more events with the option to
 * add extra attributes to each subscription through the rbusEventSubscription_t
 * structure\n Used by: Components that need to subscribe asynchronously to
 * events. For each rbusEventSubscription_t subscription, the eventName and
 * handler are required to be set.  Other options may be set to NULL or 0 if not
 * needed. Subscribing to all items in the subscription array is transactional.
 * That is, all must succeed to subscribe or none will be subscribed.
 * The subscribeHandler is called after either all subscriptions were
 * successfully made to providers, any one subscriptions received an error
 * response from a provider, an unrecoverable error occurs, or retry timeout
 * reached. If timeout is positive, internal retries will be attempted for each
 * subscription that cannot be routed to an existing provider, and the retries
 * will continue until either a provider is found, an unrecoverable error
 * occurs, or retry timeout reached.
 *  @param      handle            Bus Handle
 *  @param      subscription      The array of subscriptions to register to
 *  @param      numSubscriptions  The number of subscriptions to register to
 *  @param      subscribeHandler  The subscribe callback handler
...
rbusError_t rbusEvent_SubscribeExAsync(
    rbusHandle_t handle, rbusEventSubscription_t *subscription,
    int numSubscriptions, rbusSubscribeAsyncRespHandler_t subscribeHandler,
    int timeout);
 ...
 * For each rbusEventSubscription_t subscription, the eventName is required to
 * be set. Other options may be set NULL or 0. The subscriptions pointer can be
 * the same as that passed to rbusEvent_SubscribeEx, or it can be a different
 * list; however, the eventName for each subsription must be one previously
 * subscribed to with either rbusEvent_Subscribe or rbusEvent_SubscribeEx.
 *  @param      handle          Bus Handle
 ...
 *  @ingroup Events
 */
rbusError_t rbusEvent_UnsubscribeEx(rbusHandle_t handle,
                                    rbusEventSubscription_t *subscriptions,
                                    int numSubscriptions);
 ...
 *          int numSubscriptions)
 *  @brief  Unsubscribe from one or more events\n
 *          Used by: Components that need to unsubscribe from events subscribed
 * with rawdata method. For each rbusEventSubscription_t subscription, the
 * eventName is required to be set. Other options may be set NULL or 0. The
 * subscriptions pointer can be the same as that passed to
 * rbusEvent_SubscribeEx, or it can be a different list; however, the eventName
 * for each subsription must be one previously subscribed to with either
 * rbusEvent_Subscribe or rbusEvent_SubscribeEx.
 *  @param      handle          Bus Handle
 *  @param      subscriptions   The array of subscriptions to unregister from
 *  @param      numSubscriptions The number of subscriptions to unregister from
 *  @return RBus error code as defined by rbusError_t.
 ...
 */
rbusError_t
rbusEvent_UnsubscribeExRawData(rbusHandle_t handle,
                               rbusEventSubscription_t *subscription,
                               int numSubscriptions);
 ...
 *  @ingroup Events
 */
rbusError_t rbusEvent_PublishRawData(rbusHandle_t handle,
 ...
 *                          outparams is generated and contains the return value
 of error rbusError_t On success, it contains method specific data on the
 outParams. On failure, it contains method specific error code and error
 description on the outParams. "error_code" & "error_string" provider is
 responsible in sending the errorcode and error string. other error like no
 method/handling issues, internal err will be taken care by rbus.
 *  @return RBus error code as defined by rbusError_t.
 *  Possible values are: RBUS_ERROR_SUCCESS, RBUS_ERROR_BUS_ERROR,
 ...
 *          int timeout)
 *  @brief Invokes a remote method, non-blocking, with an asynchronous return
 * callback.
 *
 *  Invokes a remote method without blocking.
 * The return params of the method will be received asynchronously by the
 * callback provided. inParams will be retained and used to invoke the method on
 * a background thread; therefore, inParams should not be altered by the calling
 * program until the callback complete.
 *  @param      handle      Bus Handle
 *  @param      methodName  Method name
 *  @param      inParams    Input params
 *  @param      callback    Callback handler for the method's return parameters.
 *  @param      timeout     Optional maximum time in seconds to receive a
 * callback.
 *  @return RBus error code as defined by rbusError_t.
 *  Possible values are: RBUS_ERROR_INVALID_EVENT
 *  @ingroup Methods
 ...
 *
 * Providers can use this method to send the result of an invoke method
 * asynchronously. The asyncHandle is provided when the provider's MethodHandler
 * is called. The outParams param should be initialized and released by the
 * provider.  outParams can be NULL. The error params should be set to
 * RBUS_ERROR_SUCCESS if the method was successful, or to an approprioate error
 * if the method fails.
 ...
 *          uint32_t *pSessionId)
 *
 *  @brief Components use this API to create a new session to set 1 or more
 * parameters and action on table like add/remove rows. \n Used by: Components
 * that needs to Set Param & Update Tables.
 *
 *  @param handle Bus Handle
 ...
 *          uint32_t *pSessionId)
 *
 *  @brief Components use this API to get the current session to set 1 or more
 * parameters and action on table like add/remove rows. \n Used by: Components
 * that needs to Set Param & Update Tables.
 *
 *  @param handle Bus Handle
 ...
 *          uint32_t sessionId)
 *
 *  @brief Components use this API to close the current session that was used to
 * set 1 or more parameters and action on table like add/remove rows. \n Used
 * by: Components that used the session for Set Param & Update Tables.
 *
 *  @param handle Bus Handle
 ...
 *  @brief  A callback handler to get the log messages to application context
 *
 *  Used by: Component that wants to handle the logs in its own way must
 * register a callback handler to get the log messages.
 *
 ...
 *
 *  Used by: Component that wants to use default logging of RBUS instead of
 * overriding the logging, can set the log level by this.
 *
 ...
 *
 *  Used by: Component that wants to use private connection to provider and
 * avoid RBUS Daemon.
 *
 ...
 *  @return RBus error code as defined by rbusError_t.
 */
rbusError_t rbus_openDirect(rbusHandle_t handle, rbusHandle_t *myDirectHandle,
                            char const *parameterName);

/** @fn rbusError_t rbus_closeDirect(
 *          rbusHandle_t myDirectHandle)
 *
 *  @brief  Component use this API to close existing direct connection to the
 * provider for the given DML
 *
 *  Used by: Component that is no more need the direct connection and wants to
 * switch to RBUS Daemon
 *
 *  @param myDirectHandle   Direct handle that is created
 *  @return RBus error code as defined by rbusError_t.
 */
