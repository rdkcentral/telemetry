...
#if defined(CCSP_SUPPORT_ENABLED)
T2ERROR getParamValues(char **paramNames, const int paramNamesCount,
                       parameterValStruct_t ***valStructs, int *valSize) {
  if (paramNames == NULL || paramNamesCount <= 0) {
    EVENT_ERROR("paramNames is NULL or paramNamesCount <= 0 - returning\n");
    return T2ERROR_INVALID_ARGS;
  }

  int ret = CcspBaseIf_getParameterValues(bus_handle, destCompName,
                                          (char *)destCompPath, paramNames,
...
static void rBusInterface_Uninit() { rbus_close(bus_handle); }
...
static T2ERROR initMessageBus() {
  ...
  if (RBUS_ENABLED == rbus_checkStatus()) {
    ...
    rbusError_t status_rbus =
        rbus_open((rbusHandle_t *)&bus_handle, commonLibName);
    if (status_rbus != RBUS_ERROR_SUCCESS) {
      EVENT_ERROR(
          "%s:%d, init using component name %s failed with error code %d \n",
...
int filtered_event_send(const char *data, const char *markerName) {
  ...
  if (isRbusEnabled) {
    ...
    if (componentName &&
        (0 !=
         strcmp(componentName,
                T2_SCRIPT_EVENT_COMPONENT))) { // Events from scripts needs to
                                               // be sent without filtering

      EVENT_DEBUG(
          "%s markerListMutex lock & get list of marker for component %s \n",
    ...
    int eventDataLen =
        strlen(markerName) + strlen(data) + strlen(MESSAGE_DELIMITER) + 1;
    char *buffer = (char *)malloc(eventDataLen * sizeof(char));
    if (buffer) {
      snprintf(buffer, eventDataLen, "%s%s%s", markerName, MESSAGE_DELIMITER,
               data);
...
static T2ERROR doPopulateEventMarkerList() {
  ...
  snprintf(deNameSpace[0], 124, "%s%s%s", T2_ROOT_PARAMETER, componentName,
  ...
  if (ret != RBUS_ERROR_SUCCESS) {
    EVENT_ERROR("rbus mode : No event list configured in profiles %s and "
                "return value %d\n",
                deNameSpace[0], ret);
    pthread_mutex_unlock(&markerListMutex);
    EVENT_DEBUG("rbus mode : No event list configured in profiles %s and "
                "return value %d. Unlock markerListMutex\n",
                deNameSpace[0], ret);
    EVENT_DEBUG("%s --out\n", __FUNCTION__);
    return T2ERROR_SUCCESS;
  }
  ...
  if (objectValue) {
    ...
    EVENT_DEBUG("\t rbus mode :  Update event map for component %s with below "
                "events : \n",
                componentName);
    while (NULL != rbusPropertyList) {
      ...
      if (eventname && strlen(eventname) > 0) {
        EVENT_DEBUG("\t %s\n", eventname);
        hash_map_put(eventMarkerMap, (void *)strdup(eventname),
                     (void *)strdup(eventname), free);
      }
...
static void rbusEventReceiveHandler(rbusHandle_t handle,
 ...
 * */
  ...
  // Always check for t2 is ready to accept events. Shutdown target can bring
  // down t2 process at runtime
  ...
  if (!isT2Ready) {
    if (componentName && (0 != strcmp(componentName, "telemetry_client"))) {
      // From other binary applications in rbus mode if t2 daemon is yet to
      // determine state of component specific config from cloud, enable cache
      if ((t2ReadyStatus & T2_STATE_COMPONENT_READY) == 0) {
        return true;
      } else {
        rbusError_t ret = RBUS_ERROR_SUCCESS;
        doPopulateEventMarkerList();
        ret = rbusEvent_Subscribe(bus_handle, T2_PROFILE_UPDATED_NOTIFY,
                                  rbusEventReceiveHandler, "T2Event", 0);
        if (ret != RBUS_ERROR_SUCCESS) {
          EVENT_ERROR(
...
static int report_or_cache_data(char *telemetry_data, const char *markerName) {
  ...
  if (isCachingRequired()) {
    ...
    int eventDataLen = strlen(markerName) + strlen(telemetry_data) +
                       strlen(MESSAGE_DELIMITER) + 1;
    char *buffer = (char *)malloc(eventDataLen * sizeof(char));
    if (buffer) {
      // Caching format needs to be same for operation between rbus/dbus modes
      // across reboots
      snprintf(buffer, eventDataLen, "%s%s%s", markerName, MESSAGE_DELIMITER,
               telemetry_data);
 ...
 */
void t2_init(char *component) { componentName = strdup(component); }
...
T2ERROR t2_event_s(const char *marker, const char *value) {
  ...
  if (componentName == NULL) {
    EVENT_DEBUG("%s:%d, T2:component with pid = %d is trying to send event %s "
                "with value %s without component name \n",
...
T2ERROR t2_event_f(const char *marker, double value) {
  ...
  if (componentName == NULL) {
    EVENT_DEBUG("%s:%d, T2:component with pid = %d is trying to send event %s "
                "with value %lf without component name \n",
                __func__, __LINE__, (int)getpid(), marker, value);
    return T2ERROR_COMPONENT_NULL;
  }
...
T2ERROR t2_event_d(const char *marker, int value) {
  ...
  if (componentName == NULL) {
    EVENT_DEBUG("%s:%d, T2:component with pid = %d is trying to send event %s "
                "with value %d without component name \n",
                __func__, __LINE__, (int)getpid(), marker, value);
    return T2ERROR_COMPONENT_NULL;
  }
  ...
  if (value ==
      0) { // Requirement from field triage to ignore reporting 0 values
    pthread_mutex_unlock(&dMutex);
