...
extern sigset_t blocking_signal;
    ...
    defined(FEATURE_RDKB_CONFIGURABLE_WAN_INTERFACE)
...
static char *getTimezone() {
  ...
  while (zoneValue == NULL) {
    ...
    count++;
    if (count == 10) {
      T2Debug("Timezone retry count reached the limit . Timezone data source "
              "is missing\n");
      break;
    }
  }
  if (zoneValue == NULL) {
    T2Debug("Timezone value from %s is empty, Reading from  "
            "/opt/persistent/timeZoneDST file...\n",
            jsonpath);
...
T2ERROR appendRequestParams(char *buf, const int maxArgLen) {
  ...
  if (T2ERROR_SUCCESS ==
      getParameterValue(TR181_DEVICE_FW_VERSION, &paramVal)) {
    memset(tempBuf, 0, MAX_URL_ARG_LEN);
    write_size =
...
#endif
  if (T2ERROR_SUCCESS ==
      getParameterValue(TR181_DEVICE_PARTNER_ID, &paramVal)) {
    memset(tempBuf, 0, MAX_URL_ARG_LEN);
  ...
  if (T2ERROR_SUCCESS ==
      getParameterValue(TR181_DEVICE_ACCOUNT_ID, &paramVal)) {
    memset(tempBuf, 0, MAX_URL_ARG_LEN);
  ...
  // TODO Check relevance of this existing hardcoded data - can be removed if
  // not used in production
  strncat(buf, "controllerId=2504&channelMapId=2345&vodId=15660&", avaBufSize);
  ...
  T2Debug("%s:%d Final http get URL when privacymode is enabled of size %d is "
          ": \n %s \n",
          __func__, __LINE__, avaBufSize, buf);
#endif
error:
  if (NULL != tempBuf) {
    free(tempBuf);
...
static size_t httpGetCallBack(void *response, size_t len, size_t nmemb,
  ...
  char *ptr =
      (char *)realloc(httpResponse->data, httpResponse->size + realsize + 1);
  if (!ptr) {
...
T2ERROR doHttpGet(char *httpsUrl, char **data) {
   ...
   * This cleanup is not thread safe and classified as run once per application
   * life cycle. Forking the libcurl calls so that it executes and terminates to
   * release memory per execution.
   */
  if (childPid == 0) {

    T2ERROR ret = T2ERROR_FAILURE;
    curlResponseData *httpResponse =
        (curlResponseData *)malloc(sizeof(curlResponseData));
    httpResponse->data = (char *)malloc(1);
    httpResponse->data[0] =
        '\0'; // CID 282084 : Uninitialized scalar variable (UNINIT)
    httpResponse->size = 0;
    ...
    defined(FEATURE_RDKB_CONFIGURABLE_WAN_INTERFACE)
      ...
      if (mtls_enable == true) {
        ...
        do {
          ...
          pCertURI = NULL;
          xcGetCertStatus =
              rdkcertselector_getCert(xcCertSelector, &pCertURI, &pPasswd);
        ...
        } while (rdkcertselector_setCurlStatus(xcCertSelector, curl_code,
                                               (const char *)httpsUrl) ==
                 TRY_ANOTHER);
#else
        } else {
      ...
      if (http_code == 200 && curl_code == CURLE_OK) {
        ...
        if (NULL != pPasswd) {
          ...
          if (rdkconfig_free((unsigned char **)&pPasswd, sPasswdSize) ==
              RDKCONFIG_FAIL) {
            return T2ERROR_FAILURE;
          }
#else
        ...
        T2Error("%s:%d, T2:Telemetry XCONF communication Failed with http code "
                ": %ld Curl code : %d \n",
                __func__, __LINE__, http_code, curl_code);
        T2Error(
            "%s : curl_easy_perform failed with error message %s from curl \n",
...
T2ERROR fetchRemoteConfiguration(char *configURL, char **configData) {
  ...
  if (NULL != urlWithParams) {
    ...
    if (T2ERROR_SUCCESS ==
        appendRequestParams(urlWithParams, availableBufSize)) {
...
T2ERROR getRemoteConfigURL(char **configURL) {
   ...
   * PAM not ready is a definite case for caching the event and avoid bus
   * traffic
   * */
#if defined(ENABLE_RDKB_SUPPORT)
  ...
  if (T2ERROR_SUCCESS == getParameterValue(TR181_CONFIG_URL, &paramVal)) {
    if (NULL != paramVal) {
      if ((strlen(paramVal) > 8) &&
          (0 == strncmp(paramVal, "https://",
                        8))) { // Enforcing https for new endpoints
        T2Info("Setting config URL base location to : %s\n", paramVal);
        *configURL = paramVal;
        ret = T2ERROR_SUCCESS;
      } else {
        T2Error("URL doesn't start with https or is invalid !!! URL value "
                "received : %s .\n",
                paramVal);
        free(paramVal);
      }
...
static void *getUpdatedConfigurationThread(void *data) {
  ...
  do {
    ...
    while (!stopFetchRemoteConfiguration &&
           T2ERROR_SUCCESS != getRemoteConfigURL(&configURL)) {
    ...
    while (!stopFetchRemoteConfiguration) {
      ...
      if (ret == T2ERROR_SUCCESS) {
        ...
        if (T2ERROR_SUCCESS ==
            processConfigurationXConf(configData, &profile)) {
          clearPersistenceFolder(XCONFPROFILE_PERSISTENCE_PATH);
          if (T2ERROR_SUCCESS !=
              saveConfigToFile(XCONFPROFILE_PERSISTENCE_PATH, XCONF_CONFIG_FILE,
                               configData)) // Should be removed once XCONF
                                            // sends new UUID for each update.
          {
          ...
          // Touch a file to indicate script based supplementary services to
        ...
        T2Warning("XConf Telemetry profile not set for this device, "
                  "uninitProfileList.\n");
        if (configData != NULL) {
          free(configData);
          configData = NULL;
        ...
        if (xConfRetryCount >= MAX_XCONF_RETRY_COUNT) {
          T2Error("Reached max xconf retry counts : %d, Using saved profile if "
                  "exists until next reboot\n",
                  MAX_XCONF_RETRY_COUNT);
          xConfRetryCount = 0;
          break;
        }
        T2Info("Waiting for %d sec before trying fetchRemoteConfiguration, "
               "No.of tries : %d\n",
        ...
        if (n == ETIMEDOUT) {
          ...
          T2Error("ERROR inside startXConfClientThread for timedwait, error "
                  "code : %d\n",
                  n);
        }
        pthread_mutex_unlock(&xcMutex);
    ...
    if (configFetch == T2ERROR_FAILURE && !ProfileXConf_isSet()) {
      T2Error(
          "Failed to fetch updated configuration and no saved configurations "
          "on disk for XCONF, uninitializing  the process\n");
    }
  ...
  // pthread_detach(pthread_self()); commenting this line as thread will
  // detached by stopXConfClient
  T2Debug("%s --out\n", __FUNCTION__);
  return NULL;
}
...
T2ERROR initXConfClient() {
  ...
  // startXConfClient(); // Removing startXConfClient as
  // getUpdatedConfigurationThread is created in this function itself
  T2Debug("%s --out\n", __FUNCTION__);
...
T2ERROR startXConfClient() {
  ...
  if (isXconfInit) {
    ...
    T2Info("getUpdatedConfigurationThread is still active ... Ignore xconf "
           "reload \n");
  }
  T2Debug("%s --out\n", __FUNCTION__);
  return T2ERROR_SUCCESS;
