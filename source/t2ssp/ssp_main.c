...
int cmd_dispatch(int command) {
  ...
  _ansc_snprintf(CName, sizeof(CName), "%s%s", g_Subsystem,
                 gpT2StartCfg->ComponentId);

  ssp_T2Mbi_MessageBusEngage(CName, CCSP_MSG_BUS_CFG, gpT2StartCfg->DbusPath);
...
char *getComponentId() {
  ...
  if (gpT2StartCfg == NULL) {
    gpT2StartCfg =
        (PCCSP_COMPONENT_CFG)AnscAllocateMemory(sizeof(CCSP_COMPONENT_CFG));
    if (gpT2StartCfg) {
...
int initTR181_dm() {
  ...
  if (gpT2StartCfg == NULL) {
    ...
    if (gpT2StartCfg) {
      ...
      CcspTraceError(("RDKB_SYSTEM_BOOT_UP_LOG : Telemetry 2.0, Insufficient "
                      "resources for start configuration, quit!\n"));
  ...
  if (err != CCSP_SUCCESS) {
    ...
    CcspTraceError(
        ("RDKB_SYSTEM_BOOT_UP_LOG : Error in Cdm_Init in telemetry \n"));
    return -1;
  }
