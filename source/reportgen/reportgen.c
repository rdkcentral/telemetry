...
static bool checkForEmptyString(char *valueString) {
  ...
  if (valueString) {
    // rbusInterface implementation explicitly adds string as "NULL" for
    // parameters which doesn't exist on device Consider "NULL" string value as
    // qualified for empty string
    if (strlen(valueString) < 1 || !strncmp(valueString, " ", 1) ||
        !strncmp(valueString, "NULL", 4)) {
      isEmpty = true;
    }
  } else {
    isEmpty = true;
...
void trimLeadingAndTrailingws(char *string) {
  ...
  for (i = 0; string[i] == ' ' || string[i] == '\t'; i++)
    ; // To find first non-space character in the string
  ...
  for (i = 0; string[i] != '\0'; i++) {
    if (string[i] != ' ' &&
        string[i] != '\t') { // To remove the trailing whitespaces
...
T2ERROR encodeParamResultInJSON(cJSON *valArray, Vector *paramNameList,
  ...
  for (; index < Vector_Size(paramNameList); index++) {
    ...
    tr181ValStruct_t **paramValues =
        ((profileValues *)Vector_At(paramValueList, index))->paramValues;
    if (param == NULL || paramValues == NULL) {
      // Ignore tr181 params returning null values in report
      continue;
    }
    int paramValCount =
        ((profileValues *)Vector_At(paramValueList, index))->paramValueCount;
    ...
    } else if (paramValCount == 1) // Single value
    {
      if (paramValues[0]) {
        if (param->reportEmptyParam ||
            !checkForEmptyString(paramValues[0]->parameterValue)) {
          cJSON *arrayItem = cJSON_CreateObject();
          if (arrayItem == NULL) {
            T2Error("cJSON_CreateObject failed.. arrayItem is NULL \n");
            return T2ERROR_FAILURE;
          ...
          if (param->regexParam != NULL) {
            ...
            if (rc != 0) {
              ...
              rc = regexec(&regpattern, paramValues[0]->parameterValue, nmatch,
                           pmatch, 0);
              if (rc != 0) {
                T2Warning("regexec() failed, Failed to match '%s' with "
                          "'%s',returning %d.\n",
                          paramValues[0]->parameterValue, param->regexParam,
                          rc);
                ...
                T2Debug("regexec successful, Match is found %.*s\n",
                        ...
                        &paramValues[0]->parameterValue[pmatch[0].rm_so]);
          ...
          if (cJSON_AddStringToObject(arrayItem, param->name,
                                      paramValues[0]->parameterValue) == NULL) {
            T2Error("cJSON_AddStringToObject failed.\n");
            cJSON_Delete(arrayItem);
            return T2ERROR_FAILURE;
          }
      ...
      cJSON_AddItemToObject(arrayItem, param->name,
                            valList = cJSON_CreateArray());
          ...
          if (param->reportEmptyParam ||
            ...
            if (param->regexParam != NULL) {
              ...
              if (rc != 0) {
                ...
                T2Debug("regcomp() successful, returning value (%d)\n", rc);
                rc = regexec(&regpattern, paramValues[valIndex]->parameterValue,
                             nmatch, pmatch, 0);
                if (rc != 0) {
                  T2Warning("regexec() failed, Failed to match '%s' with "
                            "'%s',returning %d.\n",
                            paramValues[valIndex]->parameterValue,
                            param->regexParam, rc);
                  free(paramValues[valIndex]->parameterValue);
                  paramValues[valIndex]->parameterValue = strdup("");
                } else {
                  T2Debug(
                      "regexec successful, Match is found %.*s\n",
                      pmatch[0].rm_eo - pmatch[0].rm_so,
                      &paramValues[valIndex]->parameterValue[pmatch[0].rm_so]);
                  sprintf(
                      string, "%.*s", pmatch[0].rm_eo - pmatch[0].rm_so,
                      &paramValues[valIndex]->parameterValue[pmatch[0].rm_so]);
                  free(paramValues[valIndex]->parameterValue);
            ...
            if (cJSON_AddStringToObject(
                    valItem, paramValues[valIndex]->parameterName,
                    paramValues[valIndex]->parameterValue) == NULL) {
              T2Error("cJSON_AddStringToObject failed\n");
              cJSON_Delete(arrayItem);
...
T2ERROR encodeStaticParamsInJSON(cJSON *valArray, Vector *staticParamList) {
  ...
  for (; index < Vector_Size(staticParamList); index++) {
    ...
    if (sparam) {
      ...
      if (cJSON_AddStringToObject(arrayItem, sparam->name, sparam->value) ==
          NULL) {
        T2Error("cJSON_AddStringToObject failed.\n");
        cJSON_Delete(arrayItem);
        return T2ERROR_FAILURE;
...
T2ERROR encodeGrepResultInJSON(cJSON *valArray, Vector *grepResult) {
  ...
  for (; index < Vector_Size(grepResult); index++) {
    ...
    if (grep) {
      if (grep->markerName == NULL ||
          grep->markerValue == NULL) // Ignore null values
        continue;
      arrayItem = cJSON_CreateObject();
      if (arrayItem == NULL) {
        T2Error("cJSON_CreateObject failed..arrayItem is NULL \n");
      ...
      if (grep->regexParameter != NULL) {
        ...
        if (rc != 0) {
          ...
          if (rc != 0) {
            ...
            T2Debug("regexec successful, Match is found %.*s\n",
                    pmatch[0].rm_eo - pmatch[0].rm_so,
                    &grep->markerValue[pmatch[0].rm_so]);
            sprintf(string, "%.*s", pmatch[0].rm_eo - pmatch[0].rm_so,
                    &grep->markerValue[pmatch[0].rm_so]);
            free((char *)grep->markerValue);
      ...
      if (cJSON_AddStringToObject(arrayItem, grep->markerName,
                                  grep->markerValue) == NULL) {
        T2Error("cJSON_AddStringToObject failed.\n");
        cJSON_Delete(arrayItem);
        return T2ERROR_FAILURE;
      }
...
T2ERROR encodeEventMarkersInJSON(cJSON *valArray, Vector *eventMarkerList) {
  ...
  for (; index < Vector_Size(eventMarkerList); index++) {
    ...
    case MTYPE_COUNTER:
      if (eventMarker->u.count > 0) {
        ...
        if (eventMarker->alias) {
          if (cJSON_AddStringToObject(arrayItem, eventMarker->alias,
                                      stringValue) == NULL) {
            T2Error("cJSON_AddStringToObject failed\n");
            cJSON_Delete(arrayItem);
            return T2ERROR_FAILURE;
          ...
          if (cJSON_AddStringToObject(arrayItem, eventMarker->markerName,
                                      stringValue) == NULL) {
            T2Error("cJSON_AddStringToObject failed\n");
            cJSON_Delete(arrayItem);
            return T2ERROR_FAILURE;
        ...
        if (eventMarker->reportTimestampParam == REPORTTIMESTAMP_UNIXEPOCH) {
          if (cJSON_AddStringToObject(arrayItem, eventMarker->markerName_CT,
                                      eventMarker->timestamp) == NULL) {
            T2Error("cJSON_AddStringToObject failed\n");
            cJSON_Delete(arrayItem);
            return T2ERROR_FAILURE;
          }
    ...
    case MTYPE_ACCUMULATE:
      if (eventMarker->u.accumulatedValues != NULL &&
          Vector_Size(eventMarker->u.accumulatedValues)) {
        arrayItem = cJSON_CreateObject();
        if (arrayItem == NULL) {
          T2Error("cJSON_CreateObject failed .. arrayItem is NULL\n");
          return T2ERROR_FAILURE;
        ...
        if (eventMarker->trimParam) {
          ...
          for (i = 0; i < Vector_Size(eventMarker->u.accumulatedValues); i++) {
            char *stringValue =
                (char *)Vector_At(eventMarker->u.accumulatedValues, i);
            trimLeadingAndTrailingws(stringValue);
          }
        }
        ...
        if (eventMarker->regexParam != NULL) {
          ...
          if (rc != 0) {
            ...
            for (size_t i = 0;
                 i < Vector_Size(eventMarker->u.accumulatedValues); i++) {
              char *stringValue =
                  (char *)Vector_At(eventMarker->u.accumulatedValues, i);
              rc = regexec(&regpattern, stringValue, nmatch, pmatch, 0);
              if (strcmp(stringValue, "maximum accumulation reached") == 0) {
                ...
                T2Debug("regexec successful, Match is found %.*s\n",
                        pmatch[0].rm_eo - pmatch[0].rm_so,
                        &stringValue[pmatch[0].rm_so]);
                sprintf(string[i], "%.*s", pmatch[0].rm_eo - pmatch[0].rm_so,
                        &stringValue[pmatch[0].rm_so]);
              }
        ...
        if (regaccumulateValues != NULL &&
            Vector_Size(regaccumulateValues) > 0) {
        ...
        if (eventMarker->alias) {
          ...
          cJSON_AddItemToObject(arrayItem, eventMarker->markerName,
                                vectorToarray);
        }
        if (eventMarker->reportTimestampParam == REPORTTIMESTAMP_UNIXEPOCH) {
          ...
          convertVectorToJson(TimevectorToarray,
                              eventMarker->accumulatedTimestamp);
          T2Debug("convertVectorToJson is successful\n");
          Vector_Clear(eventMarker->accumulatedTimestamp, freeAccumulatedParam);
          cJSON_AddItemToObject(arrayItem, eventMarker->markerName_CT,
                                TimevectorToarray);
        }
        cJSON_AddItemToArray(valArray, arrayItem);
      ...
      if (eventMarker->u.markerValue != NULL) {
        ...
        if (eventMarker->regexParam != NULL) {
          ...
          if (rc != 0) {
            ...
            T2Debug("regcomp() successful, returning value (%d)\n", rc);
            rc = regexec(&regpattern, eventMarker->u.markerValue, nmatch,
                         pmatch, 0);
            if (rc != 0) {
              T2Warning("regexec() failed, Failed to match '%s' with "
                        "'%s',returning %d.\n",
                        eventMarker->u.markerValue, eventMarker->regexParam,
                        rc);
              ...
              T2Debug("regexec successful, Match is found %.*s\n",
                      pmatch[0].rm_eo - pmatch[0].rm_so,
                      &eventMarker->u.markerValue[pmatch[0].rm_so]);
              sprintf(string, "%.*s", pmatch[0].rm_eo - pmatch[0].rm_so,
                      &eventMarker->u.markerValue[pmatch[0].rm_so]);
              free(eventMarker->u.markerValue);
...
char *prepareHttpUrl(T2HTTP *http) {
  ...
  if (http->RequestURIparamList && http->RequestURIparamList->count) {
    ...
    for (; index < http->RequestURIparamList->count; index++) {
      HTTPReqParam *httpParam =
          (HTTPReqParam *)(Vector_At)(http->RequestURIparamList, index);
      ...
      int new_params_len =
          params_len /* current params length */
          + strlen(httpParam->HttpName) /* Length of parameter 'Name' */ +
          strlen("=") + strlen("&") /* Add '&' for next paramter */ + 1;
      if (httpParamVal) {
      ...
      params_len +=
          snprintf(url_params + params_len, new_params_len - params_len,
                   "%s=%s&", httpParam->HttpName, httpParamVal);
    ...
    if (params_len > 0 && url_params != NULL) {
      ...
      snprintf(httpUrl + url_len, modified_url_len - url_len, "?%s",
               url_params);
    }
    if (url_params) {
      free(url_params);
