...
static pthread_mutex_t
    plMutex; /* TODO - we can remove plMutex most likely but firseck that
                CollectAndReport doesn't cause issue */
...
static void freeProfileXConf() {
  if (singleProfile != NULL) {
    ...
    // Data elements from this list is copied in new profile. So do not destroy
    // the vector
...
static T2ERROR initJSONReportXconf(cJSON **jsonObj, cJSON **valArray) {
  *jsonObj = cJSON_CreateObject();
  ...
  cJSON_AddItemToObject(*jsonObj, "searchResult",
                        *valArray = cJSON_CreateArray());
  ...
  arrayItem = cJSON_CreateObject();
  // For Comcast platforms, requirement from field triage to be a fixed string
...
static void *CollectAndReportXconf(void *data) {
  ...
  do {
    ...
    if (profile->encodingType != NULL &&
        !strcmp(profile->encodingType, "JSON")) {
      if (T2ERROR_SUCCESS !=
          initJSONReportXconf(&profile->jsonReportObj, &valArray)) {
      ...
      if (profile->paramList != NULL && Vector_Size(profile->paramList) > 0) {
        ...
        T2Info("Fetch complete for TR-181 Object/Parameter Values for "
               "parameters \n");
      ...
      if (profile->gMarkerList != NULL &&
        ...
        getGrepResults(profile->name, profile->gMarkerList, &grepResultList,
                       ...
                       true); // Passing 5th argument as true to check rotated
                              // logs only in case of single profile
      ...
      if (profile->isUpdated) {
        T2Info("Profile is udpated, report is cached to send with updated "
               "Profile TIMEOUT\n");
        ...
        if (profile->cachedReportList != NULL &&
          ...
          T2Info("Max Cached Reports Limit Reached, Overwriting third recent "
                 "report\n");
          char *thirdCachedReport = (char *)Vector_At(profile->cachedReportList,
                                                      MAX_CACHED_REPORTS - 3);
          Vector_RemoveItem(profile->cachedReportList,
      ...
      if (profile->protocol != NULL && strcmp(profile->protocol, "HTTP") == 0) {
        ...
        if (ret == T2ERROR_FAILURE) {
          ...
          T2Info("Trying to send  %lu cached reports\n",
                 (unsigned long)Vector_Size(profile->cachedReportList));
          ret = sendCachedReportsOverHTTP(profile->t2HTTPDest->URL,
                                          profile->cachedReportList);
          if (ret == T2ERROR_SUCCESS) {
            // Do not get misleaded by function name. Call is to delete the
            // directory for storing cached reports
...
T2ERROR ProfileXConf_init() {
  ...
  if (!initialized) {
    ...
    if (Vector_Size(configList) >
        1) // This is to address corner cases where if multiple configs are
           // saved, we ignore them and wait for new config.
    {
      T2Info("Found multiple saved profiles, removing all from the disk and "
             "waiting for updated configuration\n");
      ...
      if (T2ERROR_SUCCESS ==
          processConfigurationXConf(config->configData, &profile)) {
  ...
  if (!singleProfile) {
    ...
    for (; emIndex < Vector_Size(singleProfile->eMarkerList); emIndex++) {
      ...
      addT2EventMarker(eMarker->markerName, eMarker->compName,
                       singleProfile->name, eMarker->skipFreq);
    }
    if (registerProfileWithScheduler(
            singleProfile->name, singleProfile->reportingInterval,
            INFINITE_TIMEOUT, false, true, false, DEFAULT_FIRST_REPORT_INT,
            NULL) == T2ERROR_SUCCESS) {
...
T2ERROR ProfileXConf_delete(ProfileXConf *profile) {
  ...
  if (isNameEqual) {
    ...
    if (count > 0 && profile->cachedReportList != NULL) {
      ...
      size_t index = 0;
      while (index < count) {
        Vector_PushBack(profile->cachedReportList,
                        (void *)Vector_At(singleProfile->cachedReportList, 0));
        Vector_RemoveItem(singleProfile->cachedReportList,
                          (void *)Vector_At(singleProfile->cachedReportList, 0),
                          NULL); /*TODO why this instead of Vector_destroy*/
        index++;
      }
    }
  } else {
    if (count > 0) { // Destroy the cachedReportList vector when the profile
                     // name is not equal
  ...
  if ((Vector_Size(singleProfile->eMarkerList) > 0) &&
      (Vector_Size(profile->eMarkerList) > 0)) {
    size_t i, j;
    T2Info("check the events from the old profile and forward to the new "
           "profile \n");
    for (j = 0; j < Vector_Size(profile->eMarkerList); j++) {
      ...
      T2Debug("Check the New Event : %s index : %zu \n", eMarkerNew->markerName,
              j);
      for (i = 0; i < Vector_Size(singleProfile->eMarkerList); i++) {
        EventMarker *eMarkerCurrent = NULL;
        eMarkerCurrent =
            (EventMarker *)Vector_At(singleProfile->eMarkerList, i);
        T2Debug("Check the Old Event : %s index : %zu \n",
                eMarkerCurrent->markerName, i);
        if ((strcmp(eMarkerNew->markerName, eMarkerCurrent->markerName) == 0) &&
            eMarkerNew->mType == eMarkerCurrent->mType) {
          T2Debug("Event marker with name : %s type %d is in both profiles "
                  "copy the value \n",
          ...
          case MTYPE_XCONF_ACCUMULATE:
            ...
            if (eMarkerCurrent->u.accumulatedValues != NULL && count > 0) {
              ...
              while (index < count) {
                Vector_PushBack(
                    eMarkerNew->u.accumulatedValues,
                    (void *)Vector_At(eMarkerCurrent->u.accumulatedValues, 0));
                Vector_RemoveItem(
                    eMarkerCurrent->u.accumulatedValues,
                    (void *)Vector_At(eMarkerCurrent->u.accumulatedValues, 0),
                    NULL);
                index++;
              }
          ...
          Vector_RemoveItem(singleProfile->eMarkerList, (void *)eMarkerCurrent,
                            freeEMarker);
          break;
        }
      }
...
void ProfileXConf_notifyTimeout(bool isClearSeekMap, bool isOnDemand) {
  ...
  if (!singleProfile->reportInProgress) {
    ...
    if (reportThreadExits)
      ...
      reportThreadStatus = pthread_create(&singleProfile->reportThread, NULL,
                                          CollectAndReportXconf, NULL);
      if (reportThreadStatus != 0)
        T2Error("Failed to create report thread with error code = %d !!! \n",
                reportThreadStatus);
    }
  } else
    T2Warning("Received profileTimeoutCb while previous callback is still in "
...
T2ERROR ProfileXConf_storeMarkerEvent(T2Event *eventInfo) {
  ...
  for (; eventIndex < Vector_Size(singleProfile->eMarkerList); eventIndex++) {
    EventMarker *tempEventMarker =
        (EventMarker *)Vector_At(singleProfile->eMarkerList, eventIndex);
...
T2ERROR ProfileXConf_terminateReport() {
  ...
  if (singleProfile->reportInProgress) {
    ...
    if ((xconfReportPid > 0) && !kill(xconfReportPid, 0)) {
      T2Info("Report upload in progress, terminating the forked reporting "
             "child : %d \n",
             xconfReportPid);
    ...
    T2Info("No report generation in progress. No further action required for "
           "abort.\n");
  }
