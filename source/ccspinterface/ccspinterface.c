...
static T2ERROR ccspGetParameterValues(const char **paramNames,
                                      const int paramNamesCount,
                                      parameterValStruct_t ***valStructs,
                                      int *valSize);
...
void freeParamInfoSt(parameterInfoStruct_t **paramNamesSt,
                     int paramNamesLength);
...
static T2ERROR CCSPInterface_Init() {
  ...
  int ret = CCSP_Message_Bus_Init(
      componentId, pCfg, &bus_handle,
...
static int findDestComponent(char *paramName, char **destCompName,
                             char **destPath) {
  int ret, size = 0;
  char dst_pathname_cr[256] = {0};
  componentStruct_t **ppComponents = NULL;
  T2Debug("%s ++in for paramName : %s\n", __FUNCTION__, paramName);
  snprintf(dst_pathname_cr, sizeof(dst_pathname_cr), "eRT.%s",
           CCSP_DBUS_INTERFACE_CR);
  ret = CcspBaseIf_discComponentSupportingNamespace(
      bus_handle, dst_pathname_cr, paramName, "", &ppComponents, &size);
...
bool isCCSPInitialized() { return bus_handle != NULL ? true : false; }

T2ERROR ccspGetParameterValues(const char **paramNames,
                               const int paramNamesCount,
                               parameterValStruct_t ***valStructs,
                               int *valSize) {
  ...
  *valSize = 0;
  ...
  if (CCSP_SUCCESS ==
      findDestComponent((char *)paramNames[0], &destCompName, &destCompPath)) {
    T2Debug("Calling CcspBaseIf_getParameterValues for : %s, paramCount : %d "
            "Destination name : %s and path %s\n",
            paramNames[0], paramNamesCount, destCompName, destCompPath);
    int ret = CcspBaseIf_getParameterValues(
        bus_handle, destCompName, destCompPath, (char **)paramNames,
...
void freeParamInfoSt(parameterInfoStruct_t **paramInfoSt,
                     int paramNamesLength) {
  free_parameterInfoStruct_t(bus_handle, paramNamesLength, paramInfoSt);
}

static void freeCCSPParamValueSt(parameterValStruct_t **valStructs,
                                 int valSize) {
...
T2ERROR getCCSPParamVal(const char *paramName, char **paramValue) {
  ...
  if (T2ERROR_SUCCESS != ccspGetParameterValues((const char **)paramNames, 1,
                                                &valStructs, &valSize)) {
        ...
        for (iterate = 0; iterate < paramValCount; ++iterate) {
          if (ccspParamValues[iterate]) {
            ...
            if (paramValues[iterate]) {
              paramValues[iterate]->parameterName =
                  strdup((ccspParamValues[iterate])->parameterName);
              paramValues[iterate]->parameterValue =
                  strdup((ccspParamValues[iterate])->parameterValue);
            }
          }
        }
...
T2ERROR registerCcspT2EventListener(TelemetryEventCallback eventCB) {
  ...
  CcspBaseIf_SetCallback2(bus_handle, "telemetryDataSignal", eventCB, NULL);
  ...
  if (ret != CCSP_Message_Bus_OK) {
    ...
    T2Info("Registration with CCSP Bus successful, waiting for Telemetry "
...
T2ERROR unregisterCcspT2EventListener() { return T2ERROR_SUCCESS; }
