...
bool isRbusInitialized() { return t2bus_handle != NULL ? true : false; }

void logHandler(rbusLogLevel level, const char *file, int line, int threadId,
                char *message) {
  (void)threadId; // To avoid compiler warnings, we cant remove this argument as
                  // this is requirment from rbus
  switch (level) {
  case RBUS_LOG_FATAL:
    T2Error("%s:%d %s\n", file, line, message);
    break;
...
static void rBusInterface_Uninit() { rbus_close(t2bus_handle); }
...
T2ERROR getRbusParameterVal(const char *paramName, char **paramValue) {
  ...
  // Workaround as video platforms doesn't have a TR param which gives Firmware
  // name Existing dashboards doesn't like version with file name exentsion
  // Workaround stays until initiative for unified new TR param for version name
  // gets implemented across board
  if (0 == strncmp(paramName,
...
Vector *getRbusProfileParamValues(Vector *paramList) {
  ...
  for (; i < paramList->count; i++) { // Loop through paramlist from profile
    ...
    if (paramNames[0] != NULL) {
      ...
      if (RBUS_ERROR_SUCCESS !=
          rbus_getExt(t2bus_handle, 1, (const char **)paramNames,
                      &paramValCount, &rbusPropertyValues)) {
    ...
    if (paramValCount == 0) {
      ...
      if (paramValues != NULL) {
        ...
        // If parameter doesn't exist in device we do populate with entry as
        // NULL. So count of populated data list has 1 entry and is not 0
      ...
      paramValues = (tr181ValStruct_t **)malloc(paramValCount *
                                                sizeof(tr181ValStruct_t *));
      if (paramValues != NULL) {
        rbusProperty_t nextProperty = rbusPropertyValues;
        for (iterate = 0; iterate < paramValCount;
             ++iterate) { // Loop through values obtained from query for
                          // individual param in list
          if (nextProperty) {
            char *stringValue = NULL;
            rbusValue_t value = rbusProperty_GetValue(nextProperty);
            paramValues[iterate] =
                (tr181ValStruct_t *)malloc(sizeof(tr181ValStruct_t));
            if (paramValues[iterate]) {
              ...
              if (rbusValueType == RBUS_BOOLEAN) {
                ...
                paramValues[iterate]->parameterValue =
                    rbusValue_ToString(value, NULL, 0);
              }

#if defined(ENABLE_RDKV_SUPPORT)
              // Workaround as video platforms doesn't have a TR param which
              // gives Firmware name Existing dashboards doesn't like version
              // with file name exentsion Workaround stays until initiative for
              // unified new TR param for version name gets implemented across
              // board
              if (0 ==
                      strncmp(
                          stringValue,
                ...
                char *temp = NULL;
                temp =
                    strstr(paramValues[iterate]->parameterValue, "-signed.bin");
                if (!temp) {
...
rbusError_t eventSubHandler(rbusHandle_t handle, rbusEventSubAction_t action,
                            const char *eventName, rbusFilter_t *filter,
                            int interval, bool *autoPublish) {
  (void)handle;
  (void)filter;
  (void)interval;
  (void)autoPublish;
 ...
 */
rbusError_t t2PropertyDataSetHandler(rbusHandle_t handle, rbusProperty_t prop,
  ...
  if ((strncmp(paramName, T2_EVENT_PARAM, maxParamLen) != 0) &&
      (strncmp(paramName, T2_REPORT_PROFILE_PARAM, maxParamLen) != 0) &&
      (strncmp(paramName, T2_REPORT_PROFILE_PARAM_MSG_PCK, maxParamLen) != 0) &&
      (strncmp(paramName, T2_TEMP_REPORT_PROFILE_PARAM, maxParamLen) != 0) &&
      (strncmp(paramName, T2_TOTAL_MEM_USAGE, maxParamLen) != 0) &&
      (strncmp(paramName, PRIVACYMODES_RFC, maxParamLen) != 0)) {
      ...
      if (eventName) {
        ...
        if (type_t == RBUS_STRING) {
          ...
          if (eventValue) {
            ...
            eventCallBack((char *)strdup(eventName),
                          (char *)strdup(eventValue));
            free(eventValue);
          }
        } else {
  ...
  } else if (strncmp(paramName, T2_REPORT_PROFILE_PARAM_MSG_PCK, maxParamLen) ==
             0) {
    ...
    if (type_t == RBUS_STRING) {
      ...
      if (data) {
        ...
        if (NULL == webConfigString || 0 == decodedDataLen) {
          T2Error("Invalid base64 input string. Ignore processing input "
                  "configuration.\n");
        ...
        if (T2ERROR_SUCCESS != dmMsgPckProcessingCallBack(
                                   (char *)webConfigString, decodedDataLen)) {
          free(data);
  ...
  } else if (strncmp(paramName, T2_TEMP_REPORT_PROFILE_PARAM, maxParamLen) ==
             0) {
 ...
 */
rbusError_t t2PropertyDataGetHandler(rbusHandle_t handle,
  ...
  if (strncmp(propertyName, T2_REPORT_PROFILE_PARAM, maxParamLen) == 0) {
  ...
  } else if (strncmp(propertyName, T2_REPORT_PROFILE_PARAM_MSG_PCK,
                     maxParamLen) == 0) {
    rbusValue_t value;
    rbusValue_Init(&value);
    if (reportProfilemsgPckVal)
  ...
  } else if (strncmp(propertyName, T2_TEMP_REPORT_PROFILE_PARAM, maxParamLen) ==
             0) {
    rbusValue_t value;
    rbusValue_Init(&value);
    if (tmpReportProfileVal)
    ...
    int i = 0;
    if (length == 0) { // rbus doesn't like NULL objects and seems to crash.
                       // Setting empty values instead
...
void publishReportUploadStatus(char *status) {
  ...
  err = rbusMethod_SendAsyncResponse(onDemandReportCallBackHandler,
                                     RBUS_ERROR_SUCCESS, outParams);
...
static rbusError_t
dcmOnDemandMethodHandler(rbusHandle_t handle, char const *methodName,
  ...
  if (!strncmp(methodName, T2_ON_DEMAND_REPORT, maxParamLen)) {
    if (!pthread_mutex_trylock(&asyncMutex)) {
      ...
      // Calls within reportOnDemandCallBack are thread safe, thread
      // synchronization is not required
      if (pthread_create(&rpOnDemandTh, &rpOnDemandAttr, reportOnDemandCallBack,
                         ON_DEMAND_ACTION_UPLOAD) != 0) {
        T2Error("Failed to create thread for report on demand.\n");
      }
      pthread_attr_destroy(&rpOnDemandAttr);

      /* If a callback handler is included use it to report the status of report
       * sending status */
      ...
      T2Error(
          "Failed to lock as previous upload request is already in progress\n");
      if (NULL != asyncCallBackHandle) {
        ...
        rbusValue_Init(&value);
        rbusValue_SetString(value,
                            "PREVIOUS_UPLOAD_IN_PROGRESS_REQUEST_FAILED");
        rbusObject_SetValue(outputParams, "UPLOAD_STATUS", value);

        T2Info("Sending response as PREVIOUS_UPLOAD_IN_PROGRESS_REQUEST_FAILED "
               "for %s from %s \n",
               methodName, __FUNCTION__);
        err = rbusMethod_SendAsyncResponse(asyncCallBackHandle,
                                           RBUS_ERROR_SUCCESS, outputParams);
        if (err != RBUS_ERROR_SUCCESS) {
          T2Error("%s rbusMethod_SendAsyncResponse failed err:%d\n",
                  __FUNCTION__, err);
        } else {
...
#ifdef DCMAGENT
static void rbusReloadConf(rbusHandle_t handle, rbusEvent_t const *event,
...
int getRbusDCMEventStatus() { return dcmEventStatus; }
...
T2ERROR registerRbusDCMEventListener() {
  ...
  rbusDataElement_t dataElements[2] = {
      {T2_EVENT_DCM_SETCONF,
       RBUS_ELEMENT_TYPE_EVENT,
       {NULL, NULL, NULL, NULL, (rbusEventSubHandler_t)eventSubHandler, NULL}},
      {T2_EVENT_DCM_PROCCONF,
       RBUS_ELEMENT_TYPE_EVENT,
       {NULL, NULL, NULL, NULL, (rbusEventSubHandler_t)eventSubHandler, NULL}}};
  ...
  ret = rbusEvent_Subscribe(t2bus_handle, T2_DCM_RELOAD_EVENT, rbusReloadConf,
                            NULL, 0);
  if (ret != RBUS_ERROR_SUCCESS) {
...
T2ERROR registerRbusT2EventListener(TelemetryEventCallback eventCB) {
  ...
  rbusDataElement_t dataElements[3] = {
      {T2_EVENT_PARAM,
       ...
       {NULL, t2PropertyDataSetHandler, NULL, NULL, NULL, NULL}},
      {T2_PROFILE_UPDATED_NOTIFY,
       RBUS_ELEMENT_TYPE_EVENT,
       {NULL, NULL, NULL, NULL, (rbusEventSubHandler_t)eventSubHandler, NULL}},
      {T2_OPERATIONAL_STATUS,
       RBUS_ELEMENT_TYPE_PROPERTY,
       {t2PropertyDataGetHandler, NULL, NULL, NULL, NULL, NULL}}};
...
/**
 * Register data elements for COMPONENT marker list over bus for common lib
 * event filtering. Data element over bus will be of the format
 * :Telemetry.ReportProfiles.<componentName>.EventMarkerList
 */
T2ERROR regDEforCompEventList(const char *componentName,
                              T2EventMarkerListCallback callBackHandler) {
  ...
  if (compTr181ParamMap == NULL)
    ...
    char *existingProperty =
        (char *)hash_map_get(compTr181ParamMap, componentName);
    if (existingProperty != NULL) {
      T2Warning(
          "The paramenter exist with compName : %s, the registered paramenter "
          "is %s, skipping registration to avoid duplicate registration... \n",
  ...
  snprintf(deNameSpace, 124, "%s%s%s", T2_ROOT_PARAMETER, componentName,
           T2_EVENT_LIST_PARAM_SUFFIX);
  ...
  if (ret == RBUS_ERROR_SUCCESS) {
    ...
    hash_map_put(compTr181ParamMap, (void *)strdup(deNameSpace),
                 (void *)strdup(componentName), free);
 ...
 * Data element over bus will be of the format
 * :Telemetry.ReportProfiles.<componentName>.EventMarkerList
 */
void unregisterDEforCompEventList() {
  ...
  if (count > 0) {
    ...
    if (RBUS_ERROR_SUCCESS !=
        rbus_unregDataElements(t2bus_handle, count, dataElements)) {
...
T2ERROR regDEforProfileDataModel(callBackHandlers *cbHandlers) {
  ...
  rbusDataElement_t dataElements[NUM_PROFILE_ELEMENTS] = {
      {deNameSpace,
       RBUS_ELEMENT_TYPE_PROPERTY,
       {t2PropertyDataGetHandler, t2PropertyDataSetHandler, NULL, NULL, NULL,
        NULL}},
      {deMsgPck,
       RBUS_ELEMENT_TYPE_PROPERTY,
       {t2PropertyDataGetHandler, t2PropertyDataSetHandler, NULL, NULL, NULL,
      ...
      {deReportonDemand,
       ...
       {NULL, NULL, NULL, NULL, NULL, dcmOnDemandMethodHandler}},
      {deAbortReportonDemand,
       RBUS_ELEMENT_TYPE_METHOD,
       {NULL, NULL, NULL, NULL, NULL, dcmOnDemandMethodHandler}},
      {dePrivacyModesnotshare,
       RBUS_ELEMENT_TYPE_PROPERTY,
       {t2PropertyDataGetHandler, t2PropertyDataSetHandler, NULL, NULL, NULL,
        NULL}}};
...
T2ERROR publishEventsProfileUpdates() {
  ...
  T2Debug(
      "Publishing event for t2profile update notification to components \n");
  rbusValue_Init(&value);
...
void registerConditionalReportCallBack(
    triggerReportOnCondtionCallBack triggerConditionCallback) {
  if (!reportOnConditionCallBack)
    reportOnConditionCallBack = triggerConditionCallback;
}

void reportEventHandler(rbusHandle_t handle, rbusEvent_t const *event,
                        rbusEventSubscription_t *subscription) {
  (void)handle;
  ...
  eventCallBack(eventName ? (char *)strdup(eventName) : NULL,
                eventValue ? (char *)strdup(eventValue)
                           : (char *)strdup("NOVALUE"));
  if (eventValue != NULL) {
...
void triggerCondtionReceiveHandler(rbusHandle_t handle,
  ...
  if (NULL == eventName) {
    ...
    T2Warning(
        "%s : eventName is published as null, ignoring trigger condition \n ",
        __FUNCTION__);
    return;
  }
  ...
  if (NULL == eventValue) {
    T2Warning(
        "%s : eventValue is published as null, ignoring trigger condition \n ",
        __FUNCTION__);
    return;
  }
...
T2ERROR rbusT2ConsumerReg(Vector *triggerConditionList) {
  ...
  for (j = 0; j < triggerConditionList->count; j++) {
    TriggerCondition *triggerCondition =
        ((TriggerCondition *)Vector_At(triggerConditionList, j));
    if (triggerCondition) {
...
T2ERROR rbusT2ConsumerUnReg(Vector *triggerConditionList) {
  ...
  for (j = 0; j < triggerConditionList->count; j++) {
    ...
    if (strcmp(triggerCondition->oprator, "any") == 0) {
      rc = rbusEvent_Unsubscribe(t2bus_handle, triggerCondition->reference);
...
T2ERROR T2RbusConsumer(TriggerCondition *triggerCondition) {
  ...
  rbusFilter_RelationOperator_t filterOperator =
      RBUS_FILTER_OPERATOR_EQUAL; // Initialized with a default value
  rbusFilter_t filter;
  rbusValue_t filterValue;
  rbusEventSubscription_t subscription = {triggerCondition->reference,
                                          NULL,
                                          0,
                                          0,
  ...
  if (strcmp(triggerCondition->oprator, "any") == 0) {
    T2Debug("filterOperator %s , threshold %d \n", triggerCondition->oprator,
            triggerCondition->threshold);
    rc = rbusEvent_Subscribe(t2bus_handle, triggerCondition->reference,
                             triggerCondtionReceiveHandler, user_data, 0);
    if (rc != RBUS_ERROR_SUCCESS) {
    ...
    T2Debug("Ex filterOperator %s ( %d ) , threshold %d \n",
            triggerCondition->oprator, filterOperator,
            triggerCondition->threshold);
    rbusValue_Init(&filterValue);
    rbusValue_SetInt32(filterValue, triggerCondition->threshold);
    rbusFilter_InitRelation(&filter, filterOperator, filterValue);
...
T2ERROR T2RbusReportEventConsumer(char *reference, bool subscription) {
  ...
  int rc = RBUS_ERROR_SUCCESS;
  if (!subscription) {
    rc = rbusEvent_Unsubscribe(t2bus_handle, reference);
    if (rc != RBUS_ERROR_SUCCESS)
    ...
    strcpy(user_data, "Not used");
    rc = rbusEvent_Subscribe(t2bus_handle, reference, reportEventHandler,
                             user_data, 0);
    if (rc != RBUS_ERROR_SUCCESS) {
      T2Error(" %s Subscribe failed\n", __FUNCTION__);
      ret = T2ERROR_FAILURE;
...
rbusError_t t2TriggerConditionGetHandler(rbusHandle_t handle,
                                         rbusProperty_t property,
                                         rbusGetHandlerOptions_t *opts) {
...
T2ERROR rbusMethodCaller(char *methodName, rbusObject_t *inputParams,
  ...
  rc = rbusMethod_InvokeAsync(t2bus_handle, methodName, *inputParams,
                              rbusMethodCallBack, RBUS_METHOD_TIMEOUT);
  if (rc == RBUS_ERROR_SUCCESS) {
    ret = T2ERROR_SUCCESS;
  } else {
    T2Error("rbusMethod_InvokeAsync invocation from %s failed with error code "
            "%d \n",
            __FUNCTION__, rc);
  }
  T2Debug("%s --out\n", __FUNCTION__);
