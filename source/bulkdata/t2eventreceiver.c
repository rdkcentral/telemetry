        ...
        if (event) {
          ...
          if (token != NULL) {
            ...
            if (t2_queue_count(eQueue) > T2EVENTQUEUE_MAX_LIMIT) {
              ...
              T2Debug(
                  "Adding eventName : %s eventValue : %s to t2event queue\n",
...
void T2ER_Push(char *eventName, char *eventValue) {
  ...
  if (EREnabled) {
    if (!eventName || !eventValue) {
      ...
      pthread_mutex_lock(&erMutex);
      if (t2_queue_count(eQueue) > T2EVENTQUEUE_MAX_LIMIT) {
        T2Warning("T2EventQueue max limit : %d reached, dropping packet for "
                  "eventName : %s eventValue : %s\n",
  ...
  while (!stopDispatchThread) {
    ...
    if (t2_queue_count(eQueue) > 0) {
      ...
      if (T2ERROR_SUCCESS == getMarkerProfileList(event->name, &profileList)) {
        T2Debug("Found matching profileIDs for event with markerName : %s "
                "value : %s\n",
                event->name, event->value);
        size_t index = 0;
        for (; index < Vector_Size(profileList); index++) {
          ...
          ReportProfiles_storeMarkerEvent((char *)Vector_At(profileList, index),
                                          event);
        }
      } else {
        T2Debug("No Matching Profiles for event with MarkerName : %s Value : "
                "%s - Ignoring\n",
                event->name, event->value);
      }
