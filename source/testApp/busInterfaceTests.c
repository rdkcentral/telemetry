...
static const char *TR181_SYND_SETTINGS =
...
void testGetProfileParameterValues() {
  ...
  printf("Expect device model value, syndication partner settings value and "
         "again device model value\n\n");
    ...
    for (i = 0; i < Vector_Size(paramList); ++i) {
      ...
      if (retValObj) {
        ...
        if (paramValueCount == 0) {
          printf("Value for param  %s = %s \n", (valObjArr[0])->parameterName,
                 (valObjArr[0])->parameterValue);
        } else {
          for (iterate = 0; iterate < paramValueCount; ++iterate) {
            printf("Value for param  %s = %s \n",
                   (valObjArr[iterate])->parameterName,
                   (valObjArr[iterate])->parameterValue);
          }
        }
      }
    }
    // Vector_Destroy(&pValueList, freeProfileValues); // Ignore for testapp due
    // to interdependency
  ...
  // Vector_Destroy(&paramList, freeParam); // Ignore for testapp due to
  // interdependency
  printf("\n\n Exiting %s ====>\n", __FUNCTION__);
}
