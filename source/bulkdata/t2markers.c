...
T2ERROR addT2EventMarker(const char *markerName, const char *compName,
  ...
  // We use this in grep to reduce the log search. we can implement this based
  // on need.
  pthread_mutex_lock(&t2MarkersMutex);
  T2Marker *t2Marker = (T2Marker *)hash_map_get(markerCompMap, markerName);
  // pthread_mutex_unlock(&t2MarkersMutex); unlock after modification is done on
  // t2Marker
  if (t2Marker) {
    ...
    if (t2Marker->profileList) {
      ...
      if (isPresent != true) {
        T2Debug("Found a matching T2Marker, adding new profileName %s to "
                "profileList or marker %s \n",
...
void getComponentMarkerList(const char *compName, void **markerList) {
  ...
  if (T2ERROR_SUCCESS == Vector_Create(&compMarkers)) {
    ...
    *markerList = (Vector *)compMarkers;
    ...
    T2Error(
        "Unable to create Vector for Component Markers :: Malloc failure\n");
    return;
  }
  return;
}
...
T2ERROR getMarkerProfileList(const char *markerName, Vector **profileList) {
  ...
  if (t2Marker != NULL && t2Marker->profileList != NULL) {
    ...
    for (; i < count; ++i) {
      Vector_PushBack(*profileList,
                      strdup(Vector_At(t2Marker->profileList, i)));
    }
  } else {
    pthread_mutex_unlock(&t2MarkersMutex);
    return T2ERROR_FAILURE;
