 ...
 */
static int parseMarkerListOptimized(GrepSeekProfile *gsProfile, Vector * ip_vMarkerList, Vector * out_grepResultList, bool check_rotated, char* logPath, bool isCustomPath)
{
    T2Debug("%s ++in \n", __FUNCTION__);
...
int getDCAResultsInVector(GrepSeekProfile *gSeekProfile, Vector * vecMarkerList, Vector** out_grepResultList, bool check_rotated, char* customLogPath)
{
    ...
    if(NULL != vecMarkerList)
    {
        ...
        if( (rc = parseMarkerListOptimized(gSeekProfile, vecMarkerList, *out_grepResultList, check_rotated, logPath, isCustomPath)) == -1 )
        {
