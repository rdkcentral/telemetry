 ...
 * @brief       rbusValue
 * @defgroup    rbusValue
 * @brief       An rbus value is a variant data type representing a fundamental
 * piece of data passed through rbus. An rbusValue_t is a reference counted
 * handle to an rbus value.
 *
 ...
 *  @param  Optional pvalue reference to an address where the new value will be
 * assigned. The caller is responsible for releasing the value with
 * rbusValue_Release
 ...
 * @brief These functions returns the data stored in the value according to
 * their type. The data is not coerced to the type, thus the caller should
 * ensure the value's actual type matches the function being called.  The caller
 * can call rbusValue_GetType to get the type.
 ...
 *         multiple owners.  The first owner obtains ownership with
 * rbusValue_Init. Additional owners can be assigned afterwards with
 * rbusValue_Retain. Each owner must call rbusValue_Release once done using the
 * value.
 ...
 *         a value that was retained with either rbusValue_Init or
 * rbusValue_Retain.
 ...
 *  @brief Compare two values for equality.  They are equal if both the type and
 * data are equal.
 *  @param value1 the first value to compare
 *  @param value2 the second value to compare
 *  @return The compare result where 0 is equal and non-zero if not equal.  If
 * value1
 */
 ...
 *         Parameters buf and buflen are optional and allow the caller to pass
 * in a buffer to write the string to.  If parameter buf is NULL, this method
 * will allocate a buffer to write the string to and the caller should call free
 * to deallocate the buffer. For DateTime datatype, below ISO-8601 formats as
 * per TR-069_Amendment-6 will be used for printing. YYYY-MM-DDThh:mm:ssZ
 ...
 *  @return A null-terminated string.  The caller must call free on this if
 * parameter buf was NULL.
 */
char *rbusValue_ToString(rbusValue_t value, char *buf, size_t buflen);

/** @fn char* rbusValue_ToDebugString(rbusValue_t value, char* buf, size_t
 * buflen)
 *  @brief Returns a null terminated string representing the type and data of
 * the value. Parameters buf and buflen are optional and allow the caller to
 * pass in a buffer to write the string to.  If parameter buf is NULL, this
 * method will allocate a buffer to write the string to and the caller should
 * call free to deallocate the buffer.
 *  @param value the value to convert to a string
 *  @param buf optional buffer to write the string to
 *  @param buflen the length of buf if buf was supplied, otherwise ignored
 *  @return A null-terminated string.  The caller must call free on this if
 ...
 *        the value's actual type matches the function being called.  The caller
 * can call rbusValue_GetType to get the type.
 * @param value A value to get data from.
 * @return The data as a specific type. This is meant to be a const accessor.
 *         To avoid large copies, if the type is a buffer or struct type,
...
rbusValueError_t rbusValue_GetTimeEx(rbusValue_t value,
                                     rbusDateTime_t const **tv);
rbusValueError_t rbusValue_GetStringEx(rbusValue_t value, char const **s,
                                       int *len);
rbusValueError_t rbusValue_GetBytesEx(rbusValue_t value, uint8_t const **bytes,
                                      int *len);
rbusValueError_t rbusValue_GetPropertyEx(rbusValue_t value,
                                         struct _rbusProperty **property);
rbusValueError_t rbusValue_GetObjectEx(rbusValue_t value,
...
///  @brief rbusValue_SetFromString sets the value's type to given type and data
///  to appropriate by converting the string
/**
 *  @brief Sets the type and data of a value by converting a string
 * representation of the data to a specific typed data.
 *  @param value A value to set.
 *  @param type The type of data represented by the input string and to which
 * the value will be assigned.
 *  @param str A string representation of the data which will be coerced to the
 * type specified by the type param and assigned to the value.
 *  @return bool true if this function succeeds to coerce the type and set the
 * value or false if it fails
 */
bool rbusValue_SetFromString(rbusValue_t value, rbusValueType_t type,
                             const char *str);

/** @fn void rbusValue_fwrite(rbusValue_t obj, int depth, FILE* fout)
 *  @brief A debug utility function to write the value as a string to a file
 * stream.
 */
void rbusValue_fwrite(rbusValue_t obj, int depth, FILE *fout);

/**
 *  @brief Convert a local "struct tm" type to an explicit length version, which
 * allows 64/32 bit platform message interchange
 *  @param invalue An incoming struct tm
 *  @param outvalue An outgoing explicit length time structure
 */
void rbusValue_MarshallTMtoRBUS(rbusDateTime_t *outvalue,
                                const struct tm *invalue);

/**
 *  @brief Convert an explicit length time structure to a local "struct tm",
 * which allows 64/32 bit platform message interchange
 *  @param invalue An incoming explicit length time structure
 *  @param outvalue An outgoing struct tm
 */
void rbusValue_UnMarshallRBUStoTM(struct tm *outvalue,
                                  const rbusDateTime_t *invalue);
